...
- **法律检索新增结构化事件观测链路**：新增任务事件模型、迁移与统一事件写入服务，支持请求/响应摘要、状态码、耗时、错误信息记录
- **任务详情可视化增强**：新增 API 阶段指标、流程时间线、接口返回可视化面板，并按能力门禁控制展示
- **检索稳定性增强**：检索链路增加短时熔断与冷却策略，分页空页不再错误回退，减少抖动
- **评测基线升级**：benchmark 输出新增按查询类型贡献率统计（primary/expansion/feedback），基线样本结构升级为 v2

## 📌 上一版本更新（26.16.5）

- **organization 模块代码规范整改**
  - Service 层内部方法规范化为公共查询方法
  - Adapter 层统一改为调用公共方法
  - API 层统一补齐 `_get_xxx_service()` 工厂函数
  - 相关测试通过，行为保持兼容
