...
- **组织管理** - 团队、律师、账号凭证管理
- **法律检索** - 案例关键词联合检索、相似案例匹配、PDF 附件归档下载
- **自动化功能**
  - 法院短信解析与文书下载
  - 法院文书自动抓取
  - 财产保全保险询价
...
## 🔎 法律案例检索（legal_research）

- 入口：Django Admin 的 `legal_research` 任务管理
- 输入：检索关键词（支持空格、逗号、分号、换行，系统会标准化为空格并做联合检索）+ 案情简述
- 可配置：
  - `max_candidates`：最大扫描案例数，默认 `100`
  - `min_similarity_score`：最低相似度阈值，默认 `0.9`
  - `llm_model`：硅基流动模型（用于相似度评估）
- 执行保障：
  - 任务提交前会先检查硅基流动连通性与模型可用性
  - 预检失败时任务不会入队，可在任务详情页改模型后直接重试
- 任务可控性：
  - 任务详情页支持“取消任务”按钮（`pending/queued/running` 可用）
  - 对于 `queued` 任务会尝试从 Django-Q 队列撤销；`running` 任务会在下一轮扫描检查时停止
- 进度可解释性：
  - 任务详情页会展示“候选池提示”
  - 当关键词总候选不足时，会明确提示“当前关键词仅检索到 N 篇候选案例（设置上限为 M）”
- 结果产出：命中案例会附带 PDF，可在任务详情页逐条下载或打包下载
