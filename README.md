...
- 案例检索完成“命中率与稳定性”一轮系统升级：二阶段匹配、双模型复核、自适应阈值、在线反馈闭环全部落地
- 检索策略升级为“词法优先 + 语义动态触发”，仅在低置信样本启用语义向量复核，兼顾速度与召回
- 查询扩展增强：同义词扩展、LLM 检索式变体、伪相关反馈反哺检索式
- 性能优化：案例详情缓存、相似度结果缓存、语义向量缓存，降低重复任务成本
- 可运维性增强：新增检索基准命令与基线样本，支持持续 A/B 调参与回归验证
