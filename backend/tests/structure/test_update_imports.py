...
def test_import_pattern_matching():
    ...
    test_cases = [
        # (输入行, 预期是否匹配, 预期新行)
        ("from tests.factories import CaseFactory", True, "from tests.factories import CaseFactory"),
        (
            "from tests.factories.case_factories import CaseFactory",
            True,
            "from tests.factories.case_factories import CaseFactory",
        ),
        ("import tests.factories", True, "import tests.factories"),
        ("from tests.unit.cases import TestCase", True, "from tests.unit.cases import TestCase"),
        ("from tests.mocks import MockService", True, "from tests.mocks import MockService"),
        ("from apps.core.exceptions import ValidationError", False, None),  # 不应该匹配
    ]
...
def test_import_analysis():
    ...
    # 创建临时测试文件（使用旧的导入路径，updater 应该能识别并更新）
    ...
    try:
        ...
        # 验证结果
        assert len(updates) == 0, f"应该找到 0 处更新，实际找到 {len(updates)} 处"
