...
class TestEmptyDirectoryRemoval:
    ...
    def test_no_empty_test_directories_in_apps(self, backend_root: Path):
        ...
        *For any* `apps/*/tests/` directory that only contains `__init__.py`,
        ...
        # 断言：不应该有空的测试目录
        assert len(empty_test_dirs) == 0, (
            f"发现 {len(empty_test_dirs)} 个空的测试目录应该被删除:\n" +
            "\n".join(str(d) for d in empty_test_dirs)
        )
    
    @given(st.sampled_from([
        "cases", "client", "contracts", "organization", "core", "automation"
    ]))
    @settings(max_examples=6)
    @pytest.mark.hypothesis
    def test_app_test_directory_not_empty_or_removed(
        self,
        app_name: str
    ):
        """
        ...
        # 断言：如果目录存在，必须有实际内容
        assert has_content, (
            f"apps/{app_name}/tests/ 目录存在但只包含 __init__.py，应该被删除"
        )
