    ...
    for app_name in DJANGO_APPS:
        ...
        for py_file in api_dir.glob("*.py"):
            ...
            if not py_file.name.endswith("_api.py"):
                errors.append(
                    f"App {app_name}: API file {py_file.name} does not follow "
                    f"naming convention (*_api.py)"
                )
...
def test_root_api_py_is_import_only():
    ...
    for app_name in DJANGO_APPS:
        ...
        if "@router." in content and "import" not in content.split("@router.")[0].split("\n")[-1]:
            errors.append(
                f"App {app_name}: root api.py contains @router decorator. "
                f"API routes should be in api/ subdirectory"
            )
    ...
    for app_name in DJANGO_APPS:
        ...
        for api_file in api_dir.glob("*_api.py"):
            ...
            if "router = Router(" not in content and "router=Router(" not in content:
                errors.append(
                    f"App {app_name}: API file {api_file.name} does not define a Router instance"
                )
