...
class TestTestCentralization:
    ...
    def test_factories_in_centralized_factories_directory(self, backend_root: Path):
        ...
        assert len(factory_files_in_tests) > 0, (
            "tests/factories/ 应该包含 factory 文件"
        )
    ...
    def test_mocks_in_centralized_mocks_directory(self, backend_root: Path):
        ...
        assert len(mock_files_in_tests) > 0, (
            "tests/mocks/ 应该包含 mock 文件"
        )
    
    @given(st.sampled_from(["unit", "integration", "property"]))
    @pytest.mark.hypothesis
    def test_test_directory_structure_consistency(
        self,
        test_type: str
    ):
        """
        ...
        # 断言：所有子目录应该是有效的模块名
        assert len(invalid_subdirs) == 0, (
            f"发现 {len(invalid_subdirs)} 个无效的子目录在 {test_type}/:\n" +
            "\n".join(str(d) for d in invalid_subdirs)
        )
    ...
    def test_strategies_in_centralized_strategies_directory(self, backend_root: Path):
        ...
        assert len(strategy_files_in_tests) > 0, (
            "tests/strategies/ 应该包含 strategy 文件"
        )
    ...
    def test_apps_tests_directory_only_contains_init(self, backend_root: Path):
        ...
        assert len(remaining_items) == 0, (
            f"apps/tests/ 目录应该只包含 __init__.py，但发现以下文件/目录:\n" +
            "\n".join(str(item) for item in remaining_items)
        )
