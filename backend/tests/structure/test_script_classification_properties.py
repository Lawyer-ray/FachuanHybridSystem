...
class TestScriptClassificationProperties:
    ...
    def test_development_scripts_in_development_directory(self, scripts_dir):
        ...
        if development_dir.exists():
            ...
            for script in dev_scripts:
                has_dev_prefix = any(
                    script.name.startswith(prefix) for prefix in dev_prefixes
                )
                assert has_dev_prefix, (
                    f"Script {script.name} in development/ should start with one of: "
                    f"{', '.join(dev_prefixes)}"
                )
        ...
        if refactoring_dir.exists():
            ...
            for script in refactoring_scripts:
                ...
                has_refactoring_keyword = (
                    'migrate' in script.name.lower() or
                    'refactor' in script.name.lower() or
                    'validator' in script.name.lower() or
                    'structure' in script.name.lower()
                )
    ...
    def test_script_naming_convention_property(self, script_name):
        ...
        if category is not None:
            assert category in ['testing', 'development', 'automation', 'refactoring'], (
