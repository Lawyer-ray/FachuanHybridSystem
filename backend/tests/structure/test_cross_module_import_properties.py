...
def check_di_constructor_signature(
    module_name: str,
    class_name: str,
...
def test_no_cross_module_model_imports_in_cases():
    ...
    **Validates: Requirements 6.1**
    ...
    assert len(all_violations) == 0, (
        f"发现 {module_name}/services 中存在跨模块 Model 导入:\n" +
        "\n".join(
            f"  {Path(path).name}:{line} - 导入了 {mod}.models: {stmt}"
...
def test_no_cross_module_model_imports_property(module_name: str):
    ...
    **Validates: Requirements 1.3, 6.1, 6.2, 6.3, 6.4**
    ...
    assert len(all_violations) == 0, (
        f"发现 {module_name}/services 中存在跨模块 Model 导入:\n" +
        "\n".join(
            f"  {Path(path).name}:{line} - 导入了 {mod}.models"
            for path, line, mod, stmt in all_violations[:5]
        )
    )
...
def test_contract_service_has_di_constructor():
    ...
    **Validates: Requirements 5.1**
    """
    passed, error = check_di_constructor_signature(
        'contracts',
        'ContractService',
        ['case_service']
    )
