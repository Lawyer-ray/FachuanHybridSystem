...
def test_root_directory_simplicity_property(item_name):
    ...
    allowed_items = {
        ...
        # 配置文件
        ".env.example", ".env", ".env.dev", ".gitignore", ".pre-commit-config.yaml",
        "conftest.py", "pytest.ini", "mypy.ini", "pyproject.toml", "ruff.toml",
        "requirements.txt", "Makefile", "README.md",
        ".secrets.baseline",
        # 系统文件
        ".DS_Store", ".git", "venv311", "venv312", ".venv", "venv",
        # 临时文件（应该在 .gitignore 中）
        ".coverage", "IMPLEMENTATION_CHECKLIST.md",
        # IDE 配置
        ".idea", ".vscode", ".trae", ".kiro",
        # 缓存与运行时目录
        ".cache", ".ruff_cache", "media",
        # 遗留目录（待清理）
        "backend",
        # 遗留文档文件（待迁移到 docs/）
...
def test_root_directory_has_required_directories():
    ...
    missing_required = [
        ...
        if "Missing required directory" in error
    ]
    
    assert len(missing_required) == 0, (
        f"Root directory is missing required directories:\n" +
        "\n".join(f"  - {error}" for error in missing_required)
    )
...
def test_root_directory_only_contains_allowed_items():
    ...
    unexpected_items = [
        error for error in errors
        if "Unexpected item" in error
    ]
    
    assert len(unexpected_items) == 0, (
        f"Root directory contains unexpected items:\n" +
        "\n".join(f"  - {error}" for error in unexpected_items)
    )
...
def test_root_directory_structure_complete():
    ...
    assert len(errors) == 0, (
        f"Root directory validation failed:\n" +
...
def test_essential_config_files_exist():
    ...
    assert len(missing_files) == 0, (
        f"Root directory is missing essential config files:\n" +
...
def test_cache_directories_in_gitignore():
    ...
    assert len(missing_in_gitignore) == 0, (
        f"The following patterns should be in .gitignore:\n" +
        "\n".join(f"  - {pattern}" for pattern in missing_in_gitignore)
    )
