...
def test_app_structure_consistency_property(app_name):
    ...
    # 断言：不应该有错误
    assert len(errors) == 0, (
        f"App '{app_name}' structure validation failed:\n" +
...
def test_all_apps_have_required_directories():
    ...
    assert len(failed_apps) == 0, (
        f"The following apps have structure issues:\n" +
        "\n".join(
            f"{app}:\n" + "\n".join(f"  - {error}" for error in errors)
            for app, errors in failed_apps.items()
        )
    )
...
def test_all_apps_have_required_files():
    ...
    assert len(missing_files) == 0, (
        f"The following apps are missing required files:\n" +
        "\n".join(
            f"{app}: {', '.join(files)}"
            for app, files in missing_files.items()
        )
    )
...
def test_app_admin_directory_structure():
    ...
    assert len(issues) == 0, (
        f"The following apps have admin directory issues:\n" +
        "\n".join(f"{app}: {issue}" for app, issue in issues.items())
    )
...
def test_app_api_directory_structure():
    ...
    assert len(issues) == 0, (
        f"The following apps have api directory issues:\n" +
        "\n".join(f"{app}: {issue}" for app, issue in issues.items())
    )
...
def test_app_services_directory_structure():
    ...
    assert len(issues) == 0, (
        f"The following apps have services directory issues:\n" +
        "\n".join(f"{app}: {issue}" for app, issue in issues.items())
    )
