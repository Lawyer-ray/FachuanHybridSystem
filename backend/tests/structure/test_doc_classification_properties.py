...
def test_documentation_classification_property():
    ...
    for doc_file in doc_files:
        ...
        if actual_subdir not in VALID_DOC_SUBDIRS:
            errors.append(
                f"文档文件在无效的子目录中: {doc_file} "
                f"(子目录: {actual_subdir}, 有效子目录: {VALID_DOC_SUBDIRS})"
            )
        ...
        if not expected_path.exists():
            ...
            if found_paths:
                ...
                try:
                    ...
                    errors.append(
                        f"文档文件 {filename} 在错误的位置: {rel_path} "
                        f"(应该在: {expected_dir})"
                    )
                except ValueError:
...
def test_no_docs_in_root_directory():
    ...
    # 获取根目录下的所有 .md 文件
    root_md_files = [
        f for f in root_path.glob("*.md")
        if f.name != "README.md"
    ]
    
    # 断言根目录下没有其他 .md 文件
    assert not root_md_files, (
        f"根目录不应该包含文档文件（除了 README.md）:\n" +
