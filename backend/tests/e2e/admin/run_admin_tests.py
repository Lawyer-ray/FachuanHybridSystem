...
async def run_test_class(test_class, test_name: str):
    ...
    try:
        ...
        # 获取所有测试方法
        test_methods = [
            method for method in dir(test) if method.startswith("test_") and callable(getattr(test, method))
        ]
...
async def run_all_tests():
    ...
    # 运行所有测试套件
    all_results = {"passed": 0, "failed": 0, "skipped": 0, "errors": [], "suites": []}
...
示例:
  ...
  python run_admin_tests.py --all              # 运行所有测试（同默认）
    ...
    parser.add_argument(
        "--suite", choices=["smoke", "crud", "inline", "validation", "action", "performance"], help="运行指定的测试套件"
    )
...
async def run_crud_tests():
    """运行 CRUD 测试"""
    results = {"passed": 0, "failed": 0, "skipped": 0, "errors": [], "suites": []}
