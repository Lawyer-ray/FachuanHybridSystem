...
class BaseAdminTest:
    ...
    async def setup(self):
        ...
        self.browser = await self.playwright.chromium.launch(
            headless=False, slow_mo=100  # 显示浏览器，方便调试  # 减慢操作速度，方便观察
        )
                                ...
                                for i in range(10):  # 最多等待5秒
                                    ...
                                    if total_forms_input:
                                        ...
                                        if current_value and int(current_value) > int(total_forms_before):
                                            print(
                                                f"    ✓ 新行已创建 (TOTAL_FORMS: {total_forms_before} → {current_value})"
                                            )
                                            break
                            except Exception as e:
    ...
    async def take_screenshot(self, name: str):
        ...
        try:
            await self.page.screenshot(path=f"{screenshot_dir}/{name}.png", timeout=10000)  # 10秒超时
            print(f"    ✓ 截图已保存: {name}.png")
        except Exception as e:
            print(f"    ⚠️  截图失败: {e}")
            # 尝试不等待字体加载
            try:
                await self.page.screenshot(path=f"{screenshot_dir}/{name}.png", timeout=5000)
    ...
    async def check_validation_error(
        self, field_name: Optional[str] = None, expected_message: Optional[str] = None
    ) -> bool:
        """
        检查验证错误
    ...
    async def wait_for_validation_error(self, timeout: int = 5000, field_name: Optional[str] = None) -> bool:
        """
        等待验证错误消息出现（支持动态加载）
