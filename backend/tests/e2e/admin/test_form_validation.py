...
from .validation_scenario import ValidationScenario, Stage4TestReport, ValidationTestResult, ValidationError
...
async def test_party_uniqueness_validation():
    ...
    try:
        ...
        try:
            ...
            for error in errors:
                errors_detected.append(ValidationError(
                    field=error['field'],
            ...
            # 判断测试是否通过
            passed = (
                len(errors_detected) > 0 and  # 检测到了错误
                len(found_keywords) >= 2 and  # 至少找到2个关键词
                success and                    # 修正后保存成功
                no_errors                      # 修正后没有错误
            )
