...
class TestInlineForms(BaseAdminTest):
    ...
    async def test_case_add_single_party(self):
        ...
        try:
            ...
            await self.select_option(
                self.get_inline_field_name('caseparty_set', 0, 'client'),
                '16'
            )
            await self.select_option(
                self.get_inline_field_name('caseparty_set', 0, 'legal_status'),
                'plaintiff'
            )
    ...
    async def test_case_add_multiple_parties(self):
        ...
        try:
            ...
            await self.add_inline_row('caseparty_set')
            await self.select_option(
                self.get_inline_field_name('caseparty_set', 1, 'client'),
                '17'
            )
            await self.select_option(
                self.get_inline_field_name('caseparty_set', 1, 'legal_status'),
                'defendant'
            )
    ...
    async def test_case_add_assignment(self):
        ...
        try:
            ...
            await self.select_option(
                self.get_inline_field_name('caseassignment_set', 0, 'lawyer'),
                '67'
            )
    ...
    async def test_contract_add_with_case_inline(self):
        ...
        try:
            ...
            await self.fill_field(
                self.get_inline_field_name('cases', 0, 'name'),
                '测试案件 - 从合同创建'
            )
    ...
    async def test_contract_nested_inline(self):
        ...
        try:
            ...
            try:
                ...
                await self.select_option(
                    'cases-0-parties-0-client',
                    '16'
                )
                await self.select_option(
                    'cases-0-parties-0-legal_status',
                    'plaintiff'
                )
    ...
    async def test_client_add_identity_doc(self):
        ...
        try:
            ...
            await self.fill_field_smart(
                self.get_inline_field_name('clientidentitydoc_set', 0, 'doc_type'),
                'id_card'
            )
