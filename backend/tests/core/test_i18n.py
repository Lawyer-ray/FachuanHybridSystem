    ...
    def test_all_model_verbose_names_are_lazy(self) -> None:
        ...
        for model in apps.get_models():
            ...
            if not isinstance(vn, Promise):
                non_lazy.append(f"{meta.app_label}.{meta.model_name}: verbose_name={vn!r}")
        self.assertEqual(
            non_lazy,
            [],
...
class TestUnsupportedLanguageRejection(HypothesisTestCase):
    ...
    @given(lang=st.text(min_size=1, max_size=20).filter(lambda s: s.isascii() and s not in {"zh-hans", "en"}))
