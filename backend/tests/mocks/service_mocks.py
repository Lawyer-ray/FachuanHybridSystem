...
from apps.core.interfaces import CaseDTO, ContractDTO
...
class MockCaseService(MockService):
    ...
    def check_case_access(self, user, case_id: int) -> bool:
        """检查案件访问权限"""
        result = self._get_mock_return("check_case_access", {"user": user, "case_id": case_id})
        return result if result is not None else True
...
class MockPermissionService(MockService):
    ...
    def can_create_resource(self, user, resource_type: str) -> bool:
        """检查是否可以创建资源"""
        result = self._get_mock_return("can_create_resource", {"user": user, "resource_type": resource_type})
        return result if result is not None else True

    def can_access_resource(self, user, resource) -> bool:
        """检查是否可以访问资源"""
        result = self._get_mock_return("can_access_resource", {"user": user, "resource": resource})
        return result if result is not None else True

    def can_modify_resource(self, user, resource) -> bool:
        """检查是否可以修改资源"""
        result = self._get_mock_return("can_modify_resource", {"user": user, "resource": resource})
        return result if result is not None else True

    def can_delete_resource(self, user, resource) -> bool:
        """检查是否可以删除资源"""
        result = self._get_mock_return("can_delete_resource", {"user": user, "resource": resource})
        return result if result is not None else True
