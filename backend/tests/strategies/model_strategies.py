...
from .common_strategies import chinese_text, decimal_amount, phone_number
        ...
        "username": draw(
            st.text(
                alphabet=st.characters(
                    whitelist_categories=("Ll", "Lu", "Nd"), min_codepoint=ord("a"), max_codepoint=ord("z")
                ),
                min_size=3,
                max_size=20,
            )
...
def contract_strategy(draw):
    ...
    Returns:
        dict: 合同数据字典
    """
    from apps.cases.models import CaseStatus, CaseType
...
def case_strategy(draw):
    ...
    Returns:
        dict: 案件数据字典
    """
    from apps.cases.models import CaseStage, CaseStatus, SimpleCaseType
