...
class TestManualCaseBindingIntegration:
    ...
    @patch('apps.automation.services.court_document_recognition.case_binding_service.CaseBindingService._trigger_notification')
    ...
    @patch('apps.automation.services.court_document_recognition.notification_service.DocumentRecognitionNotificationService.send_notification')
    def test_manual_bind_triggers_notification(self, mock_send_notification):
        ...
        # 配置 mock 返回成功
        mock_send_notification.return_value = NotificationResult.success_result(
...
class TestManualCaseBindingAPIIntegration:
    ...
    def test_search_cases_api(self):
        ...
        # 搜索案件（使用正确的 API 路径）
        response = client.get(
            "/api/v1/automation/court-document/search-cases",
            {"q": "王五"}
        )
...
class TestCompleteManualBindingFlow:
    ...
    @patch('apps.automation.services.court_document_recognition.notification_service.DocumentRecognitionNotificationService.send_notification')
    def test_complete_flow_upload_recognize_fail_manual_bind_success(
        self,
