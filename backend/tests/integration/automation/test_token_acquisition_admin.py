...
class TestTokenAcquisitionAdmin(TestCase):
    ...
    def setUp(self):
        ...
        self.lawyer = User.objects.create_user(
            username="lawyer1", email="lawyer1@test.com", password="testpass123", law_firm=self.law_firm
        )
    ...
    def create_test_history_records(self):
        ...
        TokenAcquisitionHistory.objects.create(
            ...
            error_details={"error_type": "CaptchaError", "attempts": 3},
    ...
    def test_token_acquisition_history_cleanup_old_records(self):
        ...
        # 检查记录是否被删除
        self.assertFalse(TokenAcquisitionHistory.objects.filter(id=old_record.id).exists())
    ...
    def test_admin_permissions(self):
        """测试管理界面权限"""
        # 创建普通用户
        normal_user = User.objects.create_user(username="normal", email="normal@test.com", password="testpass123")
...
class TestTokenAcquisitionAdminIntegration(TestCase):
    ...
    def test_admin_integration_with_services(self):
        ...
        token_service = AutoTokenAcquisitionService(
            auto_login_service=auto_login_service, account_selection_strategy=account_strategy
        )
