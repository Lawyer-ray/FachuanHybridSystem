...
class TestDocumentRenamerIntegration:
    ...
    def test_full_workflow_with_mocked_dependencies(self, mock_chat, mock_extract):
        ...
        try:
            ...
            # Mock Ollama 响应
            mock_chat.return_value = {"message": {"content": "执行通知书"}}

            # 执行重命名
            result = self.renamer.rename(tmp_path, "张三诉李四合同纠纷案", date(2025, 12, 14))
    ...
    def test_fallback_workflow(self):
        ...
        try:
            ...
            result = self.renamer.rename_with_fallback(tmp_path, "测试案件", date(2025, 12, 14), "原始文书名称")
    ...
    def test_filename_generation_edge_cases(self):
        """测试文件名生成的边界情况"""
        # 测试特殊字符
        filename = self.renamer.generate_filename("执行通知书", "张三/李四|王五:赵六", date(2025, 12, 14))
        ...
        filename = self.renamer.generate_filename(long_title, long_case, date(2025, 12, 14))
