            ...
            for idx, item in enumerate(schema.data):
                for field in required_fields:
                    assert field in item, \
                        f"数据项 {idx} 缺少必需字段: {field}"
                    assert item[field] is not None, \
                        f"数据项 {idx} 的字段 {field} 不应该为 None"
...
class TestExceptionResponseHandlingProperties:
    ...
    def test_property_3_malformed_response_raises_validation_error(self, malformed_data):
        ...
        **Validates: Requirements 1.5**
        ...
        assert any(keyword in error_str for keyword in [
            '缺少必需字段', 'missing', 'required', 'field', '字段'
    ...
    def test_property_3_error_contains_field_information(self):
        ...
        **Validates: Requirements 1.5**
        ...
        # 验证错误信息包含多个字段的信息
        # 至少应该提到一些字段名
        field_count = sum(1 for field in ['code', 'msg', 'data', 'success', 'totalRows'] 
                         if field in error_str.lower())
        assert field_count > 0, f"错误信息应该包含字段名: {error_str}"
    
    @given(
        st.integers(min_value=-1000, max_value=1000).filter(lambda x: x != 200)
    )
    @settings(max_examples=100, deadline=None)
