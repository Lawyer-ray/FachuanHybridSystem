...
class TestAdminLayerCompliance:
        ...
        **Validates: Requirements 3.2**
        ...
        for method_name, method in all_methods:
            ...
            if method_name in [
                ...
                'get_queryset', 'get_urls', 'changelist_view', 'get_readonly_fields',
                'get_list_display', 'get_list_filter', 'get_search_fields',
                'save_model', 'delete_model', 'save_related',
                'formfield_for_dbfield', 'formfield_for_manytomany', 'get_form',
                'get_inline_formsets', 'get_search_results', 'lookup_allowed',
                'render_change_form', 'response_action', 'response_add', 
                'response_change', 'response_delete', 'to_field_allowed',
                'get_fieldsets', 'get_inline_instances', 'get_formsets_with_inlines',
            ]:
                continue
            
            # 跳过显示方法（通常是格式化显示，允许较长）
            if (method_name.endswith('_display') or method_name.endswith('_view') or
                method_name.endswith('_summary') or method_name in ['quotes_summary', 'performance_summary'] or
                method_name.startswith('_build_') or method_name.startswith('_render_') or 
                method_name.startswith('_display_') or method_name.startswith('_provide_')):
                continue
            ...
            try:
                ...
                if any(method_name.startswith(prefix) for prefix in [
                    'cleanup_', 'export_', 'retry_', 'batch_', 'reanalyze_', 'execute_'
                ]):
        ...
        try:
            ...
            for pattern in prohibited_import_patterns:
                ...
                assert not violations, (
                    f"{admin_class.__name__} 不应该直接导入其他模块: {violations}"
                )
                ...
                for factory_body in factory_matches:
                    if 'ServiceLocator' in factory_body:
                        assert correct_import in source, (
                            f"{admin_class.__name__} 工厂函数必须正确导入ServiceLocator"
                        )
    ...
    def test_property_14_admin_exception_handling_delegation(self, admin_class):
        ...
        **Validates: Requirements 3.4**
        ...
        for method_name, method in all_methods:
            # 只检查Admin Action方法
            if not any(method_name.startswith(prefix) for prefix in [
                'cleanup_', 'export_', 'retry_', 'batch_', 'reanalyze_', 'execute_'
            ]):
                continue
                ...
                for try_body, exception_type, exception_var, except_body in try_blocks:
                    ...
                    if not has_simple_handling:
                        ...
                        if 'logger.' not in except_body and 'log.' not in except_body:
                            assert False, (
                                f"{admin_class.__name__}.{method_name} 异常处理应该包含用户消息显示"
                            )
        ...
        **Validates: Requirements 3.5**
        ...
        try:
            ...
            if admin_actions:
                ...
                assert factory_calls, (
                    f"{admin_class.__name__} 包含Admin Action但没有使用工厂函数获取AdminService"
                )
    ...
    def test_admin_layer_comprehensive_compliance_check(self):
        ...
        compliance_report = {
            "total_admins": len(admin_classes),
            "compliant_admins": 0,
            "violations": []
        }
        ...
        for admin_class in admin_classes:
            ...
            try:
                ...
                for method_name, method in all_methods:
                    ...
                    if (method_name.endswith('_display') or method_name.endswith('_view') or
                        method_name.endswith('_summary') or method_name.startswith('_build_') or 
                        method_name.startswith('_render_') or method_name.startswith('_display_') or 
                        method_name.startswith('_provide_') or method_name in [
                            'has_add_permission', 'has_change_permission', 'has_delete_permission',
                            'get_queryset', 'get_urls', 'changelist_view', 'get_readonly_fields',
                            'get_list_display', 'get_list_filter', 'get_search_fields',
                            'save_model', 'delete_model', 'save_related',
                            ...
                            'response_change', 'response_delete', 'to_field_allowed',
                            'get_fieldsets', 'get_inline_instances', 'get_formsets_with_inlines',
                            'quotes_summary', 'performance_summary',
                        ]):
                        continue
        ...
        compliance_rate = (
            ...
            if compliance_report["total_admins"] > 0 else 0
        )
...
def compliant_admin_action_source(draw):
    ...
    action_name = draw(st.sampled_from([
        'cleanup_old_records', 'export_to_csv', 'retry_failed_tasks',
        'batch_delete_items', 'reanalyze_performance'
    ]))
...
def non_compliant_admin_action_source(draw):
    ...
    action_name = draw(st.sampled_from([
        'cleanup_old_records', 'export_to_csv', 'batch_process'
    ]))
        ...
        for func_def in action_functions:
            ...
            for try_block in try_blocks:
                for handler in try_block.handlers:
                    ...
                    assert handler_statements <= 3, (
                        f"符合规范的Admin Action异常处理应该简单，不超过3个语句"
                    )
                            ...
                            if hasattr(target, 'id') and target.id == 'service':
                                if isinstance(node.value, ast.Call):
                                    if hasattr(node.value.func, 'id'):
                                        ...
                                        if func_name_called.endswith('Service') and not func_name_called.startswith('_get_'):
...
class TestAdminLayerFileStructureCompliance:
    ...
    def test_admin_file_structure_compliance(self):
        ...
        for admin_file in admin_files:
            # 检查文件命名规范
            assert admin_file.name.endswith("_admin.py"), (
                f"Admin文件 {admin_file.name} 必须以 '_admin.py' 结尾"
            )
            ...
            assert admin_classes, (
                f"Admin文件 {admin_file.name} 必须定义继承自 admin.ModelAdmin 的类"
            )
            ...
            if admin_actions:
                ...
                assert factory_functions, (
                    f"Admin文件 {admin_file.name} 包含Admin Action但缺少工厂函数"
                )
