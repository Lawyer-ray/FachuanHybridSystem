...
class TestCourtDocumentScraperAPIInterceptProperties:
    ...
    def test_property_6_download_function_invocation(self, api_response):
        ...
        for doc in documents:
            ...
            assert mock_page.expect_download.called, f"应该调用 Playwright 的 expect_download 方法下载 {url}"

            # 验证：应该调用 goto 导航到 URL
            assert mock_page.goto.called, f"应该调用 goto 方法导航到 {url}"

            # 验证：goto 的第一个参数应该是 URL
            call_args = mock_page.goto.call_args
            assert call_args[0][0] == url, f"goto 方法应该使用正确的 URL: {url}"
        ...
        with patch("httpx.Client") as mock_client_class:
            ...
            for doc in documents:
                ...
                if success and filepath:
                    ...
                    assert cleaned_wsmc in filename, f"文件名应该包含 c_wsmc: {cleaned_wsmc}，实际文件名: {filename}"

                    # 验证：文件扩展名应该基于 c_wjgs
                    c_wjgs = doc.get("c_wjgs", "pdf")
                    assert filename.endswith(f".{c_wjgs}"), f"文件扩展名应该是 .{c_wjgs}，实际文件名: {filename}"
    ...
    def test_property_22_download_delay_existence(self, doc_count, delay_range):
        ...
        assert (
            len(delay_calls) == expected_delay_calls
        ), f"应该有 {expected_delay_calls} 次延迟调用，实际: {len(delay_calls)}"

        # 验证：每次延迟调用的参数应该正确
        for delay_call in delay_calls:
            assert delay_call["min_sec"] == min_delay, f"延迟最小值应该为 {min_delay}，实际: {delay_call['min_sec']}"
            assert delay_call["max_sec"] == max_delay, f"延迟最大值应该为 {max_delay}，实际: {delay_call['max_sec']}"
        ...
        for i, delay_call in enumerate(delay_calls):
            ...
            assert (
                download_time_before <= delay_time <= download_time_after
    ...
    def test_property_8_error_isolation(self, total_docs, failure_indices):
        ...
        assert (
            len(processed_docs) == total_docs
        ), f"应该处理所有 {total_docs} 个文书，实际处理了 {len(processed_docs)} 个"

        # 验证 2：处理顺序应该正确（按索引顺序）
        assert processed_docs == list(
            range(total_docs)
        ...
        assert success_count == expected_success, f"应该有 {expected_success} 个成功下载，实际: {success_count}"
        assert failure_count == expected_failure, f"应该有 {expected_failure} 个失败下载，实际: {failure_count}"

        # 验证 4：所有文书都应该尝试保存到数据库
        assert len(saved_docs) == total_docs, f"应该尝试保存所有 {total_docs} 个文书到数据库，实际: {len(saved_docs)}"
        ...
        for saved_doc in saved_docs:
            ...
            assert saved_doc["success"] == expected_success, f"文书 {doc_index} 的保存状态应该为 {expected_success}"

            if not expected_success:
                assert saved_doc["error"] is not None, f"失败的文书 {doc_index} 应该有错误信息"

        # 验证 6：批量保存结果统计应该正确
        assert save_result["total"] == total_docs, f"批量保存总数应该为 {total_docs}"
        assert save_result["success"] == total_docs, f"所有文书都应该成功保存到数据库（即使下载失败）"
        assert save_result["failed"] == 0, f"数据库保存不应该失败（错误隔离）"
        ...
        with patch("apps.automation.services.scraper.scrapers.court_document.logger") as mock_logger:
            ...
            try:
                ...
                if not fallback_success:
                    assert "所有下载方式均失败" in str(e), "回退失败应该抛出包含错误链的异常"
                ...
                if "extra" in log_call["kwargs"]:
                    ...
                    if "fallback" in operation_type or "api_intercept_failed" in operation_type:
                        fallback_logs.append(
                            {"level": log_call["level"], "operation_type": operation_type, "extra": extra}
                        )
            ...
            for log in fallback_logs:
                ...
                if "fallback_reason" in extra:
                    ...
                    # 验证：回退原因应该包含原始错误信息
                    assert str(api_error) in fallback_reason or type(api_error).__name__ in fallback_reason, (
                        f"回退原因应该包含原始错误信息，" f"期望包含: {str(api_error)}，实际: {fallback_reason}"
                    )

                # 检查是否记录了错误类型
                if "error_type" in extra:
                    assert (
                        extra["error_type"] == type(api_error).__name__
            ...
            # 验证：应该记录 API 拦截失败的日志
            api_failed_logs = [log for log in fallback_logs if log["operation_type"] == "api_intercept_failed"]
            assert len(api_failed_logs) > 0, "应该记录 API 拦截失败的日志"

            # 验证：应该记录回退尝试的日志
            fallback_attempt_logs = [log for log in fallback_logs if log["operation_type"] == "fallback_attempt"]
            assert len(fallback_attempt_logs) > 0, "应该记录回退尝试的日志"

            # 如果回退成功，验证成功日志
            if fallback_success:
                fallback_success_logs = [log for log in fallback_logs if log["operation_type"] == "fallback_success"]
    ...
    def test_property_14_fallback_result_marking(self, api_error_message, use_fallback):
        ...
        with patch("apps.automation.services.scraper.scrapers.court_document.logger"):
            ...
            if use_fallback:
                ...
                assert result["method"] == "fallback", f"method 字段应该为 'fallback'，实际: {result.get('method')}"

                # 验证 2：应该包含 fallback_reason 字段
                assert "fallback_reason" in result, "结果应该包含 fallback_reason 字段"
                assert result["fallback_reason"] is not None, "fallback_reason 不应该为空"
                assert (
                    api_error_message in result["fallback_reason"]
                ), f"fallback_reason 应该包含原始错误信息: {api_error_message}"

                # 验证 3：应该包含 api_intercept_error 字段
                assert "api_intercept_error" in result, "结果应该包含 api_intercept_error 字段"
                assert isinstance(result["api_intercept_error"], dict), "api_intercept_error 应该是字典类型"
                ...
                assert api_error["type"] == "ValueError", f"错误类型应该为 ValueError，实际: {api_error['type']}"
                assert api_error_message in api_error["message"], f"错误消息应该包含: {api_error_message}"
                ...
                assert "fallback_reason" not in result, "API 拦截成功时不应该包含 fallback_reason 字段"

                # 验证 3：不应该包含 api_intercept_error 字段
                assert "api_intercept_error" not in result, "API 拦截成功时不应该包含 api_intercept_error 字段"

                # 验证 4：应该包含 db_save_result 字段（API 拦截方式特有）
                assert "db_save_result" in result, "API 拦截方式应该包含 db_save_result 字段"
    ...
    @given(
        ...
        fallback_error_message=st.text(
            min_size=5, max_size=50, alphabet=st.characters(whitelist_categories=("Lu", "Ll", "Nd", "Zs"))
        ),
    )
    def test_property_15_exception_chain_completeness(self, api_error_message, fallback_error_message):
        ...
        with patch("apps.automation.services.scraper.scrapers.court_document.logger"):
            ...
            assert api_error_message in error_message, f"异常消息应该包含 API 拦截失败信息: {api_error_message}"

            # 验证 3：异常消息应该包含回退失败的信息
            assert fallback_error_message in error_message, f"异常消息应该包含回退失败信息: {fallback_error_message}"

            # 验证 4：异常消息应该包含"所有下载方式均失败"
            assert "所有下载方式均失败" in error_message, "异常消息应该明确说明所有方式均失败"

            # 验证 5：异常 code 应该正确
            assert (
                exception.code == "DOWNLOAD_ALL_METHODS_FAILED"
            ...
            assert "api_intercept_error" in errors, "errors 应该包含 api_intercept_error"
            ...
            assert api_error["type"] == "ValueError", f"API 错误类型应该为 ValueError，实际: {api_error['type']}"
            assert api_error_message in api_error["message"], f"API 错误消息应该包含: {api_error_message}"

            # 验证 8：errors 应该包含 fallback_error
            assert "fallback_error" in errors, "errors 应该包含 fallback_error"
            ...
            assert (
                fallback_error["type"] == "RuntimeError"
            ), f"回退错误类型应该为 RuntimeError，实际: {fallback_error['type']}"
            assert (
                fallback_error_message in fallback_error["message"]
            ), f"回退错误消息应该包含: {fallback_error_message}"

            # 验证 9：异常消息应该包含错误类型信息
            assert "ValueError" in error_message, "异常消息应该包含 API 错误类型 ValueError"
            assert "RuntimeError" in error_message, "异常消息应该包含回退错误类型 RuntimeError"
            ...
            if api_error_message != fallback_error_message:
                assert api_error_pos < fallback_error_pos, "错误链应该按顺序排列：API 失败在前，回退失败在后"
            else:
                # 如果错误消息相同，至少应该出现两次
                assert (
                    error_message.count(api_error_message) >= 2
