...
class TestAPILayerCompliance:
    ...
    def test_property_1_api_factory_function_usage(self, api_module):
        ...
        **Validates: Requirements 1.1**
        ...
        try:
            ...
            if view_functions:
                # 如果有视图函数，必须有对应的工厂函数
                assert factory_functions, f"{api_module.__name__} 包含视图函数但缺少工厂函数 _get_xxx_service()"
                ...
                for _, view_name in view_functions:
                    ...
                    try:
                        ...
                        if "service" in view_source.lower():
                            assert factory_calls, f"{api_module.__name__}.{view_name} 必须使用工厂函数获取Service实例"
        ...
        try:
            ...
            for factory_body in factory_matches:
                ...
                if "ServiceLocator" in factory_body:
                    ...
                    assert servicelocator_import in source, f"{api_module.__name__} 工厂函数必须正确导入ServiceLocator"
            ...
            for pattern in prohibited_import_patterns:
                ...
                assert not violations, f"{api_module.__name__} 不应该直接导入其他模块: {violations}"
        ...
        try:
            ...
            for method, view_name, view_body in view_matches:
                ...
                for pattern in exception_handling_patterns:
                    ...
                    assert not violations, f"{api_module.__name__}.{view_name} 不应该进行异常处理: {violations}"
                ...
                for pattern in prohibited_db_patterns:
                    ...
                    if violations:
                        ...
                        for violation in violations:
                            # 检查是否在字符串或注释中
                            if not (
                                violation.startswith('"') or violation.startswith("'") or violation.startswith("#")
                            ):
                                # 排除字典的.get()方法调用
                                if violation == ".get(" and "metrics.get(" in view_body:
                                    continue
                                # 排除其他合法的.get()调用（如字典、配置等）
                                if violation == ".get(" and any(
                                    prefix in view_body
                                    for prefix in [
                                        "config.get(",
                                        "data.get(",
    ...
    def test_api_layer_comprehensive_compliance_check(self):
        ...
        compliance_report = {"total_modules": len(api_modules), "compliant_modules": 0, "violations": []}
                ...
                for pattern in db_operation_patterns:
                    ...
                    if matches:
                        ...
                        for match in matches:
                            # 排除字典的.get()等方法
                            if ".get(" in match and any(
                                prefix in source
                                for prefix in ["metrics.get(", "config.get(", "data.get(", "result.get("]
                            ):
                                continue
...
def compliant_api_view_source(draw):
    ...
    view_name = draw(
        st.sampled_from(
            ["create_item", "get_item", "update_item", "delete_item", "list_items", "process_data", "execute_task"]
        )
    )
...
def non_compliant_api_view_source(draw):
    ...
    http_method = draw(st.sampled_from(["get", "post", "put", "delete", "patch"]))
    view_name = draw(st.sampled_from(["create_item", "get_item", "update_item", "delete_item"]))

    violation_type = draw(
        st.sampled_from(["direct_service_instantiation", "try_except_usage", "db_operation", "transaction_decorator"])
    )
...
class TestAPILayerFileStructureCompliance:
    ...
    def test_api_file_structure_compliance(self):
        ...
        for api_file in api_files:
            # 检查文件命名规范
            assert api_file.name.endswith("_api.py"), f"API文件 {api_file.name} 必须以 '_api.py' 结尾"

            # 检查文件内容结构
            content = api_file.read_text(encoding="utf-8")

            # 必须包含Router定义
            assert "router = Router(" in content, f"API文件 {api_file.name} 必须定义 router"
