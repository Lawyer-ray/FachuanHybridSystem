    ...
    async def test_task_continuation_after_login(self, site_name: str, login_delay: float):
        ...
        **Validates: Requirements 1.3**
        ...
        with patch('apps.automation.services.token.auto_token_acquisition_service.cache_manager') as mock_cache, \
             ...
             patch('apps.automation.services.token.auto_token_acquisition_service.performance_monitor') as mock_perf, \
             patch('apps.automation.services.token.auto_token_acquisition_service.history_recorder') as mock_history, \
             patch('apps.automation.utils.logging.AutomationLogger') as mock_logger:
        ...
        async def mock_login_behavior(*args, **kwargs):
            ...
            if should_fail:
                ...
                raise LoginFailedError(
                    message="网络连接失败",
                    errors={"error_type": "network_error", "attempt": call_count}
                )
            else:
                # 模拟成功
    ...
    async def test_structured_logging_consistency(self, site_name: str, success: bool):
        ...
        **Validates: Requirements 3.1, 3.2, 3.3, 3.4, 3.5**
        ...
        if success:
            ...
            mock_login_service.login_and_get_token.side_effect = LoginFailedError(
                message="登录失败",
                errors={"error_type": "test_failure"}
            )
    ...
    **验证: Requirements 5.5**
    ...
    async def test_concurrency_handling_consistency(self, site_name: str, concurrent_count: int):
        ...
        **Validates: Requirements 5.5**
        ...
        assert login_call_count <= concurrent_count, f"登录调用次数不应超过并发数量: {login_call_count} <= {concurrent_count}"
