...
def check_file_has_lazy_loading(file_path: Path) -> bool:
    ...
    try:
        ...
        has_dependencies = bool(re.search(r"self\._\w+_service", content) or re.search(r"self\._\w+_client", content))
...
def check_file_uses_service_locator_correctly(file_path: Path) -> tuple[bool, str]:
    ...
    try:
        ...
        if has_service_locator_import:
            ...
            has_correct_import = any(pattern in content for pattern in correct_import_patterns)
            ...
            if isinstance(node, ast.ClassDef):
                ...
                for item in node.body:
                    if isinstance(item, ast.FunctionDef) and item.name == "__init__":
                        ...
                        # 获取非 self 参数
                        non_self_args = [arg for arg in args.args if arg.arg != "self"]
                        ...
                        if required_count > 0:
                            violations.append(
                                (class_name, f"构造函数有 {required_count} 个必需参数，不支持无参数实例化")
                            )
    ...
    **Validates: Requirements 5.1, 5.2, 5.4, 5.5, 5.6**
    ...
    def test_property_2_no_staticmethod(self):
        ...
        **Validates: Requirements 5.1**
        ...
        for file_path in self.get_all_decoupled_files():
            ...
            if violations:
                all_violations.append({"file": file_path.name, "violations": violations})
    ...
    def test_property_2_architecture_compliance_property_based(self, relative_path: str):
        ...
        **Validates: Requirements 5.1, 5.2, 5.4, 5.5, 5.6**
        ...
        assert len(staticmethod_violations) == 0, f"{file_path.name} 使用了 @staticmethod"

        # 检查跨模块 Model 导入
        model_import_violations = check_file_for_cross_module_model_import(file_path)
        assert len(model_import_violations) == 0, f"{file_path.name} 直接导入了跨模块 Model"

        # 检查延迟加载
        assert check_file_has_lazy_loading(file_path), f"{file_path.name} 有依赖但未使用 @property 延迟加载"
    ...
    **Validates: Requirements 3.4, 5.3**
        ...
        try:
            ...
            for i, line in enumerate(lines, 1):
                ...
                if in_type_checking and stripped and not stripped.startswith("#"):
                    ...
                    if (
                        current_indent <= type_checking_indent
                ...
                for pattern, module in direct_import_patterns:
                    if re.search(pattern, line):
                        violations.append(f"行 {i}: 直接导入 {module} 模块服务: {stripped}")
            ...
            if "ServiceLocator." in content:
                ...
                has_correct_usage = any(re.search(pattern, content) for pattern in correct_patterns)
    ...
    def test_property_3_service_locator_usage(self):
        ...
        **Validates: Requirements 5.3**
        ...
        for file_path in self.get_all_decoupled_files():
            ...
            if not is_correct:
                violations.append({"file": file_path.name, "error": error})
    ...
    def test_property_3_cross_module_call_property_based(self, relative_path: str):
        ...
        **Validates: Requirements 3.4, 5.3**
        ...
        assert is_compliant, f"{file_path.name} 跨模块调用不合规:\n" + "\n".join(f"  {v}" for v in violations)
    ...
    def test_comprehensive_decoupling_compliance(self):  # noqa: C901
        ...
        **Validates: Requirements 1.2, 2.2, 3.4, 4.3, 5.1, 5.2, 5.3, 5.4, 5.5, 5.6**
        ...
        for relative_path, file_type in DECOUPLED_FILES.items():
            ...
            total_lines = count_total_lines(file_path)
            if total_lines > limit:
                file_violations.append(f"行数超限: {total_lines} > {limit}")

            # 检查 2: @staticmethod
            staticmethod_violations = check_file_for_staticmethod(file_path)
            if staticmethod_violations:
                file_violations.append(f"使用了 @staticmethod ({len(staticmethod_violations)} 处)")

            # 检查 3: 跨模块 Model 导入
            model_import_violations = check_file_for_cross_module_model_import(file_path)
            if model_import_violations:
                file_violations.append(f"直接导入跨模块 Model ({len(model_import_violations)} 处)")
            ...
            if di_violations:
                file_violations.append(f"不支持依赖注入 ({len(di_violations)} 个类)")
