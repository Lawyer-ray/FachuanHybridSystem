...
class TestCaptchaRecognitionProperties:
        ...
        try:
            ...
            if result.success:
                assert result.text is None or isinstance(result.text, str), \
                    "成功时 text 应该是 None 或字符串"
            else:
                assert result.error is None or isinstance(result.error, str), \
                    "失败时 error 应该是 None 或字符串"
                    
        except Exception as e:
            # 不应该抛出任何未捕获的异常
            pytest.fail(
                f"API 不应该抛出未捕获的异常，但抛出了 {type(e).__name__}: {e}"
            )
    ...
    def test_property_2_invalid_base64_returns_error(self, invalid_text):
        ...
        **Validates: Requirements 1.2**
        ...
        try:
            ...
            # 验证返回类型
            assert isinstance(result, CaptchaRecognizeOut), \
                f"返回值应该是 CaptchaRecognizeOut 类型"
            
            # 对于无效输入，应该返回失败
            assert result.success is False, \
                f"无效输入应该返回 success=False，但得到 {result.success}"
    ...
    def test_property_3_success_response_contains_required_fields(self, service):
        ...
        **Validates: Requirements 1.3, 3.2, 3.4**
        ...
        if result.success:
            assert result.text is None or isinstance(result.text, str), \
                "成功时 text 应该是字符串或 None"
            assert result.processing_time is not None, \
                "成功时 processing_time 不应该为 None"
            assert result.processing_time >= 0, \
                "processing_time 应该是非负数"
        ...
        **Validates: Requirements 1.5**
        ...
        try:
            ...
            if not result.success and result.error:
                ...
                assert "不支持" not in result.error.lower(), \
                    f"支持的格式 {image_format} 不应该被拒绝: {result.error}"
                    
        except Exception as e:
            pytest.fail(
                f"处理 {image_format} 格式时不应该抛出异常: {type(e).__name__}: {e}"
            )
        ...
        **Validates: Requirements 3.1**
        ...
        try:
            ...
            assert isinstance(result.success, bool), \
                f"success 必须是布尔值，但得到 {type(result.success)}"
            ...
            pytest.fail(
                f"API 不应该抛出未捕获的异常: {type(e).__name__}: {e}"
            )
    ...
    def test_property_7_file_size_limit_enforced(self, size_mb):
        ...
        **Validates: Requirements 6.2, 6.3**
        ...
        if size_mb > 5:
            # 超过 5MB 应该被拒绝
            assert result.success is False, \
                f"超过 5MB 的图片应该被拒绝，但 {size_mb}MB 的图片被接受了"
