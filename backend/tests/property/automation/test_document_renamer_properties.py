    ...
    if case_name_type == "normal":
        ...
        return draw(
            st.text(
                ...
                max_size=30,
                alphabet=st.characters(
                    whitelist_categories=("Lu", "Ll", "Nd", "Lo"), blacklist_characters='<>:"|?*\\/.'
                ),
            )
        )
...
class TestDocumentRenamerProperties:
    ...
    def test_property_7_filename_format_correctness(self, title, case_name, received_date): # noqa: C901
        ...
        **Validates: Requirements 4.3**
        ...
        # 验证：特殊字符处理
        # 验证实际的标题和案件名称不包含非法字符
        assert not re.search(illegal_chars, actual_title), f"标题部分不应包含非法字符: {actual_title}"
        assert not re.search(illegal_chars, actual_case_name), f"案件名称部分不应包含非法字符: {actual_case_name}"
        ...
        assert filename_without_ext.upper() not in windows_reserved_names, f"文件名不应使用Windows保留名称: {filename}"
        ...
        for component_name, component_value in components.items():
            if component_name == "extension":
                assert filename.endswith(component_value), f"文件名应包含{component_name}: {component_value}"
            else:
                assert str(component_value) in filename, f"文件名应包含{component_name}: {component_value}"
        ...
        assert re.match(expected_pattern, filename), f"文件名应符合预期格式模式: {filename}"
        ...
        if re.search(chinese_char_pattern, title) or re.search(chinese_char_pattern, case_name):
            ...
            assert re.search(chinese_char_pattern, filename), f"包含中文输入时，文件名也应包含中文字符: {filename}"
        ...
        assert filename == reconstructed, f"文件名应该可以按格式重新构建: {filename} != {reconstructed}"
