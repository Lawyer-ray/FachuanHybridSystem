...
class TestServiceLayerCompliance:
    ...
    def test_property_7_service_lazy_loading_usage(self, service_class):
        ...
        **Validates: Requirements 2.2**
        ...
        # 查找@property装饰的方法
        property_methods = [
            name for name, member in class_members
        ...
        **Validates: Requirements 2.4**
        ...
        try:
            ...
            for exception_stmt in found_exceptions:
                ...
                # 如果是业务异常，必须包含三个参数
                if any(exc_type in exception_stmt for exc_type in [
                    'ValidationException', 'NotFoundError', 'ConflictError',
                    'BusinessException', 'AutoTokenAcquisitionError'
                ]):
        ...
        **Validates: Requirements 2.5**
        ...
        try:
            ...
            for pattern in prohibited_import_patterns:
                matches = re.findall(pattern, source)
                assert not matches, (
                    f"{service_class.__name__} 不应该直接导入其他模块: {matches}"
                )
    ...
    def test_service_layer_comprehensive_compliance_check(self):
        ...
        compliance_report = {
            "total_services": len(service_classes),
        ...
        compliance_rate = (
            ...
            if compliance_report["total_services"] > 0 else 0
        )
...
def service_method_source(draw):
    """生成Service方法源代码的策略"""
    method_names = draw(st.sampled_from([
        'create_client', 'update_client', 'delete_client',
        'create_case', 'update_case', 'execute_task',
        'get_data', 'list_items', 'process_request'
    ]))
