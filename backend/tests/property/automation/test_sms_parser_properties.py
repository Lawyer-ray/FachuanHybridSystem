...
def sms_content_strategy(draw):
    ...
    safe_text = st.text(
        ...
        alphabet=st.characters(
            blacklist_categories=('Cs',),  # 排除 surrogate 字符
...
def case_number_strategy(draw):
    ...
    # 法院代码（中文）
    court_codes = ["粤", "京", "沪", "津", "渝", "冀", "晋", "蒙", "辽", "吉", "黑", "苏", "浙", "皖", "闽", "赣", "鲁", "豫", "鄂", "湘", "桂", "琼", "川", "贵", "云", "藏", "陕", "甘", "青", "宁", "新"]
        ...
        if expected_valid_links:
            ...
            for expected_link in expected_valid_links:
                ...
                for extracted_link in extracted_links:
                    if (self._extract_param_value(expected_link, 'qdbh') == self._extract_param_value(extracted_link, 'qdbh') and
                        self._extract_param_value(expected_link, 'sdbh') == self._extract_param_value(extracted_link, 'sdbh') and
                        self._extract_param_value(expected_link, 'sdsin') == self._extract_param_value(extracted_link, 'sdsin')):
                        found = True
                        break
    ...
    def test_property_4_case_number_extraction_and_normalization(self, sms_data):
        ...
        **Validates: Requirements 1.4, 3.1**
        ...
        if original_case_numbers:
            # 至少应该提取到一些案号
            assert len(extracted_case_numbers) > 0, f"应该提取到案号，但返回空列表。原始案号: {original_case_numbers}, 内容: {content[:100]}..."
        ...
        # 验证：去重功能正常工作
        # 如果有重复的规范化案号，应该被去重
        assert len(extracted_case_numbers) == len(set(extracted_case_numbers)), f"案号列表应该去重: {extracted_case_numbers}"
    ...
    def test_property_3_sms_type_determination_consistency(self, sms_data):
        ...
        **Validates: Requirements 1.3**
        ...
        if result.has_valid_download_link:
            ...
            assert len(result.download_links) > 0, \
                f"has_valid_download_link=True 时 download_links 不应为空"
            
        else:
            # 如果没有有效下载链接，类型应该是信息通知或立案通知
            assert result.sms_type in [CourtSMSType.INFO_NOTIFICATION, CourtSMSType.FILING_NOTIFICATION], \
                f"无下载链接时类型应为 INFO_NOTIFICATION 或 FILING_NOTIFICATION，实际为: {result.sms_type}"
            
            # 验证：download_links 为空
            assert len(result.download_links) == 0, \
                f"has_valid_download_link=False 时 download_links 应为空"
        
        # 验证：has_valid_download_link 与 download_links 的一致性
        if len(result.download_links) > 0:
            assert result.has_valid_download_link == True, \
                f"有下载链接时 has_valid_download_link 应为 True"
        else:
            assert result.has_valid_download_link == False, \
                f"无下载链接时 has_valid_download_link 应为 False"
        
        # 验证：立案通知的特殊逻辑
        if result.sms_type == CourtSMSType.FILING_NOTIFICATION:
            # 立案通知类型应该在内容中包含"立案"关键词
            assert "立案" in content, \
                f"类型为 FILING_NOTIFICATION 时内容应包含'立案'关键词"
            
            # 立案通知不应该有下载链接
            assert not result.has_valid_download_link, \
                f"立案通知不应该有有效下载链接"
            assert len(result.download_links) == 0, \
                f"立案通知的下载链接列表应为空"
        ...
        if result.sms_type == CourtSMSType.DOCUMENT_DELIVERY:
            # 文书送达必须有下载链接
            assert result.has_valid_download_link, \
                f"文书送达必须有有效下载链接"
            assert len(result.download_links) > 0, \
                f"文书送达的下载链接列表不应为空"
        ...
        assert result2.sms_type == result.sms_type, \
            f"重复解析应得到相同类型: {result.sms_type} != {result2.sms_type}"
        ...
        # 验证：类型判定的逻辑完整性
        # 确保所有可能的情况都被覆盖
        all_possible_types = {CourtSMSType.DOCUMENT_DELIVERY, CourtSMSType.INFO_NOTIFICATION, CourtSMSType.FILING_NOTIFICATION}
