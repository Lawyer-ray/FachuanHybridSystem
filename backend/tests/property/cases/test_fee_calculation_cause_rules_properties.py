...
class TestPersonalityRightsFeeWithRangeProperties:
    ...
    def test_property_4_personality_rights_tiered_fee_calculation(self, amount: Decimal):
        ...
        if amount <= 0:
            # 无金额或金额为0：返回基础费用范围
            assert result["fee_min"] == base_min, (
                f"金额 {amount} 的最小费用应为 {base_min}，但得到 {result['fee_min']}"
            )
            assert result["fee_max"] == base_max, (
                f"金额 {amount} 的最大费用应为 {base_max}，但得到 {result['fee_max']}"
            )
    ...
    def test_personality_rights_boundary_50000(self, fee_calculator_service):
        ...
        """
        result = fee_calculator_service.calculate_personality_rights_fee_with_range(
            Decimal("50000")
        )
...
class TestFixedFeeProperties:
    ...
    def test_property_6_fixed_fee_correctness(self, case_type: str):
        ...
        assert FIXED_FEES[case_type] == expected_fees[case_type], (
            f"案件类型 {case_type} 的固定费用应为 {expected_fees[case_type]}，"
            f"但配置为 {FIXED_FEES[case_type]}"
        )
...
class TestDisplayControlProperties:
    ...
    def test_property_7_display_control_correctness(self, case_type: str):
        ...
        if case_type == SpecialCaseType.PAYMENT_ORDER:
            ...
            assert config["show_acceptance_fee"] is True, (
                "支付令案件 show_acceptance_fee 应为 True"
            )
            assert config["show_half_fee"] is True, (
                "支付令案件 show_half_fee 应为 True"
            )
            assert config["show_payment_order_fee"] is True, (
                "支付令案件 show_payment_order_fee 应为 True"
            )
        else:
            # 撤销仲裁/公示催告/劳动争议案件：所有显示标志应为 False
            config = DISPLAY_CONFIG[case_type]
            assert config["show_acceptance_fee"] is False, (
                f"{case_type} 案件 show_acceptance_fee 应为 False"
            )
            assert config["show_half_fee"] is False, (
                f"{case_type} 案件 show_half_fee 应为 False"
            )
            assert config["show_payment_order_fee"] is False, (
                f"{case_type} 案件 show_payment_order_fee 应为 False"
            )
    ...
    def test_fixed_fee_cases_display_config(self):
        ...
        for case_type in fixed_fee_types:
            config = DISPLAY_CONFIG[case_type]
            assert config["show_acceptance_fee"] is False, (
                f"{case_type} 应隐藏案件受理费"
            )
            assert config["show_half_fee"] is False, (
                f"{case_type} 应隐藏减半后受理费"
            )
            assert config["show_payment_order_fee"] is False, (
                f"{case_type} 应隐藏支付令申请费"
            )
