...
class TestFolderGenerationProperties:
    ...
    @given(
        ...
        contract_name=st.text(
            min_size=1, max_size=50, alphabet=st.characters(whitelist_categories=("Lu", "Ll", "Nd", "Zs"))
        ),
    )
    def test_root_folder_name_format_correctness(self, case_type, contract_name):
        ...
        **Validates: Requirements 3.1, 3.2, 3.3, 3.4, 3.5**
        ...
        # Mock 类型映射
        type_mapping = {"civil": "民商事", "criminal": "刑事", "administrative": "行政"}
        ...
        with patch("apps.documents.services.generation.folder_generation_service.date") as mock_date:
            ...
            with patch("apps.documents.services.generation.folder_generation_service.CaseType") as mock_case_type:
                mock_case_type.choices = [("civil", "民商事"), ("criminal", "刑事"), ("administrative", "行政")]
    ...
    @given(
        root_name=st.text(
            min_size=1, max_size=50, alphabet=st.characters(whitelist_categories=("Lu", "Ll", "Nd", "Zs", "Pc"))
        ),
    ...
    def test_zip_file_structure_correctness(self, root_name, num_documents):
        ...
        **Validates: Requirements 5.1, 5.2**
        ...
        # 创建文件夹结构
        folder_structure = {"name": root_name, "children": [{"name": "subfolder1"}, {"name": "subfolder2"}]}

        # 创建文档列表
        documents = []
        for i in range(num_documents):
            documents.append(("subfolder1" if i % 2 == 0 else "", f"content_{i}".encode(), f"file_{i}.txt"))
    ...
    @given(structure_name=st.text(min_size=1, max_size=30), new_root_name=st.text(min_size=1, max_size=30))
    def test_folder_structure_root_replacement(self, structure_name, new_root_name):
        ...
        # Mock 模板
        mock_template = Mock()
        mock_template.structure = {"name": structure_name, "children": [{"name": "child1"}, {"name": "child2"}]}
