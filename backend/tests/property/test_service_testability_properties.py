...
from tests.strategies.common_strategies import chinese_text, phone_number, decimal_amount
...
def case_create_data(draw):
    ...
    return {
        ...
        "current_stage": draw(st.one_of(st.none(), st.sampled_from([
            "investigation", "prosecution", "first_trial", "second_trial"
        ]))),
    }
...
def contract_create_data(draw):
    ...
    data = {
        ...
        "fee_mode": fee_mode,
        "representation_stages": draw(st.lists(
...
def lawfirm_create_data(draw):
    ...
    return {
        ...
        "social_credit_code": draw(st.one_of(st.none(), st.text(
...
def test_contract_service_list_without_http_request(page, page_size):
    ...
    # 调用 Service 方法（不需要 user 参数）
    results = service.list_contracts(
        case_type=None,
        status=None,
        is_archived=None
    )
