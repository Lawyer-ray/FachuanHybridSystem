...
class TestExceptionStructureProperties:
    ...
    def test_exception_has_required_attributes(
        self,
        exception_class,
        message,
        code,
        ...
        if code is not None:
            ...
            # 如果没有提供 code，应该使用默认值
            assert exc.code is not None and len(exc.code) > 0, \
                f"{exception_class.__name__}.code 应该有默认值"
    ...
    def test_exception_errors_is_always_dict(self, exception_class, message):
        ...
        # 验证 errors 是空字典
        assert exc.errors == {}, \
            f"{exception_class.__name__}.errors 应该是空字典，实际是 {exc.errors}"
    ...
    def test_exception_to_dict_structure(self, exception_class, message, errors):
        ...
        assert isinstance(result, dict), \
            f"{exception_class.__name__}.to_dict() 应该返回字典类型"

        # 验证包含必需的字段
        assert 'error' in result, \
            f"{exception_class.__name__}.to_dict() 应该包含 'error' 字段"
        assert 'code' in result, \
            f"{exception_class.__name__}.to_dict() 应该包含 'code' 字段"
        assert 'errors' in result, \
            f"{exception_class.__name__}.to_dict() 应该包含 'errors' 字段"

        # 验证字段类型
        assert isinstance(result['error'], str), \
            f"{exception_class.__name__}.to_dict()['error'] 应该是字符串类型"
        assert isinstance(result['code'], str), \
            f"{exception_class.__name__}.to_dict()['code'] 应该是字符串类型"
        assert isinstance(result['errors'], dict), \
            f"{exception_class.__name__}.to_dict()['errors'] 应该是字典类型"

        # 验证字段值
        assert result['error'] == message, \
            f"{exception_class.__name__}.to_dict()['error'] 值不正确"
        assert result['errors'] == errors, \
            f"{exception_class.__name__}.to_dict()['errors'] 值不正确"
    ...
    def test_exception_code_is_string(self, exception_class, message, code):
        ...
        assert isinstance(exc.code, str), \
            f"{exception_class.__name__}.code 应该是字符串类型，实际是 {type(exc.code)}"

        # 验证 code 非空
        assert len(exc.code) > 0, \
            f"{exception_class.__name__}.code 不应该是空字符串"

        # 验证 code 值正确
        assert exc.code == code, \
            f"{exception_class.__name__}.code 值不正确"
    ...
    def test_exception_has_default_code(self, exception_class, message):
        ...
        assert hasattr(exc, 'code'), \
            f"{exception_class.__name__} 应该有 code 属性"
        assert isinstance(exc.code, str), \
            f"{exception_class.__name__}.code 应该是字符串类型"
        assert len(exc.code) > 0, \
            f"{exception_class.__name__}.code 不应该是空字符串"
    ...
    def test_exception_errors_preserves_structure(self, exception_class, message, errors):
        ...
        assert exc.errors == errors, \
            f"{exception_class.__name__}.errors 结构应该保持不变"

        # 验证 to_dict() 中的 errors 也保持不变
        result = exc.to_dict()
        assert result['errors'] == errors, \
            f"{exception_class.__name__}.to_dict()['errors'] 结构应该保持不变"
    ...
    def test_exception_inheritance(self, exception_class, message, code, errors):
        ...
        assert isinstance(exc, BusinessException), \
            f"{exception_class.__name__} 应该继承自 BusinessException"
        assert isinstance(exc, Exception), \
            f"{exception_class.__name__} 应该继承自 Exception"
