...
class TestGenerationConfigManagement:
    ...
    def test_create_generation_config_success(
        self, generation_service, document_template, db
    ):
        """测试成功创建生成配置"""
    ...
    def test_create_generation_config_with_condition(
        self, generation_service, document_template, db
    ):
        """测试创建带条件的生成配置"""
    ...
    def test_create_generation_config_template_not_found(
        self, generation_service, db
    ):
        """测试创建配置时模板不存在"""
    ...
    def test_create_generation_config_template_inactive(
        self, generation_service, inactive_template, db
    ):
        """测试创建配置时模板已禁用"""
    ...
    def test_create_generation_config_empty_folder_path(
        self, generation_service, document_template, db
    ):
        """测试创建配置时文件夹路径为空"""
        with pytest.raises(ValidationException) as exc_info:
    ...
    def test_update_generation_config(
        self, generation_service, document_template, db
    ):
        """测试更新生成配置"""
        config = generation_service.create_generation_config(
    ...
    def test_get_configs_for_case_priority_order(
        self, generation_service, document_template, db
    ):
        """测试获取配置按优先级排序"""
        # 创建多个配置，优先级不同
    ...
    def test_get_configs_for_case_excludes_inactive(
        self, generation_service, document_template, db
    ):
        """测试获取配置默认排除禁用的"""
        generation_service.create_generation_config(
    ...
    def test_validate_config_references_valid(
        self, generation_service, document_template, db
    ):
        """测试验证有效的配置引用"""
        config = generation_service.create_generation_config(
    ...
    def test_validate_config_references_template_deleted(
        self, generation_service, document_template, db
    ):
        """测试验证引用已删除模板的配置"""
        config = generation_service.create_generation_config(
    ...
    def test_delete_generation_config_soft_delete(
        self, generation_service, document_template, db
    ):
        """测试软删除生成配置"""
        config = generation_service.create_generation_config(
...
class TestTaskManagement:
    ...
    def test_update_task_status_to_processing(
        self, generation_service, db
    ):
        """测试更新任务状态为处理中"""
        task = generation_service.create_task()
        ...
        updated = generation_service.update_task_status(
            task.id, GenerationTaskStatus.PROCESSING
        )
    ...
    def test_update_task_status_to_completed(
        ...
        updated = generation_service.update_task_status(
            task.id, GenerationTaskStatus.COMPLETED
        )
        
        assert updated.status == GenerationTaskStatus.COMPLETED
        assert updated.completed_at is not None
    
    def test_update_task_status_to_failed_with_error(
        self, generation_service, db
    ):
        """测试更新任务状态为失败并记录错误"""
        task = generation_service.create_task()
    ...
    def test_generate_requires_case(
        self, generation_service, db
    ):
