...
from apps.contracts.models import Contract, ContractAssignment, ContractParty, FeeMode, PartyRole
...
class ContextBuilderTest(TestCase):
    ...
    def setUp(self):
        ...
        # 创建律所
        self.law_firm = LawFirm.objects.create(name="测试律所", address="测试地址", phone="12345678901")
        ...
        ContractParty.objects.create(contract=self.contract, client=self.client2, role=PartyRole.BENEFICIARY)

        ContractParty.objects.create(contract=self.contract, client=self.client3, role=PartyRole.OPPOSING)

        # 创建律师指派
        ContractAssignment.objects.create(contract=self.contract, lawyer=self.lawyer1, is_primary=True, order=1)

        ContractAssignment.objects.create(contract=self.contract, lawyer=self.lawyer2, is_primary=False, order=2)
    ...
    def test_build_contract_context_no_parties(self):
        ...
        contract_no_parties = Contract.objects.create(
            name="无当事人合同", case_type=CaseType.CIVIL, status=CaseStatus.ACTIVE, specified_date=date(2024, 1, 15)
        )
    ...
    def test_build_contract_context_no_lawyers(self):
        ...
        contract_no_lawyers = Contract.objects.create(
            name="无律师合同", case_type=CaseType.CIVIL, status=CaseStatus.ACTIVE, specified_date=date(2024, 1, 15)
        )
