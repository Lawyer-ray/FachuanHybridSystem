...
from apps.documents.models import FolderTemplate, DocumentCaseType, DocumentCaseStage
...
class TestFolderTemplateServiceValidation(TestCase):
    ...
    def test_validate_valid_structure(self):
        ...
        structure = {
            "children": [
                {"id": "1", "name": "诉讼材料", "children": [
                    {"id": "1-1", "name": "起诉状", "children": []}
                ]},
...
class TestFolderTemplateServiceQuery(TestCase):
    ...
    def test_get_template_for_case_returns_latest(self):
        ...
        # 应该返回最新创建的模板2
        found = self.service.get_template_for_case(
            DocumentCaseType.CIVIL,
            DocumentCaseStage.FIRST_TRIAL
        )
    ...
    def test_get_template_for_case_no_match(self):
        """测试无匹配模板返回 None"""
        found = self.service.get_template_for_case(
            DocumentCaseType.CRIMINAL,
            DocumentCaseStage.SECOND_TRIAL
        )
