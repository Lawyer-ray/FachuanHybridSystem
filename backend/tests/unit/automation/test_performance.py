...
class TestPerformance:
    ...
    async def test_concurrent_query_performance_10_companies(self, service, mock_credential):
        ...
        # 创建 10 个保险公司
        mock_companies = [InsuranceCompany(c_id=str(i), c_code=f"CODE{i}", c_name=f"保险公司{i}") for i in range(1, 11)]
    ...
    async def test_concurrent_vs_serial_performance(self, service, mock_credential):
        ...
        # 创建 5 个保险公司（减少测试时间）
        mock_companies = [InsuranceCompany(c_id=str(i), c_code=f"CODE{i}", c_name=f"保险公司{i}") for i in range(1, 6)]
