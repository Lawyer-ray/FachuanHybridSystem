    ...
    **Feature: property-preservation-insurance, Property 20: Token 复用**
    ...
    async def test_token_reuse_across_multiple_tasks(
        ...
        **Feature: property-preservation-insurance, Property 20: Token 复用**
        ...
        # 创建随机数量的保险公司
        mock_companies = [
            InsuranceCompany(c_id=str(i), c_code=f"CODE{i}", c_name=f"保险公司{i}") for i in range(1, company_count + 1)
        ]
        ...
        for quote in quotes:
            ...
            assert quote.status == QuoteStatus.SUCCESS, f"任务 {quote.id} 应该成功完成，实际状态: {quote.status}"
    ...
    async def test_token_consistency_across_operations(
        ...
        **Feature: property-preservation-insurance, Property 20: Token 复用**
        ...
        assert len(unique_tokens) == 1, f"应该只有 1 个唯一的 Token，实际有 {len(unique_tokens)} 个: {unique_tokens}"
