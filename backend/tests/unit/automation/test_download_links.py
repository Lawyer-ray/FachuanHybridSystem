...
if os.getenv("RUN_EXTERNAL_DOWNLOAD_TESTS") != "1":
    pytest.skip("external download tests disabled (set RUN_EXTERNAL_DOWNLOAD_TESTS=1 to enable)", allow_module_level=True)
...
def create_download_dir():
    """创建下载目录"""
    download_dir = Path(settings.MEDIA_ROOT) / "automation" / "test_downloads" / datetime.now().strftime("%Y%m%d_%H%M%S")
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if not confirm_button:
                try:
                    confirm_button = page.locator("button:has-text('确认'), button:has-text('确定'), button:has-text('预览')")
                    if confirm_button.count() > 0 and confirm_button.first.is_visible():
                        print("  ✓ 通过按钮选择器找到确认按钮")
                    else:
                        confirm_button = None
            ...
            if frame:
                ...
                if not download_button:
                    try:
                        ...
                        download_button = frame.locator("#download, #downloadButton, button[title*='下载'], button[title*='Download']")
...
def main():
    ...
    parser.add_argument("--type", choices=["gdems", "zxfw", "zxfw_multi", "all"], 
