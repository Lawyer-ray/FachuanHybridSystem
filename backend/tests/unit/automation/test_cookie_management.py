    ...
    **Feature: automation-decoupling, Property 9: Cookie loading before login**
    ...
    def test_property_cookie_loading_before_login(self):
        ...
        **Validates: Requirements 4.2**
        ...
        # 预先保存一些 Cookie
        test_cookies = [
            {"name": "session", "value": "valid_session", "domain": ".court.gov.cn"}
        ]
        ...
        assert mock_cookie_service.load_called, \
            "应该调用 load 尝试加载 Cookie"
        
        # 2. Cookie 被添加到浏览器上下文
        assert mock_context.add_cookies_called, \
            "应该将 Cookie 添加到浏览器上下文"
        
        # 3. 登录成功
        assert result["success"], "登录应该成功"
        
        # 4. 使用了 Cookie 登录
        assert result.get("used_cookie") is True, \
            "应该标记为使用 Cookie 登录"
    ...
    def test_cookie_loading_called_first(self):
        ...
        assert cookie_path is not None
        mock_cookie_service.storage[cookie_path] = [
        ...
        # 验证 load 被调用
        assert mock_cookie_service.load_call_count >= 1, \
            "应该至少调用一次 load"
    ...
    **Feature: automation-decoupling, Property 10: Cookie-based login skip**
    ...
    def test_property_valid_cookies_skip_login(self):
        ...
        **Validates: Requirements 4.3**
        ...
        assert mock_cookie_service.save_call_count == 0, \
            "使用 Cookie 登录时不应该保存新 Cookie"
    ...
    **Feature: automation-decoupling, Property 11: Cookie saving after login**
    ...
    def test_property_save_cookies_after_successful_login(self):
        ...
        **Validates: Requirements 4.4**
        ...
        # 设置一些 Cookie
        mock_context.cookies_list = [
            {"name": "session", "value": "new_session", "domain": ".court.gov.cn"}
        ]
        ...
        # 验证：
        # 1. save 被调用
        assert mock_cookie_service.save_called, \
            "应该调用 save"
        
        # 2. 保存到了正确路径
        expected_path = service._get_cookie_path("test_user")
        assert mock_cookie_service.last_saved_path == expected_path, \
            "应该保存到正确的路径"
    ...
    **Feature: automation-decoupling, Property 12: Fresh login on expired cookies**
    ...
    def test_property_fresh_login_when_cookies_expired(self):
        ...
        **Validates: Requirements 4.5**
        ...
        # 保存"过期"的 Cookie
        expired_cookies = [
            {"name": "session", "value": "expired_token", "domain": ".court.gov.cn"}
        ]
        ...
        # （在真实场景中会执行完整登录流程）
        assert mock_page.goto_call_count >= 1, \
            "Cookie 无效时应该尝试执行登录流程"
