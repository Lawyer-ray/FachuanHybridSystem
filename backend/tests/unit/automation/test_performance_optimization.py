...
from apps.automation.services.token.concurrency_optimizer import (
    ConcurrencyOptimizer, ConcurrencyConfig
)
from apps.automation.services.token.history_recorder import TokenHistoryRecorder
from apps.core.interfaces import TokenAcquisitionResult, LoginAttemptResult
...
class TestPerformanceMonitor(TestCase):
    ...
    def test_record_acquisition_lifecycle(self):
        ...
        # 记录结束（成功）
        self.monitor.record_acquisition_end(
            acquisition_id, True, 10.5, 8.2, None
        )
