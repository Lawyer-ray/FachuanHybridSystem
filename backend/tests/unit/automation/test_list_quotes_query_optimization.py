...
class TestListQuotesQueryOptimization:
    ...
    @given(
        num_quotes=st.integers(min_value=1, max_value=20),
        quotes_per_task=st.integers(min_value=0, max_value=5)
    )
    @settings(
        max_examples=10,
        deadline=None,
    ...
    def test_list_quotes_no_n_plus_1(self, service, mock_credential, num_quotes, quotes_per_task):
        ...
        assert query_count <= max_allowed_queries, (
            ...
            f"\n查询详情:\n" + "\n".join([
                f"{i+1}. {q['sql'][:100]}..." for i, q in enumerate(connection.queries)
            ])
        )
        ...
        assert additional_queries == 0, (
            f"访问预加载的关系产生了 {additional_queries} 次额外查询！"
            f"这表明 prefetch_related 没有正确工作。"
        )
