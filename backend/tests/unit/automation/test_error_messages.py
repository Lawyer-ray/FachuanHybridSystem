    ...
    **Feature: automation-decoupling, Property 13: Error message clarity**
    ...
    def test_property_browser_creation_error_clarity(self):
        ...
        **Validates: Requirements 8.1, 8.2, 8.3, 8.4, 8.5**
        ...
        # 验证：错误消息应该包含关键信息
        assert "无法启动 Chromium" in error_msg, "错误消息应该包含主要错误描述"

        assert "headless" in error_msg, "错误消息应该包含配置信息"

        assert "RuntimeError" in error_msg, "错误消息应该包含原始错误类型"

        assert "端口 9222" in error_msg, "错误消息应该包含原始错误详情"

        # 验证：错误对象应该保存上下文
        assert error.config is not None, "错误对象应该保存配置信息"

        assert error.original_error is not None, "错误对象应该保存原始错误"

    def test_browser_configuration_error_clarity(self):
        """测试配置错误消息清晰度"""
        error = BrowserConfigurationError(field="viewport_width", value=-100, reason="宽度必须为正数")
    ...
    def test_captcha_recognition_error_clarity(self):
        """测试验证码识别错误消息清晰度"""
        error = CaptchaRecognitionError(message="OCR 引擎返回空结果", attempts=3, selector="#captcha-image")
    ...
    def test_cookie_load_error_clarity(self):
        """测试 Cookie 加载错误消息清晰度"""
        error = CookieLoadError(message="Cookie 格式无效", site_name="court_zxfw", account="user@example.com")
    ...
    def test_error_messages_are_descriptive(self):
        ...
        for error in errors:
            error_msg = str(error)
            # 错误消息不应该为空
            assert len(error_msg) > 0, f"{type(error).__name__} 错误消息不应该为空"

            # 错误消息应该包含错误类型的关键词
            error_type = type(error).__name__
            # 至少应该包含一些描述性文本
            assert len(error_msg) > 10, f"{error_type} 错误消息应该足够详细"

    def test_error_context_preservation(self):
        """测试错误上下文被正确保存"""
        # BrowserCreationError 应该保存配置和原始错误
        browser_error = BrowserCreationError("test", config={"key": "value"}, original_error=ValueError("original"))
