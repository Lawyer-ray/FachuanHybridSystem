...
class TestExtractFromPdf:
    ...
    @patch('apps.automation.services.court_document_recognition.text_extraction_service.TextExtractionService._extract_pdf_text_direct')
    ...
    @patch('apps.automation.services.court_document_recognition.text_extraction_service.TextExtractionService._extract_pdf_text_direct')
    @patch('apps.automation.services.court_document_recognition.text_extraction_service.TextExtractionService._extract_pdf_with_ocr')
    def test_pdf_fallback_to_ocr(self, mock_ocr, mock_direct):
        """测试 PDF 降级到 OCR"""
        mock_direct.return_value = ""  # 直接提取失败
        mock_ocr.return_value = TextExtractionResult(
            text="OCR识别的文字",
