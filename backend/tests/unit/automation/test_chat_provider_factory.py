...
class TestChatProviderFactory:
    ...
    def test_get_provider_platform_mismatch_raises_error(self):
        ...
        assert "无法创建群聊提供者实例" in str(exception) or "提供者实例的平台属性不匹配" in str(
            exception.errors.get("original_error", "")
        )
    ...
    def test_logging_on_register(self, mock_logger):
        ...
        mock_logger.debug.assert_called_once_with("已注册群聊提供者: feishu -> MockChatProvider")
