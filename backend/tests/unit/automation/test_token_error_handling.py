...
class TestTokenErrorHandling:
    ...
    def test_token_error_provides_helpful_message(self):
        ...
        with patch.object(service.token_service, 'get_token', return_value=None):
            # Mock get_or_create_token 也返回 None
            with patch('apps.automation.services.insurance.preservation_quote_service.get_or_create_token', return_value=None):
                # 创建询价任务
    ...
    def test_fallback_to_any_valid_token(self):
        ...
        with patch.object(service.token_service, 'get_token', return_value=None):
            ...
            with patch('apps.automation.services.insurance.preservation_quote_service.get_or_create_token', return_value="Bearer valid_token_123"):
                ...
                with patch.object(AccountCredential.objects, 'get') as mock_get:
                    ...
                    with patch.object(service.insurance_client, 'fetch_insurance_companies', new_callable=AsyncMock) as mock_fetch:
    ...
    def test_django_q_task_handles_token_error_gracefully(self):
        ...
        # Mock TokenService.get_token 返回 None
        with patch('apps.automation.services.scraper.core.token_service.TokenService.get_token', return_value=None):
            # Mock get_or_create_token 也返回 None
            with patch('apps.automation.services.insurance.preservation_quote_service.get_or_create_token', return_value=None):
                # 执行 Django Q 任务
