...
class TestCourtDocumentService:
    ...
    def test_create_document_with_case_id(self):
        ...
        # 创建律所
        lawfirm = LawFirm.objects.create(name="测试律所", social_credit_code="TEST123")

        # 创建合同（不再使用 assigned_lawyer 字段，已重构为 ContractAssignment）
        contract = Contract.objects.create(
            name="测试合同", case_type="civil", status="active", representation_stages=["investigation"]
        )

        # 创建案件
        case = Case.objects.create(name="测试案件", contract=contract, current_stage="investigation")
        ...
        document = self.service.create_document_from_api_data(
            scraper_task_id=self.scraper_task.id, api_data=api_data, case_id=case.id
        )
    ...
    def test_create_document_missing_required_fields(self):
        ...
        with pytest.raises(ValidationException) as exc_info:
            self.service.create_document_from_api_data(scraper_task_id=self.scraper_task.id, api_data=api_data)
    ...
    def test_create_document_scraper_task_not_found(self):
        ...
        with pytest.raises(NotFoundError) as exc_info:
            self.service.create_document_from_api_data(scraper_task_id=99999, api_data=api_data)
    ...
    def test_update_download_status_failed(self):
        ...
        # 更新为失败状态
        updated_document = self.service.update_download_status(
            document_id=document.id, status=DocumentDownloadStatus.FAILED, error_message="下载超时"
        )
    ...
    def test_update_download_status_invalid_status(self):
        ...
        # 使用无效状态
        with pytest.raises(ValidationException) as exc_info:
            self.service.update_download_status(document_id=document.id, status="invalid_status")
    ...
    def test_update_download_status_document_not_found(self):
        """测试文书记录不存在"""
        with pytest.raises(NotFoundError) as exc_info:
            self.service.update_download_status(document_id=99999, status=DocumentDownloadStatus.SUCCESS)
    ...
    def test_get_documents_by_task_empty(self):
        ...
        new_task = ScraperTask.objects.create(
            task_type="court_document", status="pending", url="https://test2.example.com", priority=5
        )
