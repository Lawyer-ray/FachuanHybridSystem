...
class TestDependencyInjection:
    ...
    def test_property_injected_recognizer_is_used(self):
        ...
        **Validates: Requirements 3.5**
        ...
        # 验证：
        # 1. 注入的识别器被调用
        assert mock_recognizer.recognize_from_element_called, \
            "注入的识别器应该被调用"
        
        # 2. 返回值来自注入的识别器
        assert result == "INJECTED", \
            f"应该返回注入识别器的值 'INJECTED'，但得到 '{result}'"
        
        # 3. 调用次数正确
        assert mock_recognizer.recognize_from_element_call_count == 1, \
            "识别器应该被调用一次"
    ...
    def test_multiple_recognizer_calls_use_same_instance(self):
        ...
        # 验证同一个实例被调用了 3 次
        assert mock_recognizer.recognize_from_element_call_count == 3, \
            "识别器应该被调用 3 次"
