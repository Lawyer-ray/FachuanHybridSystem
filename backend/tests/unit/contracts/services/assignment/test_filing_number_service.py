...
def test_generate_contract_filing_number_on_sqlite_does_not_fail():
    ...
    filing_number = service.generate_contract_filing_number(
        contract_id=contract.id, case_type=contract.case_type, created_year=created_year  # type: ignore[attr-defined]
    )
    assert filing_number.startswith(f"{created_year}_")
    assert "_HT_" in filing_number

    contract.filing_number = filing_number  # type: ignore[attr-defined]
    contract.save(update_fields=["filing_number"])  # type: ignore[attr-defined]
    ...
    filing_number2 = service.generate_contract_filing_number(
        contract_id=contract2.id, case_type=contract2.case_type, created_year=created_year  # type: ignore[attr-defined]
    )
