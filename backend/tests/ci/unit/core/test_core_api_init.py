...
class TestSchemas:
    ...
    def test_system_config_group_out(self):
        ...
        item = SystemConfigItemOut(key="k", value="v", category="c", description="d", is_secret=False, is_active=True)
...
class TestListSystemConfigs:
    @patch("apps.core.api.SystemConfigService")
    def test_basic(self, mock_svc):
        ...
        mock_svc.return_value.get_all_active_configs.return_value = [cfg]
        request = MagicMock()
    ...
    @patch("apps.core.api.SystemConfigService")
    def test_secret_masked(self, mock_svc):
        from apps.core.api import list_system_configs
    ...
    @patch("apps.core.api.SystemConfigService")
    def test_empty_value(self, mock_svc):
        from apps.core.api import list_system_configs
...
class TestUpdateSystemConfigs:
    @patch("apps.core.api.SystemConfigService")
    def test_update_existing_key(self, mock_svc):
        from apps.core.api import SystemConfigUpdateIn, update_system_configs

        existing = SimpleNamespace(category="c", description="d", is_secret=False)
        instance = mock_svc.return_value
        instance.get_config_by_key.return_value = existing
    ...
    @patch("apps.core.api.SystemConfigService")
    def test_create_new_key(self, mock_svc):
        from apps.core.api import SystemConfigUpdateIn, update_system_configs

        instance = mock_svc.return_value
        instance.get_config_by_key.return_value = None
...
class TestCreateSystemConfig:
    @patch("apps.core.api.SystemConfigService")
    def test_create_success(self, mock_svc):
        from apps.core.api import SystemConfigCreateIn, create_system_config
        ...
        instance.set_value.return_value = config
    ...
    @patch("apps.core.api.SystemConfigService")
    def test_create_conflict(self, mock_svc):
        from apps.core.api import SystemConfigCreateIn, create_system_config

        existing = SimpleNamespace(id=1)
        mock_svc.return_value.get_config_by_key.return_value = existing
...
class TestPatchSystemConfig:
    @patch("apps.core.api.SystemConfigService")
    def test_patch_success(self, mock_svc):
        from apps.core.api import SystemConfigPatchIn, patch_system_config

        config = SimpleNamespace(
            id=1, key="k", value="old", category="c", description="d", is_secret=False, is_active=True
        )
        instance = mock_svc.return_value
        instance.get_config_by_key.return_value = config
        ...
        instance.update_config.return_value = updated
        result = patch_system_config(request, "k", payload)
        assert result.value == "new_val"

    @patch("apps.core.api.SystemConfigService")
    def test_patch_not_found(self, mock_svc):
        from apps.core.api import SystemConfigPatchIn, patch_system_config

        mock_svc.return_value.get_config_by_key.return_value = None
...
class TestDeleteSystemConfig:
    @patch("apps.core.api.SystemConfigService")
    def test_delete_success(self, mock_svc):
        ...
        assert result["success"] is True
        instance.delete_config.assert_called_once_with(1)

    @patch("apps.core.api.SystemConfigService")
    def test_delete_not_found(self, mock_svc):
        from apps.core.api import delete_system_config
