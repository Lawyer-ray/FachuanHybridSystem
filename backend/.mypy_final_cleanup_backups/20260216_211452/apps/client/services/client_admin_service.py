...
class ClientAdminService:
    ...
    def import_from_json(self, json_data: Dict[str, Any], admin_user: str) -> ImportResult:
        ...
        try:
            ...
            return ImportResult(success=False, error_message=f"导入失败: {str(e)}")
    ...
    def _process_single_form(
        self, client_id: int, form_data: Dict[str, Any], admin_user: str
    ...
    def save_and_rename_file(self, client_id: int, client_name: str, doc_id: int, doc_type: str, uploaded_file) -> str:
        """
        保存上传文件并重命名，更新数据库记录
