...
def execute_document_recognition_task(task_id: int) -> None:
    ...
    try:
        ...
        if result.binding:
            ...
            if result.cast(int, binding.case_id):
                task.case_id = result.cast(int, binding.case_id)
...
def _send_recognition_notification(task: Any, result: Any) -> None:
    try:
        from apps.automation.services.court_document_recognition.notification_service import DocumentRecognitionNotificationService
        notification_service = DocumentRecognitionNotificationService()
        file_path = task.renamed_file_path or task.file_path
        notification_result = notification_service.send_notification(case_id=result.cast(int, binding.case_id), document_type=task.document_type, case_number=task.case_number, key_time=task.key_time, file_path=file_path, case_name=result.binding.case_name)
