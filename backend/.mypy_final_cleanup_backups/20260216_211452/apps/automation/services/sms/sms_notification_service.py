...
class SMSNotificationService:
    ...
    def send_case_chat_notification(self, sms: CourtSMS, document_paths: List[str] = None) -> bool:
        """发送案件群聊通知
        ...
        try:
            ...
            try:
                chat = chat_service.get_or_create_chat(case_id=sms.case.id, platform=platform)
            ...
            try:
                ...
                if result.success:
                    ...
                    logger.warning(
                        f"案件群聊通知发送失败: SMS ID={sms.id}, Chat ID={chat.chat_id}, " f"错误: {result.message}"
                    )
                    return False
    ...
    def _get_or_create_chat(self, case_id: int, platform: ChatPlatform) -> "CaseChat":
        ...
        """
        return self.case_chat_service.get_or_create_chat(case_id=case_id, platform=platform)
