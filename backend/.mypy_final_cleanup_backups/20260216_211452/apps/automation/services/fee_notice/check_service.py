...
class FeeNoticeCheckService:
    ...
    def check_fee_notices(
        ...
        # 获取案件比对信息
        case_info = self.comparison_service.get_case_info_for_comparison(sms.cast(int, case.id))
        ...
        for file_path in fee_notice_files:
            check_item = self._check_single_file(file_path, sms.cast(int, case.id), case_info)
            if check_item:
    ...
    def format_feishu_message(self, result: FeeCheckResult) -> Optional[str | None]:
        """格式化飞书通知消息"""
