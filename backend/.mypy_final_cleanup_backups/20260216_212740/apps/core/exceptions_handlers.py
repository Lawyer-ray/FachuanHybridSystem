...
def _parse_retry_after(raw: Any) -> int | None:
    """从 RateLimitError 的 errors 中解析 retry_after 值"""
...
def _register_business_handlers(api: NinjaAPI, create_response: _CreateResponse) -> None:
    """注册业务异常处理器"""
    _register_core_business_handlers(api, create_response)
    _register_service_error_handlers(api, create_response)


def _register_core_business_handlers(api: NinjaAPI, create_response: _CreateResponse) -> None:
    """注册核心业务异常处理器(400/401/403/404/409/429)"""
    ...
    def handle_validation_exception(request: HttpRequest, exc: ValidationException) -> HttpResponse:
        logger.info(f"验证失败: {exc.message}", extra=_log_extra(request, code=exc.code, errors=exc.errors))
    ...
    def handle_authentication_error(request: HttpRequest, exc: AuthenticationError) -> HttpResponse:
        logger.warning(f"认证失败: {exc.message}", extra=_log_extra(request, code=exc.code))
    ...
    def handle_permission_denied_exception(request: HttpRequest, exc: PermissionDenied) -> HttpResponse:
        logger.warning(f"权限拒绝: {exc.message}", extra=_log_extra(request, code=exc.code))
    ...
    def handle_not_found_exception(request: HttpRequest, exc: NotFoundError) -> HttpResponse:
        logger.info(f"资源不存在: {exc.message}", extra=_log_extra(request, code=exc.code))
    ...
    def handle_conflict_exception(request: HttpRequest, exc: ConflictError) -> HttpResponse:
        logger.info(f"资源冲突: {exc.message}", extra=_log_extra(request, code=exc.code, errors=exc.errors))
    ...
    def handle_rate_limit_exception(request: HttpRequest, exc: RateLimitError) -> HttpResponse:
        logger.warning(f"频率限制: {exc.message}", extra=_log_extra(request, code=exc.code))
...
def _register_service_error_handlers(api: NinjaAPI, create_response: _CreateResponse) -> None:
    """注册服务错误和通用业务异常处理器"""
    ...
    def handle_service_unavailable_error(request: HttpRequest, exc: ServiceUnavailableError) -> HttpResponse:
        logger.error(
            f"服务不可用: {exc.message}",
            extra=_log_extra(
                request, code=exc.code, errors=exc.errors, service_name=getattr(exc, "service_name", None)
            ),
        )
    ...
    def handle_recognition_timeout_error(request: HttpRequest, exc: RecognitionTimeoutError) -> HttpResponse:
        logger.error(
            f"识别超时: {exc.message}",
            extra=_log_extra(
                request, code=exc.code, errors=exc.errors, timeout_seconds=getattr(exc, "timeout_seconds", None)
            ),
        )
    ...
    def handle_external_service_error(request: HttpRequest, exc: ExternalServiceError) -> HttpResponse:
        logger.error(f"外部服务错误: {exc.message}", extra=_log_extra(request, code=exc.code, errors=exc.errors))
    ...
    def handle_business_exception(request: HttpRequest, exc: BusinessException) -> HttpResponse:
        logger.warning(f"业务异常: {exc.message}", extra=_log_extra(request, code=exc.code, errors=exc.errors))
    ...
    def handle_business_error(request: HttpRequest, exc: BusinessError) -> HttpResponse:
        logger.warning(f"BusinessError: {exc.code} - {exc.message}", extra=_log_extra(request))
        status = int(getattr(exc, "status", 400))
        return create_response(request, exc.to_dict(), status=status)
...
def _register_llm_handlers(api: NinjaAPI, create_response: _CreateResponse) -> None:
    ...
    try:
        ...
        logger.exception("LLM 异常处理器注册失败")
...
def _register_django_handlers(api: NinjaAPI, create_response: _CreateResponse) -> None:
    ...
    def handle_404(request: HttpRequest, exc: Http404) -> HttpResponse:
        ...
        return create_response(
            request,
            {"success": False, "code": "NOT_FOUND", "message": "资源不存在", "error": "资源不存在", "errors": {}},
            status=404,
        )
    ...
    def handle_django_permission_denied(request: HttpRequest, exc: DjangoPermissionDenied) -> HttpResponse:
        logger.warning(f"Permission denied: {request.path}", extra=_log_extra(request))
        return create_response(
            request,
            {
                "success": False,
    ...
    def handle_ninja_validation_error(request: HttpRequest, exc: ValidationError) -> HttpResponse:
        logger.info(f"Validation error: {request.path}", extra=_log_extra(request, errors=exc.errors))
        return create_response(
            request,
            {
                "success": False,
...
def _register_jwt_handler(api: NinjaAPI, create_response: _CreateResponse) -> None:
    ...
    try:
        ...
        def handle_invalid_token(request: HttpRequest, exc: InvalidToken) -> HttpResponse:
            logger.info(f"Invalid token: {request.path}")
            ...
            return create_response(
                request,
                {"success": False, "code": "INVALID_TOKEN", "message": error_msg, "error": error_msg, "errors": {}},
                status=401,
            )
...
def _register_generic_handlers(api: NinjaAPI, create_response: _CreateResponse) -> None:
    """注册通用异常处理器"""
    ...
    def handle_http_error(request: HttpRequest, exc: HttpError) -> HttpResponse:
        ...
        return create_response(
            request,
            {"success": False, "code": "HTTP_ERROR", "message": error_message, "error": error_message, "errors": {}},
    ...
    def handle_unexpected_exception(request: HttpRequest, exc: Exception) -> HttpResponse:
        ...
        return create_response(
            request,
            {"success": False, "code": "INTERNAL_ERROR", "message": message, "error": message, "errors": {}},
            status=500,
        )
...
def register_exception_handlers(api: NinjaAPI) -> None:
    ...
    _register_generic_handlers(api, _create_response)
