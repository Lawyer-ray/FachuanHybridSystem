...
class CourtSMSHelpersMixin:
    ...
    def _create_download_task(self: "_SMSHelperHost", sms: CourtSMS) -> ScraperTask | None:
        ...
        try:
            ...
            logger.error(f"创建下载任务失败: SMS ID={cast(int, sms.pk)}, 错误: {e!s}")
            return None
    ...
    def _should_wait_for_document_download(self: "_SMSHelperHost", sms: CourtSMS) -> bool:
        ...
        try:
            ...
            if task.status in [ScraperTaskStatus.SUCCESS, ScraperTaskStatus.FAILED]:
                logger.info(f"短信 {cast(int, sms.pk)} 的下载任务已完成(状态: {task.status}),不再等待")
                return False

            return self._check_documents_need_wait(sms)

        except Exception as e:
            logger.error(f"检查下载状态失败: SMS ID={cast(int, sms.pk)}, 错误: {e!s}")
            return False
    ...
    def _refresh_scraper_task_status(self, sms: CourtSMS) -> bool:
        ...
        try:
            ...
            fresh_task = ScraperTask.objects.get(id=cast(int, scraper_task.pk))
            sms.scraper_task = fresh_task
            return True
        except ScraperTask.DoesNotExist:
            logger.warning(f"短信 {cast(int, sms.pk)} 的下载任务不存在,无需等待")
            return False
    ...
    def _create_case_binding(self: "_SMSHelperHost", sms: CourtSMS) -> bool:
        ...
        try:
            ...
            logger.info(f"案件绑定创建成功: SMS ID={cast(int, sms.pk)}, CaseLog ID={cast(int, case_log.pk)}")
            return True

        except Exception as e:
            logger.error(f"创建案件绑定失败: SMS ID={cast(int, sms.pk)}, 错误: {e!s}")
            return False
    ...
    def _add_case_numbers_to_case(self: "_SMSHelperHost", sms: CourtSMS) -> None:
        ...
        try:
            ...
            if not valid_case_numbers:
                logger.info(f"短信 {cast(int, sms.pk)} 没有有效的案号需要写入")
                return

            admin_lawyer_dto = self.lawyer_service.get_admin_lawyer_internal()
            user_id = cast(int, admin_lawyer_dto.id) if admin_lawyer_dto else None
            ...
            logger.warning(f"写入案号失败: SMS ID={cast(int, sms.pk)}, 错误: {e!s}")
    ...
    def _get_document_paths_for_extraction(self, sms: CourtSMS) -> list[Any]:
        ...
        try:
            ...
            logger.warning(f"获取文书路径失败: SMS ID={cast(int, sms.pk)}, 错误: {e!s}")
