...
class EvidenceAdminService:
    ...
    def merge_and_update(self, list_id: int) -> dict[str, Any]:
        ...
        # 更新后续清单的页码
        self.evidence_service.update_subsequent_lists_pages(cast(int, evidence_list.case_id), evidence_list.order + 1)  # type: ignore[attr-defined]
    ...
    def recount_pages(self, list_id: int) -> dict[str, Any]:
        ...
        for item in items:
            item_updated, item_pages, item_error = self._recount_item_pages(item, os, get_pdf_page_count_with_error)
