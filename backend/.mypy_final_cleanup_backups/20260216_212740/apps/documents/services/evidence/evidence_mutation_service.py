...
class EvidenceMutationService:
    ...
    def create_evidence_list(self, *, case: Any, title: str, list_type: str | None = None, user: Any | None = None) -> EvidenceList:
        ...
        if (not title or not title.strip()) and list_type:
            title = dict(ListType.choices).get(list_type, list_type)  # type: ignore[assignment]
    ...
    def validate_list_type_creation(
        ...
        required_previous_type = LIST_TYPE_PREVIOUS.get(list_type)  # type: ignore[attr-defined]
