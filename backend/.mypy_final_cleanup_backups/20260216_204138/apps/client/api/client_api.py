...
def list_clients(request: Any, page: int=1, page_size: Optional[int]=None, client_type: Optional[str]=None, is_our_client: Optional[bool]=None, search: Optional[str]=None) -> Any:
    """
    获取客户列表
    ...
    clients = service.list_clients(page=page, page_size=page_size, client_type=client_type, is_our_client=is_our_client, search=search, user=user)
    return list(clients)
...
def create_client_with_docs(request: Any, payload: ClientIn, doc_types: List[str], files: List[UploadedFile]=File(...)) -> Any:
    """
    创建客户并上传文档
        ...
        for doc_type, file in zip(doc_types, files):
            ...
            with open(target_path, 'wb+') as f:
                for chunk in file.chunks():
                    f.write(chunk)
            identity_doc_service.add_identity_doc(client_id=client.id, doc_type=doc_type, file_path=os.path.abspath(target_path), user=user)
    return client
