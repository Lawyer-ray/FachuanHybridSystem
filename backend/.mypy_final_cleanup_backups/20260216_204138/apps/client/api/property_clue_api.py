...
import os
from ..schemas import PropertyClueIn, PropertyClueUpdateIn, PropertyClueOut, PropertyClueAttachmentOut, ContentTemplateOut
...
def delete_property_clue(request: Any, clue_id: int) -> tuple[Any, ...]:
    """
    删除财产线索
...
def upload_attachment(request: Any, clue_id: int, file: UploadedFile=File(...)) -> Any:
    """
    为财产线索上传附件
    ...
    with open(target_path, 'wb+') as f:
        for chunk in file.chunks():
            f.write(chunk)
    attachment = service.add_attachment(clue_id=clue_id, file_path=os.path.abspath(target_path), file_name=file.name or 'unnamed', user=user)
    return attachment

@router.delete('/property-clue-attachments/{attachment_id}', response={204: None})
def delete_attachment(request: Any, attachment_id: int) -> tuple[Any, ...]:
    """
    删除财产线索附件
...
def get_content_template(request: Any, clue_type: str) -> Any:
    """
    获取内容模板
