...
def add_identity_doc(request: Any, client_id: int, doc_type: str, file: UploadedFile=File(...)) -> dict[str, Any]:
    """
    添加证件文档
    ...
    identity_doc = service.add_identity_doc(client_id=client_id, doc_type=doc_type, file_path=os.path.abspath(target_path), user=getattr(request, 'user', None))
...
def get_identity_doc(request: Any, doc_id: int) -> dict[str, Any]:
    ...
    return {'id': identity_doc.id, 'client_id': identity_doc.client_id, 'doc_type': identity_doc.doc_type, 'file_path': identity_doc.file_path, 'created_at': identity_doc.created_at.isoformat() if hasattr(identity_doc, 'created_at') else None, 'updated_at': identity_doc.updated_at.isoformat() if hasattr(identity_doc, 'updated_at') else None}
...
def parse_text_any(request: Any) -> Any:
    """
    解析客户文本
