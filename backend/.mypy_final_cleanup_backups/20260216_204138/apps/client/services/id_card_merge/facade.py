...
class IdCardMergeService:
    ...
    def merge_id_card_with_detection(self, front_image: UploadedFile, back_image: UploadedFile) -> dict[str, Any]:
        ...
        if front_corners is None or back_corners is None:
            logger.info('自动检测失败,保存临时图片', extra={'front_detected': front_corners is not None, 'back_detected': back_corners is not None})
            front_temp_path = self._save_temp_image(front_image, 'front')
            back_temp_path = self._save_temp_image(back_image, 'back')
            return {'success': False, 'error': 'AUTO_DETECT_FAILED', 'message': '无法自动检测身份证边缘,请手动选取四角', 'front_image_url': f'/media/{front_temp_path}', 'back_image_url': f'/media/{back_temp_path}'}
    ...
    def merge_id_card_manual(self, front_image_path: str, back_image_path: str, front_corners: list[list[int]], back_corners: list[list[int]]) -> dict[str, Any]:
        ...
        for label, img in [('正面', front_cv_image), ('反面', back_cv_image)]:
            if img is None:
                return {'success': False, 'error': 'INVALID_CORNERS', 'message': f'无法读取{label}图片,请确保图片格式正确'}
    ...
    def _validate_image_format(self, image: UploadedFile) -> dict[str, Any] | None:
        return validation.validate_image_format(image, supported_formats=self.SUPPORTED_FORMATS, supported_extensions=self.SUPPORTED_EXTENSIONS)
    ...
    def _generate_pdf(self, front_image: np.ndarray, back_image: np.ndarray) -> str:
        ...
        return pdf.generate_a4_pdf(front_image, back_image, id_card_aspect_ratio=self.ID_CARD_ASPECT_RATIO, output_dir=output_dir, temp_dir=temp_dir, logger=logger)
...
def validation_np_transform(image: np.ndarray, corners: np.ndarray, *, id_card_aspect_ratio: float, logger: Any) -> np.ndarray:
    from .transform import perspective_transform
    return perspective_transform(image, corners, id_card_aspect_ratio=id_card_aspect_ratio, min_output_width=400, logger=logger)