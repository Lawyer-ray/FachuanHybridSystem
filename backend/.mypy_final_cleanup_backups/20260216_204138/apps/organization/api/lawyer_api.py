...
def create_lawyer(request: Any, payload: LawyerCreateIn, license_pdf: UploadedFile | None=File(None)) -> Any:
    """创建律师"""
    service = _get_lawyer_service()
    user = getattr(request, 'auth', None) or getattr(request, 'user', None)
...
def update_lawyer(request: Any, lawyer_id: int, payload: LawyerUpdateIn, license_pdf: UploadedFile | None=File(None)) -> Any:
    """更新律师"""
    service = _get_lawyer_service()
    user = getattr(request, 'auth', None) or getattr(request, 'user', None)
...
def delete_lawyer(request: Any, lawyer_id: int) -> dict[str, Any]:
    """删除律师"""
    service = _get_lawyer_service()
    user = getattr(request, 'auth', None) or getattr(request, 'user', None)
