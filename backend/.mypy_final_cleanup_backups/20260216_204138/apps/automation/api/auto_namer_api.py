...
def auto_namer_process(request: Any, file: UploadedFile=File(...), prompt: str=DEFAULT_FILENAME_PROMPT, model: str='qwen3:0.6b', limit: int | None=None, preview_page: int | None=None) -> Any:
    """自动命名工具API"""
    service = _get_auto_namer_service()
    result = service.process_document_for_naming(uploaded_file=file, prompt=prompt, model=model, limit=limit, preview_page=preview_page)
    return AutoToolProcessOut(text=result.get('text'), ollama_response=result.get('ollama_response'), error=result.get('error'))
...
def auto_namer_process_by_path(request: Any, payload: AutoToolProcessIn) -> Any:
    ...
    extraction = extract_document_content(file_path.as_posix(), limit=payload.limit, preview_page=payload.preview_page or 1)
    text_value = (extraction.text or '').strip()
    if not text_value:
        return AutoToolProcessOut(text=None, ollama_response=None, error='文档中没有提取到文字内容，无法生成命名')
    filename_suggestion = service.generate_filename(document_content=text_value, prompt=payload.prompt, model=payload.model)
