...
def _validate_file_format(filename: str) -> str:
    ...
    if ext not in SUPPORTED_EXTENSIONS:
        raise ValidationException(message='不支持的文件格式', code='UNSUPPORTED_FILE_FORMAT', errors={'file': f'不支持 {ext} 格式，请上传 PDF 或图片'})
    return ext
...
def recognize_document(request: Any, file: UploadedFile=File(...)) -> Any:
    """
    提交文书识别任务（异步）
    ...
    _validate_file_format(file.name or '')
    file_path = _save_uploaded_file(file)
    task = DocumentRecognitionTask.objects.create(file_path=file_path, original_filename=file.name, status=DocumentRecognitionStatus.PENDING)
    async_task('apps.automation.tasks.execute_document_recognition_task', task.id, task_name=f'document_recognition_{task.id}')
...
def get_task_status(request: Any, task_id: int) -> Any:
    """
    查询识别任务状态和结果
    """
    ...
    if task.status == 'success':
        recognition = RecognitionResultSchema(document_type=task.document_type, case_number=task.case_number, key_time=task.key_time.isoformat() if task.key_time else None, confidence=task.confidence, extraction_method=task.extraction_method)
        if task.binding_success is not None:
            binding = BindingResultSchema(success=task.binding_success, case_id=task.case_id, case_name=task.case.name if task.case else None, case_log_id=task.case_log_id, message=task.binding_message, error_code=task.binding_error_code)
    return TaskStatusResponseSchema(task_id=task.id, status=task.status, file_path=task.renamed_file_path or task.file_path, recognition=recognition, binding=binding, error_message=task.error_message, created_at=task.created_at.isoformat(), finished_at=task.finished_at.isoformat() if task.finished_at else None)
...
def search_cases_for_binding(request: Any, q: str='', limit: int=20) -> Any:
    """
    搜索可绑定的案件
    ...
    if not q or not q.strip():
        cases = Case.objects.select_related().prefetch_related('case_numbers', 'parties__client').order_by('-id')[:limit]
    else:
        search_term = q.strip()
        ...
        case_ids_by_party = CaseParty.objects.filter(client__name__icontains=search_term).values_list('case_id', flat=True)
        cases = Case.objects.filter(name_query | Q(id__in=case_ids_by_number) | Q(id__in=case_ids_by_party)).select_related().prefetch_related('case_numbers', 'parties__client').distinct().order_by('-id')[:limit]
    results: list[Any] = []
    for case in cases:
        case_numbers = [cn.number for cn in case.case_numbers.all()]
        parties = [p.client.name for p in case.parties.all() if p.client]
        results.append(CaseSearchResultSchema(id=case.id, name=case.name, case_numbers=case_numbers, parties=parties, created_at=case.start_date.isoformat() if case.start_date else None))
    logger.info(f'案件搜索完成', extra={'action': 'search_cases_for_binding', 'query': q, 'result_count': len(results)})
    return results

@router.post('/court-document/task/{task_id}/bind', response=ManualBindingResponseSchema)
def manual_bind_case(request: Any, task_id: int, payload: ManualBindingRequestSchema) -> Any:
    """
    手动绑定案件
    ...
    if task.binding_success:
        return ManualBindingResponseSchema(success=False, case_id=task.case_id, case_name=task.case.name if task.case else None, case_log_id=task.case_log_id, message='任务已绑定到案件', error_code='ALREADY_BOUND')
    binding_service = _get_case_binding_service()
    try:
        result = binding_service.manual_bind_document_to_case(task_id=task_id, case_id=payload.case_id, user=getattr(request, 'user', None))
        return ManualBindingResponseSchema(success=result.success, case_id=result.case_id, case_name=result.case_name, case_log_id=result.case_log_id, message=result.message, error_code=result.error_code)
    except NotFoundError as e:
        return ManualBindingResponseSchema(success=False, case_id=None, case_name=None, case_log_id=None, message=str(e), error_code='CASE_NOT_FOUND')
    except Exception as e:
        logger.error(f'手动绑定失败：{e}', extra={'action': 'manual_bind_case', 'task_id': task_id, 'case_id': payload.case_id, 'error': str(e)})
        return ManualBindingResponseSchema(success=False, case_id=None, case_name=None, case_log_id=None, message=f'绑定失败：{str(e)}', error_code='BINDING_ERROR')

@router.post('/court-document/task/{task_id}/update-info', response=UpdateInfoResponseSchema)
def update_task_info(request: Any, task_id: int, payload: UpdateInfoRequestSchema) -> Any:
    """
    手动更新识别信息（案号、关键时间）
    ...
    if payload.key_time is not None:
        if payload.key_time:
            try:
                ...
                raise ValidationException(message='时间格式不正确', code='INVALID_TIME_FORMAT', errors={'key_time': '请使用正确的时间格式'})
        else:
            task.key_time = None
        updated_fields.append('key_time')
    if updated_fields:
        task.save(update_fields=updated_fields)
        logger.info(f'识别信息已更新', extra={'action': 'update_task_info', 'task_id': task_id, 'updated_fields': updated_fields, 'case_number': task.case_number, 'key_time': str(task.key_time) if task.key_time else None})
    return UpdateInfoResponseSchema(success=True, message='保存成功', case_number=task.case_number, key_time=task.key_time.isoformat() if task.key_time else None)