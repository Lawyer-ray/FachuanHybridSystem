...
from apps.automation.schemas import PerformanceMetricsOut, StatisticsReportOut, HealthCheckOut, ResourceUsageOut
...
def get_statistics_report(request: HttpRequest, days: int=Query(7, description='统计天数', ge=1, le=90), site_name: Optional[str]=Query(None, description='网站名称过滤')) -> dict[str, Any]:
    """
    获取Token获取服务的统计报告
    ...
    logger.info('获取统计报告成功', extra={'days': days, 'site_name': site_name, 'total_acquisitions': report.get('total_acquisitions', 0)})
...
def warm_up_cache(request: HttpRequest, site_name: str=Query(..., description='网站名称')) -> dict[str, Any]:
    """
    预热指定网站的缓存
    ...
    return {'success': True, 'data': {'site_name': site_name, 'status': 'completed'}, 'message': f'网站 {site_name} 的缓存预热完成'}
