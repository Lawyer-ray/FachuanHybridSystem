...
class CaseLogService:
    ...
    def list_logs(self, case_id: Optional[int]=None, user: Any | None=None, org_access: Optional[Dict[str, Any]]=None, perm_open_access: bool=False) -> QuerySet:
        """
        获取日志列表
        ...
        if user and getattr(user, 'is_authenticated', False):
            if getattr(user, 'is_admin', False):
                return qs
            if org_access:
                qs = qs.filter(Q(case__assignments__lawyer_id__in=list(org_access.get('lawyers', []))) | Q(case__assignments__lawyer_id=getattr(user, 'id', None)) | Q(case_id__in=list(org_access.get('extra_cases', set())))).distinct()
        return qs

    def get_log(self, log_id: int, user: Any | None=None, org_access: Optional[Dict[str, Any]]=None, perm_open_access: bool=False) -> CaseLog:
        """
        获取单个日志
    ...
    def create_log(self, case_id: int, content: str, user: Any | None=None, reminder_type: Optional[str]=None, reminder_time: Optional[datetime]=None) -> CaseLog:
        """
        创建案件日志
        ...
        return CaseLog.objects.create(case_id=case_id, content=content, actor_id=actor_id, reminder_type=reminder_type, reminder_time=reminder_time)

    @transaction.atomic
    def update_log(self, log_id: int, data: Dict[str, Any], user: Any | None=None, org_access: Optional[Dict[str, Any]]=None, perm_open_access: bool=False) -> CaseLog:
        """
        更新案件日志（保存历史版本）
    ...
    def delete_log(self, log_id: int, user: Any | None=None, org_access: Optional[Dict[str, Any]]=None, perm_open_access: bool=False) -> Dict[str, bool]:
        """
        删除案件日志
    ...
    def upload_attachments(self, log_id: int, files: List[UploadedFile], user: Any | None=None, org_access: Optional[Dict[str, Any]]=None, perm_open_access: bool=False) -> Dict[str, int]:
        """
        上传日志附件
    ...
    def _check_case_access(self, case_obj: Any, user: Any, org_access: Optional[Dict[str, Any]]) -> bool:
        """
        检查用户是否有权限访问案件
    ...
    def _validate_attachment(self, file: UploadedFile) -> None:
        ...
        if ext not in self.ALLOWED_EXTENSIONS:
            raise ValidationException('不支持的文件类型', errors={'file': f'允许的类型: {', '.join(self.ALLOWED_EXTENSIONS)}'})
        size = getattr(file, 'size', 0)
        if size and size > self.MAX_FILE_SIZE:
            raise ValidationException('文件大小超过限制', errors={'file': f'最大允许 {self.MAX_FILE_SIZE // (1024 * 1024)}MB'})

    def get_logs_for_case(self, case_id: int, user: Any | None=None, org_access: Optional[Dict[str, Any]]=None, perm_open_access: bool=False) -> QuerySet:
        """
        获取案件的所有日志
    ...
    def get_log_versions(self, log_id: int, user: Any | None=None, org_access: Optional[Dict[str, Any]]=None, perm_open_access: bool=False) -> List[CaseLogVersion]:
        """
        获取日志的历史版本
    ...
    def delete_attachment(self, attachment_id: int, user: Any | None=None, org_access: Optional[Dict[str, Any]]=None, perm_open_access: bool=False) -> Dict[str, bool]:
        """
        删除日志附件
