...
class ContractReminderService:
    ...
    def list_reminders(self, contract_id: Optional[int]=None, user: Any | None=None, perm_open_access: bool=False) -> QuerySet:
        """
        获取提醒列表
    ...
    @transaction.atomic
    def create_reminder(self, contract_id: int, kind: str, content: str, due_date: date) -> ContractReminder:
        """
        创建提醒记录
        ...
        reminder = ContractReminder.objects.create(contract_id=contract_id, kind=kind, content=content, due_date=due_date)
        return reminder

    @transaction.atomic
    def update_reminder(self, reminder_id: int, data: dict[str, Any]) -> ContractReminder:
        """
        更新提醒记录
