...
def create_project(name: str) -> dict[str, Any]:
    """创建聊天记录取证项目。"""
    return client.post("/chat-records/projects", json={"name": name})  # type: ignore[return-value]


def list_projects() -> list[dict[str, Any]]:
    """列出当前用户的所有取证项目。"""
    return client.get("/chat-records/projects")  # type: ignore[return-value]


def list_recordings(project_id: int) -> list[dict[str, Any]]:
    """列出项目下的录屏列表。"""
    return client.get(f"/chat-records/projects/{project_id}/recordings")  # type: ignore[return-value]


def list_screenshots(project_id: int) -> list[dict[str, Any]]:
    """列出项目下的截图列表。"""
    return client.get(f"/chat-records/projects/{project_id}/screenshots")  # type: ignore[return-value]
...
def get_export_task(task_id: str) -> dict[str, Any]:
    ...
    return client.get(f"/chat-records/exports/{task_id}")  # type: ignore[return-value]
