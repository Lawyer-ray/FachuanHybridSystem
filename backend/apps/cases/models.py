...
class Case(models.Model):
    ...
    cause_of_action = models.CharField(
        max_length=128, blank=True, null=True, default="合同纠纷", verbose_name=_("案由")
    )
...
class CaseLog(models.Model):
    ...
    reminder_type = models.CharField(
        max_length=32, choices=CaseLogReminderType.choices, blank=True, null=True, verbose_name=_("提醒类型")
    )
...
class CaseChat(models.Model):
    ...
    def add_creation_audit_entry(self, action: str, details: dict, save: bool = True):
        ...
        entry = {"timestamp": timezone.now().isoformat(), "action": action, "details": details}
...
class ChatAuditLog(models.Model):
    ...
    details = models.JSONField(default=dict, verbose_name=_("操作详情"), help_text=_("操作的详细信息，以JSON格式存储"))
