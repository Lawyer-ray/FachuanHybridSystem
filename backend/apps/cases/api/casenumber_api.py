...
def list_case_numbers(request: Any, case_id: int | None = None) -> list[CaseNumberOut]:
    ...
    return service.list_numbers(case_id=case_id, user=user)  # type: ignore[no-any-return]
...
def get_case_number(request: Any, number_id: int) -> CaseNumberOut:
    ...
    service = _get_case_number_service()
    user = getattr(request, "user", None)
    return service.get_number(number_id=number_id, user=user)  # type: ignore[no-any-return]
...
def create_case_number(request: Any, payload: CaseNumberIn) -> CaseNumberOut:
    ...
    service = _get_case_number_service()
    user = getattr(request, "user", None)
    return service.create_number(  # type: ignore[no-any-return]
        case_id=payload.case_id, number=payload.number, remarks=payload.remarks, user=user
    )
...
def update_case_number(request: Any, number_id: int, payload: CaseNumberUpdate) -> CaseNumberOut:
    ...
    return service.update_number(number_id=number_id, data=data, user=user)  # type: ignore[no-any-return]
