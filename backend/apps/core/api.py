...
def get_business_config(request, case_type: str | None = None):
    """
    获取业务配置
    ...
    # 根据案件类型获取可用的阶段
    stages = [StageItem(value=v, label=l) for v, l in business_config.get_stages_for_case_type(case_type)]

    # 根据案件类型获取可用的诉讼地位
    legal_statuses = [
        LegalStatusItem(value=v, label=l) for v, l in business_config.get_legal_statuses_for_case_type(case_type)
    ]
...
def get_stages(request, case_type: str | None = None):
    """
    获取案件阶段列表
    ...
    """
    return [StageItem(value=v, label=l) for v, l in business_config.get_stages_for_case_type(case_type)]


@router.get("/legal-statuses", response=list[LegalStatusItem])
def get_legal_statuses(request, case_type: str | None = None):
    """
    获取诉讼地位列表
    ...
    Args:
        case_type: 案件类型代码，用于过滤
    """
    return [LegalStatusItem(value=v, label=l) for v, l in business_config.get_legal_statuses_for_case_type(case_type)]
