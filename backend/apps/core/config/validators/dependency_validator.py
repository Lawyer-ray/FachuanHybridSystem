...
class DependencyValidator(ConfigValidator):
    ...
    def validate(
        self, key: str, value: Any, field_def: Any | None = None, config: dict[str, Any] | None = None
    ) -> ValidationResult:
        """验证配置项依赖关系"""
    ...
    def can_validate(self, field_def: Any) -> bool:
        """检查是否可以验证指定字段"""
        return (
            (hasattr(field_def, "depends_on") and field_def.depends_on)
            or (hasattr(field_def, "conditional_dependencies") and field_def.conditional_dependencies)
            or (hasattr(field_def, "mutually_exclusive") and field_def.mutually_exclusive)
        )

    def _validate_simple_dependencies(
        self, key: str, value: Any, depends_on: list[str], config: dict[str, Any]
    ) -> ValidationResult:
        """验证简单依赖关系"""
        result = ValidationResult(is_valid=True)
    ...
    def _validate_conditional_dependencies(
        self, key: str, value: Any, conditional_deps: dict[str, Any], config: dict[str, Any]
    ) -> ValidationResult:
        """验证条件依赖关系"""
        result = ValidationResult(is_valid=True)
    ...
    def _validate_mutually_exclusive(
        self, key: str, value: Any, mutually_exclusive: list[str], config: dict[str, Any]
    ) -> ValidationResult:
        """验证互斥依赖关系"""
        result = ValidationResult(is_valid=True)
        ...
        if existing_exclusive_keys:
            ...
            result.add_error(f"配置项 '{key}' 与配置项 '{exclusive_keys_str}' 互斥，不能同时设置")
    ...
    def validate(
        ...
        for group_name, group_keys in required_groups.items():
            ...
            if not has_value:
                ...
                result.add_error(f"必需组 '{group_name}' 中的配置项 ['{group_keys_str}'] 至少需要设置一个")
