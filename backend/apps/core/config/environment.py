...
from .exceptions import SensitiveConfigError
...
class EnvironmentConfig:
    ...
    def _detect_environment(self, env_override: str | None = None) -> Environment:
        """
        检测当前环境
    ...
    def validate_sensitive_config(
        self, key: str, value: Any, is_sensitive: bool, env_var_name: str | None = None
    ) -> None:
        """
        验证敏感配置
        ...
        if self.is_production():
            ...
            if not env_value:
                raise SensitiveConfigError(key=key, environment=self.environment_name)
    ...
    def validate_all_sensitive_configs(self, config: dict[str, Any], sensitive_keys: list[str]) -> list[str]:
        """
        验证所有敏感配置
    ...
    def _get_parent_environment(self) -> Environment | None:
        """
        获取父级环境
