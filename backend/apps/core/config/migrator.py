...
class ConfigMigrator:
    ...
    def __init__(self, config_manager: ConfigManager, backup_dir: str | None = None, enable_auto_rollback: bool = True):
        """
        初始化配置迁移器
    ...
    def _register_django_core_fields(self, schema: ConfigSchema) -> None:
        ...
        schema.register(
            ConfigField(
                name="database.engine", type=str, default="django.db.backends.sqlite3", description="数据库引擎"
            )
        )
    ...
    def _register_business_fields(self, schema: ConfigSchema) -> None:
        ...
        schema.register(
            ConfigField(
                name="features.case_chat.default_platform", type=str, default="feishu", description="默认群聊平台"
            )
        )
