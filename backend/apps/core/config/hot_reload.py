...
class ConfigFileWatcher(FileSystemEventHandler):
    def __init__(self, config_manager: ConfigManager, watched_files: list[str]) -> None:
        ...
        self.watched_files = {Path(str(f)).abspath() for f in watched_files}
    ...
    def on_modified(self, event: Any) -> None:
        ...
        file_path = Path(str(event.src_path)).abspath()
...
class HotReloadManager:
    ...
    def add_watch_file(self, file_path: str) -> None:
        with self._lock:
            abs_path = Path(str(file_path)).abspath()
        ...
        with self._lock:
            ...
            try:
                ...
                for file_path in self.watched_files:
                    dir_path = str(Path(str(file_path)).parent)
