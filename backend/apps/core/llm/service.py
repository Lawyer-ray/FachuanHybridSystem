...
class LLMService:
    ...
    def get_moonshot_backend(self) -> MoonshotBackend:
        """
        获取 Moonshot 后端实例

        便捷方法,用于访问 Moonshot 特有的文件操作功能.

        Returns:
            MoonshotBackend 实例
        """
        return cast(MoonshotBackend, self._get_backend(self.BACKEND_MOONSHOT))

    def get_ollama_backend(self) -> OllamaBackend:
        """
        获取 Ollama 后端实例

        便捷方法,用于访问 Ollama 特有的 chat_with_options 功能.

        Returns:
            OllamaBackend 实例
        """
        return cast(OllamaBackend, self._get_backend(self.BACKEND_OLLAMA))
