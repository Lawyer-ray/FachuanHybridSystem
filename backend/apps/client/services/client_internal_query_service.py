...
class ClientInternalQueryService:
    def get_client(self, *, client_id: int) -> Client | None:
        return (
            Client.objects.prefetch_related("identity_docs")
            .filter(id=client_id)
            .first()
        )
    ...
    def list_property_clues_by_client(
        self, *, client_id: int
    ) -> list[PropertyClue]:
    ...
    def is_natural_person(self, *, client_id: int) -> bool:
        client_type: str | None = (
            ...
            .values_list("client_type", flat=True)
            .first()
        )
        return bool(client_type == Client.NATURAL)

    def list_identity_docs_by_client(
        self, *, client_id: int
    ) -> list[ClientIdentityDoc]:
