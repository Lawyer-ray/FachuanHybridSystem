    ...
    def custom_app_list(request: Any, app_label: Any = None) -> Any:
        ...
        if automation_app and automation_index is not None:
            ...
            for model in automation_app.get("models", []):
                ...
                is_tool_model = (
                    "DocumentDeliverySchedule" in model_name
                    or "CourtSMS" in model_name
                    or "PreservationQuote" in model_name
                    or "InsuranceQuote" in model_name
                    or "TestCourt" in model_name
                    or "DocumentRecognitionProxy" in model_name
                    or "文书送达定时任务" in model_verbose_name
                    or "法院短信" in model_verbose_name
                    or "财产保全询价" in model_verbose_name
                    or "测试法院" in model_verbose_name
                    or "文书智能识别" in model_verbose_name
                )

                # 自动化记录：Token管理、Token获取历史、文书查询历史、法院文书、任务管理
                is_record_model = (
                    "CourtToken" in model_name
                    or "TokenAcquisitionHistory" in model_name
                    or "DocumentQueryHistory" in model_name
                    or "CourtDocument" in model_name
                    or "ScraperTask" in model_name
                    or "Token管理" in model_verbose_name
                    or "Token获取历史" in model_verbose_name
                    or "文书查询历史" in model_verbose_name
                    or "法院文书" in model_verbose_name
                    or "任务管理" in model_verbose_name
                    or "爬虫任务" in model_verbose_name
                )
