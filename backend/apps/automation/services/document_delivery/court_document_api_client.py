...
class CourtApiError(ExternalServiceError):
    ...
    def __init__(self, message: str = "法院 API 调用错误", code: str | None = None, errors: dict | None = None):
...
class TokenExpiredError(TokenError):
    ...
    def __init__(self, message: str = "Token 已过期", code: str | None = None, errors: dict | None = None):
...
class CourtDocumentApiClient:
    ...
    def _make_request(self, url: str, token: str, data: dict, retry_count: int = DEFAULT_RETRY_COUNT) -> dict[str, Any]:
        """
        发送 HTTP POST 请求（带重试）
