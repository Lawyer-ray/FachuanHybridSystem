...
class FeeNoticeCheckService:
    ...
    def check_fee_notices(
        ...
        # 获取案件比对信息
        case_info = self.comparison_service.get_case_info_for_comparison(sms.cast(int, case.id))  # type: ignore[name-defined, attr-defined]
        ...
        for file_path in fee_notice_files:
            check_item = self._check_single_file(file_path, sms.cast(int, case.id), case_info)  # type: ignore[name-defined, attr-defined]
            if check_item:
