...
class AccountSelectionStrategy:
    ...
    async def _get_available_accounts(self, site_name: str, exclude_accounts: list[str]) -> list[AccountCredentialDTO]:
        ...
        def get_credentials() -> Any:
            ...
            # 获取指定站点的凭证
            credentials = organization_service.get_credentials_by_site_internal(site_name)  # type: ignore[attr-defined]
    ...
    async def update_account_statistics(self, account: str, site_name: str, success: bool) -> None:
        ...
        try:
            ...
            def update_credential() -> None:
                ...
                # 获取凭证
                cred = organization_service.get_credential_by_account_internal(account, site_name)  # type: ignore[attr-defined]

                # 更新登录统计
                if success:
                    organization_service.update_login_success_internal(cred.id)  # type: ignore[attr-defined]
                    # 重新获取更新后的值
                    cred = organization_service.get_credential_internal(cred.id)
                    return cred.login_success_count, None  # type: ignore[return-value]
                else:
                    organization_service.update_login_failure_internal(cred.id)  # type: ignore[attr-defined]
                    # 重新获取更新后的值
                    cred = organization_service.get_credential_internal(cred.id)
                    return None, cred.login_failure_count  # type: ignore[return-value]

            success_count, failure_count = await update_credential()  # type: ignore[func-returns-value, misc]

            # 更新缓存的统计信息
            stats = {
                "success_count": success_count if success else None,  # type: ignore[has-type]
                "failure_count": failure_count if not success else None,  # type: ignore[has-type]
            ...
            if success:
                ...
                logger.info(
                    ...
                    extra={
                        "account": account,
                        "site_name": site_name,
                        "success_count": success_count,  # type: ignore[has-type]
                    },
                )
            else:
                # 失败后添加到黑名单
                ...
                logger.warning(
                    ...
                    extra={
                        "account": account,
                        "site_name": site_name,
                        "failure_count": failure_count,  # type: ignore[has-type]
                    },
                )
