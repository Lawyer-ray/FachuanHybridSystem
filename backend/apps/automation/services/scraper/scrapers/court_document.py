...
class CourtDocumentScraper(BaseScraper):
    ...
    def _try_click_download(self, timeout: int = 30000) -> Any | None:
        """
        尝试多种方式点击下载按钮
    ...
    def _download_zxfw_court(self, url: str) -> dict[str, Any]:
        ...
        try:
            logger.info(
                ...
                extra={"operation_type": "direct_api_attempt", "timestamp": time.time(), "url": url},
            )
        ...
        try:
            logger.info(
                ...
                extra={"operation_type": "api_intercept_attempt", "timestamp": time.time(), "url": url},
            )
    ...
    def _save_document_to_db(
        ...
        try:
            ...
            document = self.document_service.create_document_from_api_data(
                scraper_task_id=self.task.id, api_data=document_data, case_id=self.task.case_id
            )
            ...
            if success:
                ...
                document = self.document_service.update_download_status(
                    document_id=document.id, status="success", local_file_path=filepath, file_size=file_size
                )
            else:
                # 更新为失败状态
                document = self.document_service.update_download_status(
