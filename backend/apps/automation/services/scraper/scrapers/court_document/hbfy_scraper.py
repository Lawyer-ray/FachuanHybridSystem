...
class HbfyCourtScraper(BaseCourtDocumentScraper):
    ...
    def _run_public_mode_playwright(self) -> dict[str, Any]:
        ...
        self.page.wait_for_timeout(3000)  # type: ignore[union-attr]
        ...
        if not files:
            try:
                preview_btn = self.page.locator("button:has-text('预览文书')")  # type: ignore[union-attr]
                if preview_btn.count() > 0:
                    preview_btn.first.click(force=True, timeout=3000)
                    self.page.wait_for_timeout(1000)  # type: ignore[union-attr]
    ...
    def _solve_public_captcha_if_present(self) -> None:
        captcha_input = self.page.locator("input[name='captcha']")  # type: ignore[union-attr]
        if captcha_input.count() <= 0:
            return

        captcha_image = self.page.locator("img.code_img, img[src^='data:image'], img[src*='captcha']")  # type: ignore[union-attr]
        submit_button = self.page.locator("button:has-text('提交验证'), button:has-text('提交')")  # type: ignore[union-attr]
        ...
        for _ in range(8):
            ...
            try:
                ...
                if not captcha_text:
                    ...
                    self.page.wait_for_timeout(600)  # type: ignore[union-attr]
                    continue
                ...
                self.page.wait_for_timeout(1500)  # type: ignore[union-attr]

                if self.page.locator("text=送达文书").count() > 0:  # type: ignore[union-attr]
                    return
                if self.page.locator("button:has-text('下载全部')").count() > 0:  # type: ignore[union-attr]
                    return
            except Exception:
                continue
    ...
    def _try_download_all_with_confirm(self, download_dir: Path) -> str | None:
        try:
            download_all = self.page.locator("button:has-text('下载全部'), div:has-text('下载全部')")  # type: ignore[union-attr]
            if download_all.count() <= 0:
                return None

            captured: list[Any] = []
            self.page.on("download", lambda d: captured.append(d))  # type: ignore[union-attr]

            download_all.first.click(force=True, timeout=3000)
            self.page.wait_for_timeout(500)  # type: ignore[union-attr]

            confirm_btn = self.page.locator("button:has-text('确认'), span:has-text('确认'), div:has-text('确认')")  # type: ignore[union-attr]
    ...
    def _try_expect_download(self, selector: str, download_dir: Path, prefix: str) -> str | None:
        try:
            target = self.page.locator(selector)  # type: ignore[union-attr]
            if target.count() <= 0:
                return None
            with self.page.expect_download(timeout=15000) as download_info:  # type: ignore[union-attr]
