...
class CourtDocumentGdemsDownloadMixin:
    def _download_gdems(self: "_GdemsHost", url: str) -> dict[str, Any]:
        ...
        self._gdems_click_confirm_button()  # type: ignore[attr-defined]

        screenshot_preview = self.screenshot("gdems_preview")

        download_dir = self._prepare_download_dir()
        zip_filepath = self._gdems_download_zip(download_dir)  # type: ignore[attr-defined]

        extracted_files = self._gdems_extract_zip(zip_filepath, download_dir)  # type: ignore[attr-defined]
        all_files: list[Any] = []
        return {
            "source": "sd.gdems.com",
    ...
    def _gdems_click_confirm_button(self: "_GdemsHost") -> None:
        """点击"确认并预览材料"按钮"""
        try:
            submit_button = self._gdems_find_confirm_button()  # type: ignore[attr-defined]
    ...
    def _gdems_download_zip(self: "_GdemsHost", download_dir: Path) -> Any:
        """查找下载按钮并下载 ZIP 文件"""
        download_button = self._gdems_find_download_button()  # type: ignore[attr-defined]
