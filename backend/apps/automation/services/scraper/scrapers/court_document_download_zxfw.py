...
if TYPE_CHECKING:
    ...
    class _ZxfwHost(Protocol):
        ...
        def _zxfw_detect_doc_count(self) -> int: ...
        def _zxfw_click_doc_item(self, doc_index: int, doc_count: int) -> None: ...
        def _zxfw_find_pdf_iframe(self) -> Any: ...
        def _zxfw_download_from_frame(self, frame: Any, doc_index: int, download_dir: Path) -> str | None: ...
        def _zxfw_download_from_frame_fallback(
            self, frame: Any, doc_index: int, download_dir: Path
        ) -> str | None: ...
...
class CourtDocumentZxfwDownloadMixin:
    ...
    def _download_via_api_intercept(self: "_ZxfwHost", download_dir: Path) -> dict[str, Any]:
        return self._download_via_api_intercept_with_navigation(download_dir)
