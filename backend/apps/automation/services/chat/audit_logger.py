...
class ChatAuditLogger:
    ...
    def log_chat_creation_start(
        ...
        entry = self._create_audit_entry(action="CREATE_START", case_id=case_id, success=True, details=details)
    ...
    def log_chat_creation_success(
        ...
        entry = self._create_audit_entry(
            action="CREATE_SUCCESS", case_id=case_id, chat_id=chat_id, success=True, details=details
        )
    ...
    def log_chat_creation_failure(
        ...
        entry = self._create_audit_entry(
            action="CREATE_FAILED", case_id=case_id, success=False, details=details, error_message=error
        )
    ...
    def log_owner_retry_attempt(
        ...
        entry = self._create_audit_entry(
            action="OWNER_RETRY", case_id=case_id, chat_id=chat_id, success=True, details=details
        )
    ...
    def log_configuration_error(
        ...
        Args:
            ...
            additional_details: 额外的详细信息
        """
        details = {"platform": platform, "missing_config": missing_config, "error_time": timezone.now().isoformat()}

        if additional_details:
            details.update(additional_details)

        entry = self._create_audit_entry(action="CONFIG_ERROR", success=False, details=details, error_message=error)
    ...
    def export_audit_logs(self, case_id: int | None = None, chat_id: str | None = None, format: str = "json") -> str:
        """导出审计日志
        ...
        export_info = {
            ...
            "filter_criteria": {"case_id": case_id, "chat_id": chat_id, "format": format},
        }
