...
def _get_download_service() -> Any:
    from apps.automation.services.wiring import get_invoice_download_service
...
def upload_invoices(
    request: Any,
    task_id: int,
    files: list[UploadedFile] = File(...),  # type: ignore[type-arg, assignment]
) -> dict[str, Any]:
    """多文件上传 + 自动识别"""
...
def get_task_status(
    ...
    # 序列化 datetime/date/Decimal 为字符串
