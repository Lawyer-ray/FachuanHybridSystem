...
from apps.chat_records.models import ChatRecordProject, ChatRecordScreenshot, ExportType
    ...
    def from_payload(cls, export_type: str, payload: dict[str, Any]) -> ExportLayout:
        ...
        if export_type == ExportType.DOCX:
            if images_per_page not in (1, 2):
                raise ValidationException("Word 仅支持 1 张/页 或 2 张/页")
        else:
            if images_per_page not in (1, 2):
                raise ValidationException("PDF 仅支持 1 张/页 或 2 张/页")

        return cls(images_per_page=images_per_page, show_page_number=show_page_number, header_text=header_text)
...
class ExportService:
    ...
    def _draw_pdf_image(
        self, c: Any, shot: Any, x0: float, y0: float, cell_w: float, cell_h: float, font_name: str
    ) -> int:
        from PIL import Image
        ...
        try:
            with Image.open(shot.image.path) as img:
                rgb_img = img.convert("RGB")
                rgb_img.thumbnail((max(1, int(draw_w * 2)), max(1, int(draw_h * 2))), _LANCZOS)
                img_buf = io.BytesIO()
    ...
    def _insert_docx_image(self, cell: Any, shot: Any, width: Any) -> None:
        from PIL import Image
