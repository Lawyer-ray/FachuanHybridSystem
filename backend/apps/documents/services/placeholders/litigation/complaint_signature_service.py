...
class ComplaintSignatureService(BasePlaceholderService):
    ...
    def generate_signature_info(self, case_id: int) -> str:
        ...
        for party_dict in parties:
            # 使用中文角色名
            role = legal_status_map.get(party_dict.get("legal_status"), party_dict.get("legal_status"))  # type: ignore[arg-type, arg-type]
        ...
        logger.info(f"生成起诉状签名盖章信息成功: case_id={case_id}, 签名主体数={len(signature_blocks)}")
