...
class AuthorizationMaterialGenerationService:
    ...
    def _resolve_media_path(self, media_root: str, file_path: str) -> str:
        return resolve_media_path(media_root, file_path)  # type: ignore[no-any-return]

    def _safe_arcname(self, name: str) -> str:
        return safe_arcname(name)  # type: ignore[no-any-return]

    def _safe_name(self, name: str) -> str:
        return safe_name(name)  # type: ignore[no-any-return]
    ...
    def _render_template(self, template_path: Path, context: dict[str, Any]) -> bytes:
        ...
        try:
            ...
            return DocxRenderer().render(str(template_path), context)  # type: ignore[no-any-return]
        except Exception as e:
