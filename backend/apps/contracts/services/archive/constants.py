...
CASE_MATERIAL_KEYWORD_MAPPING: dict[str, dict[str, list[str]]] = {
    ...
    #
    # 本映射与 archive_classifier.py 中的 _FILENAME_KEYWORD_TO_ARCHIVE_CODE 保持一致，
    # 确保案件材料同步和文件夹扫描两个入口使用相同的关键词规则。
    # 差异仅在于 source="case" 的清单项才出现在本映射中。
    "non_litigation": {
        "nl_12": [
            "授权委托书",
            "授权",
        ...
        "nl_8": ["法律意见书", "法律意见", "律师函"],
        "nl_9": ["其他", "其它", "关联材料", "证据清单", "证据明细", "证据材料", "材料清单", "受理通知"],
    },
    "litigation": {
        "lt_20": [
            "授权委托书",
    ...
    "criminal": {
        ...
        "cr_7": ["会见笔录", "会见被告人", "谈话笔录", "询问笔录"],
        "cr_8": ["调查", "取证", "证据清单", "证据明细", "证据材料", "材料清单", "取保候审", "调查笔录", "受理通知"],
        # cr_12 排在 cr_9 之前：实践中"辩护意见"更常指正式辩护词，而非内部承办意见
        "cr_12": ["辩护词", "辩护意见", "代理词", "代理意见"],
        "cr_9": ["承办意见", "内部意见"],
        "cr_10": ["集体讨论", "讨论记录"],
        "cr_11": ["起诉书", "起诉状", "上诉书", "上诉状", "不起诉申请"],
        "cr_13": ["出庭通知", "开庭通知", "传票"],
        "cr_14": ["裁定书", "判决书", "刑事判决", "判决", "裁定"],
        "cr_15": ["上诉书", "抗诉书"],
    },
}
...
ARCHIVE_SUBITEM_ORDER_RULES: dict[str, list[str]] = {
    ...
    "cr_15": ["传统", "要素式"],
    # ── 授权委托证明材料 ──
    # 顺序：授权委托书 → 营业执照 → 法定代表人身份证明 → 身份证 → 港澳通行证 → 所函 → 律师证 → 执业证
    "nl_12": ["授权委托书", "营业执照", "法定代表人", "身份证明", "身份证", "港澳通行证", "所函", "律师证", "执业证"],
    "lt_20": ["授权委托书", "营业执照", "法定代表人", "身份证明", "身份证", "港澳通行证", "所函", "律师证", "执业证"],
    "cr_18": ["授权委托书", "营业执照", "法定代表人", "身份证明", "身份证", "港澳通行证", "所函", "律师证", "执业证"],
    # ── 判决书、裁定书、调解书 ──
    # 顺序：判决书 → 裁定书 → 调解书 → 撤诉裁定 → 查封裁定 → 终本/执行裁定
    "lt_17": ["判决书", "裁定书", "调解书", "撤诉", "查封", "终本", "执行裁定"],
    "cr_14": ["判决书", "裁定书", "刑事判决"],
    # ── 调查材料等案件关联材料 ──
    # 顺序：证据清单 → 证据明细 → 材料清单 → 调查笔录 → 调查令
    "lt_10": ["证据清单", "证据明细", "材料清单", "调查笔录", "调查令"],
    "nl_9": ["证据清单", "证据明细", "材料清单"],
    "cr_8": ["证据清单", "证据明细", "材料清单", "调查笔录"],
    # ── 诉讼保全申请书等 ──
    # 顺序：财产保全 → 证据保全 → 受理通知书 → 缴费/交费通知
    "lt_11": ["财产保全", "证据保全", "受理通知书", "缴费通知", "交费通知"],
}
