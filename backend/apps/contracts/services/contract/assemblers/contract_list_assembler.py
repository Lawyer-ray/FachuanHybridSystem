...
class ContractListAssembler:
    ...
    def _attach_template_info(self, contracts: list[Contract]) -> None:
        ...
        for contract in contracts:
            ...
            contract._computed_matched_folder_templates = info.get("folder_template")  # type: ignore[attr-defined]
            contract._computed_has_matched_templates = bool(info.get("has_templates", False))  # type: ignore[attr-defined]
    ...
    def _attach_dtos(self, contracts: list[Contract]) -> None:
        ...
        for contract in contracts:
            ...
            if cases is not None and hasattr(cases, "all"):
                contract.case_dtos = [CaseDTO.from_model(c) for c in cases.all()]  # type: ignore[attr-defined]
