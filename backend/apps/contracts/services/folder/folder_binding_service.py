...
class FolderBindingService(FolderBindingCrudService):
    ...
    def _resolve_subdir_path(self, *, owner_type: str, subdir_key: str) -> str | None:
        return self._subdir_path_resolver.resolve(case_type=owner_type, subdir_key=subdir_key)  # type: ignore[no-any-return]

    def _sanitize_file_name(self, file_name: str) -> str:
        """兼容旧测试入口：委托给统一路径校验器。"""
        return self.path_validator.sanitize_file_name(file_name)  # type: ignore[no-any-return]

    def _normalize_relative_path(self, relative_path: str) -> str:
        """兼容旧测试入口：委托给统一路径校验器。"""
        return self.path_validator.normalize_relative_path(relative_path)  # type: ignore[no-any-return]
    ...
    def delete_binding_for_contract(self, contract_id: int) -> bool:
        """删除合同的文件夹绑定(便捷方法)"""
        return self.delete_binding(owner_id=contract_id)  # type: ignore[no-any-return]
    ...
    def extract_zip_to_bound_folder(self, contract_id: int, zip_content: bytes) -> str | None:
        ...
        return super().extract_zip_to_bound_folder(owner_id=contract_id, zip_content=zip_content)  # type: ignore[no-any-return]
