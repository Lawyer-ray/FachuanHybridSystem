...
class TestServiceExceptionTypesPreservation:
    ...
    **Validates: Requirements 3.1**
    ...
    def test_reminder_service_not_found_on_missing_contract(self) -> None:
        """create_reminder 对不存在的合同抛出 NotFoundError。"""
        from datetime import date

        from apps.contracts.services.contract_reminder_service import (
            ContractReminderService,
        )

        svc = ContractReminderService()
    ...
    def test_reminder_service_validation_on_empty_kind(self) -> None:
        """create_reminder 对空类型抛出 ValidationException。"""
        from datetime import date
        ...
        contract = Contract.objects.create(name="异常类型测试", case_type=CaseType.CIVIL)
        svc = ContractReminderService()
    ...
    def test_reminder_service_validation_on_empty_content(self) -> None:
        """create_reminder 对空内容抛出 ValidationException。"""
        from datetime import date
        ...
        contract = Contract.objects.create(name="异常类型测试2", case_type=CaseType.CIVIL)
        svc = ContractReminderService()
        with pytest.raises(ValidationException):
            svc.create_reminder(
                contract_id=contract.id,
    ...
    def test_reminder_get_not_found(self) -> None:
        """get_reminder 对不存在的 ID 抛出 NotFoundError。"""
        from apps.contracts.services.contract_reminder_service import (
            ContractReminderService,
        )
