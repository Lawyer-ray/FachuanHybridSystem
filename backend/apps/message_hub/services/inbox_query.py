...
def list_sources() -> list[MessageSource]:  # pragma: no cover
    """获取所有消息来源（含凭证关联）。"""
    return list(MessageSource.objects.select_related("credential").all())


def get_source_or_none(source_id: int) -> MessageSource | None:  # pragma: no cover
    """按 ID 获取消息来源，不存在返回 None。"""
    return MessageSource.objects.select_related("credential").filter(pk=source_id).first()


def create_source(**kwargs: Any) -> MessageSource:  # pragma: no cover
    """创建消息来源。"""
    return MessageSource.objects.create(**kwargs)


def get_enabled_sources() -> QuerySet[MessageSource]:  # pragma: no cover
    """获取所有启用的消息来源。"""
