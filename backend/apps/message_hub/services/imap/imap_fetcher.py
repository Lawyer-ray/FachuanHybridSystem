        ...
        attachments.append(
            {
                ...
                "size": len(content),
                "part_index": idx,
                "local_path": rel_path,  # 存相对路径，跨 WSL/Windows 环境可读
            }
        )
    return attachments
...
class ImapFetcher(MessageFetcher):  # pragma: no cover
    def _connect(self, source: MessageSource) -> imaplib.IMAP4_SSL:  # pragma: no cover
        ...
        for host in hosts:
            ...
            try:
                ...
                raise ValueError(f"IMAP 登录失败: host={host}, account={account}, error={e}") from e
        ...
        raise ValueError(f"IMAP 主机配置无效，未找到可用候选主机: candidates={hosts}, account={account}")
    ...
    def fetch_new_messages(self, source: MessageSource) -> int:  # pragma: no cover
        ...
        try:
            ...
            for uid_bytes in uid_list:
                uid, created = self._process_single_uid(m, source, uid_bytes)
                if created:
                    new_count += 1
    ...
    def download_attachment(
    ...
    ) -> tuple[bytes, str, str]:  # pragma: no cover
        from apps.message_hub.models import InboxMessage
        ...
        if inbox_msg and isinstance(inbox_msg.attachments_meta, list):
            for att in inbox_msg.attachments_meta:
                if int(att.get("part_index", -1)) != part_index:
                    continue
                # 兼容历史 WSL/Windows 绝对路径，再按相对 MEDIA_ROOT 解析
        ...
        try:
            ...
            for idx, part in enumerate(msg.walk()):
                ...
                filename = _decode_header_value(part.get_filename()) or f"attachment_{idx}"
                rel_path, _file_path = _build_local_attachment_path(source.pk, message_id, idx, filename)
                ...
                if inbox_msg and isinstance(inbox_msg.attachments_meta, list):
                    ...
                    for att in inbox_msg.attachments_meta:
                        if int(att.get("part_index", -1)) != idx:
                            continue
                        att["local_path"] = rel_path  # 存相对路径，跨 WSL/Windows 环境可读
