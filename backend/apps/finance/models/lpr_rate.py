...
class LPRRate(models.Model):
    ...
    effective_date = models.DateField(
        unique=True,
        verbose_name=_("生效日期"),
        help_text=_("LPR利率生效日期，通常为每月20日")
    )
    rate_1y = models.DecimalField(
        max_digits=5,
        decimal_places=2,
        verbose_name=_("一年期LPR(%)"),
        help_text=_("一年期贷款市场报价利率")
    )
    rate_5y = models.DecimalField(
        max_digits=5,
        decimal_places=2,
        verbose_name=_("五年期LPR(%)"),
        help_text=_("五年期以上贷款市场报价利率")
    )
    source = models.CharField(
        max_length=255,
        blank=True,
        verbose_name=_("数据来源"),
        help_text=_("数据来源说明，如：中国人民银行官网")
    )
    is_auto_synced = models.BooleanField(
        default=False,
        verbose_name=_("自动同步"),
        help_text=_("是否通过系统自动同步获取")
    )
