...
class ReminderAdmin(SimpleHistoryAdmin, admin.ModelAdmin):  # pragma: no cover
    ...
    def _group_events_by_day(
    ...
    ) -> dict[int, list[dict[str, object]]]:  # pragma: no cover
        """按日归集事件，同一庭审的多条同步记录合并成一条。

        合并与归一化逻辑走共享 service（apps/reminders/services/
        calendar_view_service.py）——首页工作台用的是同一份，
        避免两边口径漂移（以前 admin 只按 source_id 合并，会漏掉
        「同一个庭落了两个相邻案号」那种情况）。
        """
        from ..services.calendar_view_service import merge_events, to_event_item
        ...
        for item in merge_events(raws):
            due_local = datetime.strptime(f"{item.day} {item.time}", "%Y-%m-%d %H:%M").replace(tzinfo=tz)
            events_by_day.setdefault(due_local.day, []).append(
                {
                    "id": item.id,
                    "time": item.time,
                    "due_display": due_local.strftime("%Y-%m-%d %H:%M"),
                    # title 优先关联对象名（content 常是「明天开庭」这类工作笔记）
                    "title": item.title,
                    "content": item.content,
                    "type_label": item.kind_label,
                    "target_type": item.target_type or "未绑定",
                    # admin 以前未绑定显示"独立提醒"，保持这个文案
                    "target_name": item.target_name or "独立提醒",
                    "courtroom": item.place,
                    "location": "",
                    "lawyer_name": item.person,
                    "lawyer_names": [n for n in item.person.split("、") if n],
