...
def analyze_no_any_return_errors() -> None:
    ...
    # 生成报告
    generate_report(
        no_any_return_errors, dict(errors_by_file), dict(errors_by_module), simple_errors, complex_errors_list
    )
...
def categorize_by_complexity(errors: list[ErrorRecord]) -> tuple[list[ErrorRecord], list[ErrorRecord]]:
    """
    按函数复杂度分类
