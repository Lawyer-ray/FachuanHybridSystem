...
ERROR_TYPE_CONFIG: dict[str, ErrorConfig] = {
    ...
    'misc': {
        'priority': 4,
        'label': 'Low',
        'suggestion': '其他类型问题，查看具体错误信息进行修复。'
    },
    'override': {
        'priority': 4,
        'label': 'Low',
        'suggestion': '方法重写签名不匹配。确保子类方法签名与父类一致。'
    },
    'union-attr': {
        'priority': 4,
        'label': 'Low',
...
def calculate_priority(error: dict[str, str]) -> ErrorPriority:
    ...
    error_config: ErrorConfig = ERROR_TYPE_CONFIG.get(
        error_code,
        {'priority': 4, 'label': 'Low', 'suggestion': '查看具体错误信息进行修复。'}
    )
...
def print_priority_report(priorities: list[ErrorPriority], json_output: bool = False) -> None:
    ...
    for priority in sorted(groups.keys()):
        ...
        print(f"涉及模块: {', '.join(f'{m}({c})' for m, c in sorted(module_counts.items(), key=lambda x: x[1], reverse=True))}")
        print()
