...
class ErrorAnalyzer:
    ...
    def get_sorted_by_count(self, categorized: dict[str, list[ErrorRecord]]) -> list[tuple[str, int]]:
        """
        按错误数量排序分类结果
        ...
        """
        sorted_items = sorted(
            [(key, len(errors)) for key, errors in categorized.items()], key=lambda x: x[1], reverse=True
        )
        return sorted_items
