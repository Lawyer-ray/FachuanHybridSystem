...
ERROR_PATTERN = re.compile(r"^(apps/[^:]+):(\d+):(\d+): error: (.+?) \[([^\]]+)\]")
...
def print_analysis(analysis: dict[str, Any]) -> None:
    ...
    print("按模块统计错误数（前20个）:")
    print(f"{'='*80}")
    module_errors = sorted(analysis["module_errors"].items(), key=lambda x: x[1], reverse=True)
    ...
    print("按错误类型统计（前20个）:")
    print(f"{'='*80}")
    error_type_counts = sorted(analysis["error_type_counts"].items(), key=lambda x: x[1], reverse=True)
    ...
    print("高错误模块（错误数 > 200）:")
    print(f"{'='*80}")
    high_error_modules = [(module, count) for module, count in module_errors if count > 200]
    for module, count in high_error_modules:
        percentage = (count / analysis["total_errors"]) * 100
        print(f"  {module:50s} {count:5d} ({percentage:5.2f}%)")

        # 显示该模块的主要错误类型
        module_types = sorted(analysis["module_error_types"][module].items(), key=lambda x: x[1], reverse=True)[:5]
