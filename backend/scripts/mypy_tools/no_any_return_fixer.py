    ...
    def fix_file(self, file_path: str, errors: list[ErrorRecord]) -> FixResult:
        ...
        if transformer.modified:
            ...
            if self.write_source(full_path, modified_tree):
                ...
                logger.info(f"成功修复文件 {file_path}，" f"为 {errors_fixed} 个函数替换了返回类型")
            else:
                return FixResult(
                    file_path=file_path,
                    errors_fixed=0,
...
class ReturnTypeReplacer(ast.NodeTransformer):
    ...
    def _infer_return_type(self, func: ast.FunctionDef | ast.AsyncFunctionDef) -> ast.expr | None:
        """
        推断返回值类型
    ...
    def _create_type_node(self, type_str: str) -> ast.expr:
        ...
        if "[" in type_str:
            ...
            if len(params) == 1:
                ...
                return ast.Subscript(value=base_node, slice=ast.Tuple(elts=param_nodes, ctx=ast.Load()), ctx=ast.Load())
