...
class TypeArgFixer(BatchFixer):
    ...
    def fix_file(self, file_path: str, errors: list[ErrorRecord]) -> FixResult:
        ...
        logger.info(f"修复完成: {file_path}, " f"修复了 {errors_fixed}/{len(errors)} 个错误")
    ...
    def _fix_type_arg_error(self, tree: ast.Module, error: ErrorRecord, source_lines: list[str]) -> bool:
        """
        修复单个type-arg错误
    ...
    def _ensure_future_annotations(self, tree: ast.Module, file_path: str) -> None:
        ...
        import_node = ast.ImportFrom(module="__future__", names=[ast.alias(name="annotations", asname=None)], level=0)
...
class TypeArgVisitor(ast.NodeTransformer):
    ...
    def __init__(self, target_line: int, generic_type: str, defaults: dict[str, str]) -> None:
        """
        初始化访问器
    ...
    def visit_Name(self, node: ast.Name) -> ast.AST:
        """访问Name节点（变量名）"""
        # 检查是否是目标行的目标类型
        if hasattr(node, "lineno") and node.lineno == self.target_line and node.id == self.generic_type:
    ...
    def visit_Attribute(self, node: ast.Attribute) -> ast.AST:
        """访问Attribute节点（属性访问）"""
        # 处理 typing.Dict 等情况
        if hasattr(node, "lineno") and node.lineno == self.target_line and node.attr == self.generic_type:
    ...
    def _create_subscript_node(self, base_node: ast.Name, type_str: str) -> ast.Subscript | None:
        ...
        try:
            ...
            if len(params) == 1:
                ...
                # 多参数：dict[str, Any]
                slice_node = ast.Tuple(elts=[self._create_name_node(p) for p in params], ctx=ast.Load())

            # 构建Subscript节点
            subscript = ast.Subscript(value=base_node, slice=slice_node, ctx=ast.Load())
