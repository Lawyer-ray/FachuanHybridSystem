...
def fix_deque_types(file_path: Path) -> bool:
    ...
    # 修复 deque 类型注解
    content = re.sub(r": deque = deque\(maxlen=", r": deque[dict[str, Any]] = deque(maxlen=", content)

    # 确保导入了 deque 和 Any
    if "from collections import deque" in content and "deque[" in content:
        if "from typing import" in content and "Any" not in content:
            content = re.sub(r"from typing import ([^)]+)", r"from typing import \1, Any", content, count=1)
...
def fix_callable_types(file_path: Path) -> bool:
    ...
    # 修复 Callable 类型注解
    content = re.sub(r"loading_func: Callable\)", r"loading_func: Callable[..., Any])", content)
    content = re.sub(r"loader_func: Callable\)", r"loader_func: Callable[..., Any])", content)

    # 确保导入了 Callable
    if "Callable[" in content:
        if "from typing import" in content and "Callable" not in content:
            content = re.sub(r"from typing import ([^)]+)", r"from typing import \1, Callable", content, count=1)
...
def fix_set_types(file_path: Path) -> bool:
    ...
    # 修复 set 返回类型
    content = re.sub(r"-> set:", r"-> set[Any]:", content)
...
def fix_optional_defaults(file_path: Path) -> bool:
    ...
    content = re.sub(r"fallback_settings_key: str = None", r"fallback_settings_key: str | None = None", content)

    # 修复 default: T = None
    content = re.sub(r"default: T = None\)", r"default: T | None = None)", content)
...
def fix_cast_any_returns(file_path: Path) -> bool:
    ...
    if "from typing import" in content and "cast" not in content:
        content = re.sub(r"from typing import ([^)]+)", r"from typing import \1, cast", content, count=1)
