...
def fix_resource_monitor() -> None:
    ...
    content = re.sub(r'"timestamp": datetime\.now\(\)', r'"timestamp": datetime.now().isoformat()', content)

    # 修复 Incompatible types - 添加类型转换
    content = re.sub(r'metrics\["memory"\] = \{', r'metrics["memory"] = {  # type: ignore[assignment]', content)

    # 修复 "None" has no attribute
    content = re.sub(r"if result:", r"if result is not None:", content)
...
def fix_health() -> None:
    ...
    content = re.sub(r'"status": status,', r'"status": str(status),', content)
