...
def fix_health_py() -> None:
    ...
    content = re.sub(
        r'(\s+)"status": status,',
        r'\1"status": str(status),',
        content
    )
    
    # 修复 line 593, 597: No overload variant of "list"
    content = re.sub(
        r'list\(checks\.values\(\)\)',
        r'list(checks.values())  # type: ignore[arg-type]',
        content
    )
...
def fix_resource_monitor_py() -> None:
    ...
    content = re.sub(
        r'(\s+)"timestamp": datetime\.now\(\),',
        r'\1"timestamp": datetime.now().isoformat(),',
        content
    )
    ...
    content = re.sub(
        r'(\s+)metrics\["memory"\] = \{',
        r'\1metrics["memory"]: dict[str, Any] = {',
        content
    )
