...
'''
        content = content.replace("    def _get_position_key", to_list_method + "    def _get_position_key")
...
'''
        ...
        content = re.sub(
            r"(    def _get_position_key.*?\n(?:        .*\n)*)", r"\1" + timestamp_method, content, count=1
        )
    ...
    if "if not box:" in content and "return (0.0, 0.0)" not in content:
        content = re.sub(r"if not box:\s*return", "if not box:\n            return (0.0, 0.0)", content)
...
def fix_task_recovery_service(file_path: Path) -> bool:
    ...
    # 修复result字典类型
    content = re.sub(r"result\s*=\s*\{", "result: dict[str, Any] = {", content)
...
def fix_contract_workflows(file_path: Path) -> bool:
    ...
    # 修复关联字段访问
    content = re.sub(
        r"source_contract\.contract_parties\.all\(\)", "cast(Any, source_contract.contract_parties).all()", content
    )
    content = re.sub(r"source_contract\.assignments\.all\(\)", "cast(Any, source_contract.assignments).all()", content)
    content = re.sub(r"source_contract\.reminders\.all\(\)", "cast(Any, source_contract.reminders).all()", content)
    content = re.sub(
...
def fix_schemas_return_types(file_path: Path) -> bool:
    ...
    if "PreservationQuoteCreateSchema" in content and "PreservationQuoteSchema" in content:
        content = re.sub(
            r"(\s+def from_dict\(cls.*?\) -> )PreservationQuoteCreateSchema:", r'\1"PreservationQuoteSchema":', content
        )
        content = re.sub(
...
def fix_image_rotation_schema(file_path: Path) -> bool:
    ...
    # 修复valid_sizes类型
    content = re.sub(r"valid_sizes=\{\}", 'valid_sizes: tuple[str, ...] = ("A4", "A3", "B4", "B5")', content)
...
def fix_anti_detection(file_path: Path) -> bool:
    ...
    # 为page参数添加类型注解
    content = re.sub(
        r"def inject_stealth_script\(page\) -> None:", "def inject_stealth_script(page: Any) -> None:", content
    )
    content = re.sub(
    ...
    content = re.sub(r"def random_mouse_move\(page\) -> None:", "def random_mouse_move(page: Any) -> None:", content)
...
def fix_captcha_services(file_path: Path) -> bool:
    ...
    # 修复return result
    content = re.sub(r"return result$", "return cast(str, result)", content, flags=re.MULTILINE)
    content = re.sub(r"return cleaned_result$", "return cast(str, cleaned_result)", content, flags=re.MULTILINE)
...
def fix_core_schemas(file_path: Path) -> bool:
    ...
    # 修复return value
    content = re.sub(r"(\s+)return value$", r"\1return cast(datetime, value)", content, flags=re.MULTILINE)
    content = re.sub(r"(\s+)return getter\(\)$", r"\1return cast(str, getter())", content, flags=re.MULTILINE)
    content = re.sub(
        r"(\s+)return file_field\.url$", r"\1return cast(str, file_field.url)", content, flags=re.MULTILINE
    )
    content = re.sub(
        r"(\s+)return file_field\.path$", r"\1return cast(str, file_field.path)", content, flags=re.MULTILINE
    )
...
def fix_exceptions_factory(file_path: Path) -> bool:
    ...
    if "RecognitionTimeoutError" in content and "class RecognitionTimeoutError" not in content:
        # 添加前向引用
        content = re.sub(r'-> "RecognitionTimeoutError":', r'-> "AutomationException":', content)
...
def fix_document_delivery_schema(file_path: Path) -> bool:
    ...
    # 修复send_time参数
    content = re.sub(r"send_time=send_time,", "send_time=send_time if send_time else datetime.now(),", content)
