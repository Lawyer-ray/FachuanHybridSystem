...
def fix_typing_imports(content: str) -> str:
    ...
    content = re.sub(r"from typing import ([^;\n]*?)dict\[str, Any\],?\s*", r"from typing import \1", content)
    content = re.sub(r"from typing import ([^;\n]*?),\s*dict\[str, Any\]", r"from typing import \1", content)

    # 修复typing导入中混入了其他导入语句的情况
    # from typing import ..., from .xxx import -> from typing import ...\nfrom .xxx import
    content = re.sub(r"from typing import ([^;\n]*?),\s*from\s+", r"from typing import \1\nfrom ", content)

    # 修复typing导入中混入了logger定义的情况
    # from typing import ..., logger = -> from typing import ...\nlogger =
    content = re.sub(r"from typing import ([^;\n]*?),\s*logger\s*=", r"from typing import \1\n\nlogger =", content)
