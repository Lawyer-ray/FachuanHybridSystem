...
def fix_migrator_py():
    ...
    for line in lines:
        ...
        if "self._current_migration.migration_id" in line:
            line = line.replace("self._current_migration.migration_id", "self._ensure_current_migration().migration_id")

        if "self._current_migration.total_configs" in line:
            line = line.replace(
                "self._current_migration.total_configs", "self._ensure_current_migration().total_configs"
            )

        if "self._current_migration.migrated_configs" in line:
            line = line.replace(
                "self._current_migration.migrated_configs", "self._ensure_current_migration().migrated_configs"
            )
    ...
    content = re.sub(r"(\s+)analysis\s*=\s*\{", r"\1analysis: dict[str, Any] = {", content, count=1)
    ...
    content = re.sub(r"(\s+)report:\s*Collection\[str\]\s*=", r"\1report: dict[str, Any] =", content)
