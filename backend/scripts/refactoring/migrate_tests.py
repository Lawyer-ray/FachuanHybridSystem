...
class TestFileMigrator:
    ...
    def scan_test_files(self) -> None:
        ...
        logger.info("找到 %d 个测试文件需要迁移", len(self.migrations))
    ...
    def _analyze_imports(self, file_path: Path) -> None:
        ...
        try:
            ...
            logger.warning("无法分析 %s: %s", file_path, e)

    def update_imports(self) -> None:
        """更新导入路径"""
        if not self.import_updates:
            logger.info("没有需要更新的导入路径")
            return

        logger.info("更新 %d 个文件的导入路径...", len(self.import_updates))

        for file_path, updates in self.import_updates.items():
            if self.dry_run:
                logger.info("[DRY RUN] 更新 %s", file_path)
                for old_pattern, new_pattern in updates:
                    logger.info("  %s -> %s", old_pattern, new_pattern)
            else:
                try:
                    content = file_path.read_text(encoding="utf-8")
                    ...
                    logger.error("无法更新 %s: %s", file_path, e)

    def cleanup_old_files(self) -> None:
        """清理旧的测试文件"""
        if self.dry_run:
            logger.info("[DRY RUN] 将删除以下文件:")
            for source, _ in self.migrations:
                logger.info("  %s", source)
            return

        logger.info("清理旧的测试文件...")
        for source, _ in self.migrations:
            try:
                if source.exists():
                    source.unlink()
                    logger.info("✓ 删除 %s", source)
            except Exception as e:
                logger.error("无法删除 %s: %s", source, e)
        ...
        for app_dir in apps_path.iterdir():
            ...
            if tests_dir.exists() and tests_dir.is_dir():
                ...
                if not any(tests_dir.iterdir()):
                    try:
                        tests_dir.rmdir()
                        logger.info("✓ 删除空目录 %s", tests_dir)
                    except Exception as e:
                        logger.warning("无法删除 %s: %s", tests_dir, e)
...
def main():
    ...
    # 创建迁移器
    migrator = TestFileMigrator(root_path=root_path, dry_run=not args.execute)

    # 运行迁移
    migrator.run()

    if not args.execute:
        logger.info("提示: 使用 --execute 参数实际执行迁移")
