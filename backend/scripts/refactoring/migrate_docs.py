...
from scripts.refactoring.migrate_structure import CreateDirectoryMigration, MoveFileMigration, StructureMigrator
...
def migrate_documentation(dry_run: bool = True):
    ...
    # 确保目标目录存在
    docs_subdirs = ["docs/api", "docs/architecture", "docs/guides", "docs/operations", "docs/quality"]

    for subdir in docs_subdirs:
        migrator.add_migration(CreateDirectoryMigration(root_path / subdir))
    ...
    for source_rel, dest_rel in doc_migrations:
        ...
        if source.exists():
            migrator.add_migration(MoveFileMigration(source, destination))
        else:
...
def main():
    ...
    parser.add_argument("--execute", action="store_true", help="执行实际迁移（默认为 dry-run）")
