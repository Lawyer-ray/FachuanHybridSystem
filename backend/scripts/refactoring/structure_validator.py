...
class ProjectStructureValidator:
    ...
    def validate_app_structure(self, app_name: str) -> List[str]:
        ...
        models_pkg = app_path / "models" / "__init__.py"
        if not models_file.exists() and not models_pkg.exists():
            errors.append(f"Missing file in {app_name}: models.py")

        schemas_file = app_path / "schemas.py"
        schemas_pkg = app_path / "schemas" / "__init__.py"
        if not schemas_file.exists() and not schemas_pkg.exists():
            errors.append(f"Missing file in {app_name}: schemas.py")
    ...
    def validate_scripts_structure(self) -> List[str]:
        ...
        existing_subdirs = [d.name for d in scripts_path.iterdir() if d.is_dir() and not d.name.startswith(".")]
