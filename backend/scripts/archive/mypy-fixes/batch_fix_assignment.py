...
def get_assignment_errors():
    """获取所有assignment错误"""
    result = subprocess.run(["python", "-m", "mypy", "apps/", "--strict"], capture_output=True, text=True)
    ...
    for line in lines:
        if "[assignment]" in line and "error:" in line:
            ...
            if match:
                ...
                errors.append({"file": file_path, "line": int(line_num), "col": int(col), "message": message})
