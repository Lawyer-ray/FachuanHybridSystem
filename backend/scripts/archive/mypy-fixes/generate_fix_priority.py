...
def calculate_module_priority(module: str, error_count: int, total_errors: int) -> float:
    """
    计算模块修复优先级
...
def generate_priority_report(data: dict[str, Any]) -> None:
    ...
    for i, (module, count, importance, priority) in enumerate(module_priorities, 1):
        logger.info(f"{i:<4} {module:<40} {count:<8} {importance:<8} {priority:.2f}")
    ...
    for i, (error_type, count, complexity, priority) in enumerate(error_priorities, 1):
        logger.info(f"{i:<4} {error_type:<25} {count:<8} {complexity:<8} {priority:.2f}")
    ...
    simple_errors = [(et, c) for et, c, comp, _ in error_priorities if comp <= 3]
    ...
    core_modules = [(m, c) for m, c, imp, _ in module_priorities if imp >= 8]
    ...
    business_modules = [(m, c) for m, c, imp, _ in module_priorities if 6 <= imp < 8]
    ...
    automation_modules = [(m, c) for m, c, imp, _ in module_priorities if imp < 6]
