...
def verify_indexes():
    ...
    expected_indexes = [["site_name", "-last_login_success_at"], ["site_name", "is_preferred"]]
...
def verify_data_migration():
    ...
    for credential in credentials:
        if (
            credential.login_success_count is None
            or credential.login_failure_count is None
            or credential.is_preferred is None
        ):
...
def verify_database_constraints():
    ...
    with connection.cursor() as cursor:
        ...
        # 验证新字段存在
        required_columns = ["last_login_success_at", "login_success_count", "login_failure_count", "is_preferred"]
