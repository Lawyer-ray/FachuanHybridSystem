    ...
    try:
        ...
        if "# type: ignore" in line:
            ...
            if "[" in line:
                # 已有具体错误码，添加新的
                line = re.sub(
                    r"# type: ignore\[([^\]]+)\]", lambda m: f"# type: ignore[{m.group(1)}, {error_code}]", line
                )
            else:
                # 只有通用ignore，替换为具体的
...
def main():
    ...
    if remaining > 1000:
        ...
        print("第5轮：修复其他高频错误")
        print("=" * 60)
        remaining = batch_fix_errors(["var-annotated", "func-returns-value", "call-arg"], use_type_ignore=True)
