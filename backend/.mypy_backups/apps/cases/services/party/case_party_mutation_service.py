...
class CasePartyMutationService:
        ...
        *,
        ...
        for existing_status, client_name in our_party_statuses:
            ...
            if existing_config and existing_config.group == opposing_group:
                ...
                conflict_msg = (
                    f"我方当事人诉讼地位冲突:"
                    f"案件中已有我方当事人「{client_name}」"
                    f"为{existing_status_label},"
                    f"不能再添加我方当事人为{new_status_label}"
                )
                raise ValidationException(
    ...
    def _validate_update_uniqueness(self, party_id: int, party: CaseParty, new_case_id, new_client_id) -> None:
        """验证更新后不会产生重复当事人"""
