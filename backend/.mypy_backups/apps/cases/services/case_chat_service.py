...
class CaseChatService:
    ...
    def create_chat_for_case(
        ...
        try:
            with transaction.atomic():
                ...
                logger.info(f"群聊创建成功: case_id={case_id}, chat_id={result.chat_id}, "
    ...
    def get_or_create_chat(
        ...
        if existing_chat:
            logger.debug(f"找到现有群聊: chat_id={existing_chat.chat_id}, "
    ...
    def send_document_notification(
        ...
        Example:
            ...
            if result.success:
                print("通知发送成功")
        """
        logger.info(f"发送文书通知: case_id={case_id}, platform={platform.value}, "
        ...
        try:
            ...
            if not result.success:
                logger.error(f"消息发送失败: chat_id={chat.chat_id}, "
                ...
                if self._is_chat_not_found_error(result):
                    ...
                    try:
                        ...
                        logger.info(f"创建新群聊成功，重试发送消息: old_chat_id={chat.chat_id}, "
            ...
            if document_paths and result.success:
                ...
                if successful_files == len(document_paths):
                    ...
                    logger.warning(f"部分文件发送失败: chat_id={chat.chat_id}, "
            ...
            logger.info(f"文书通知发送完成: case_id={case_id}, chat_id={chat.chat_id}, "
            ...
            logger.error(f"发送文书通知时发生未预期错误: case_id={case_id}, "
    ...
    def bind_existing_chat(
        ...
        """
        logger.info(f"绑定已存在的群聊: case_id={case_id}, platform={platform.value}, "
                   f"chat_id={chat_id}")
        ...
        try:
            with transaction.atomic():
                ...
                logger.info(f"群聊绑定成功: case_id={case_id}, chat_id={chat_id}, "
                           f"platform={platform.value}, name={chat_name}")
                
                return case_chat
                
        except Exception as e:
            logger.error(f"创建群聊绑定记录失败: case_id={case_id}, chat_id={chat_id}, "
                        f"error={str(e)}")
            raise ValidationException(
                message="创建群聊绑定记录失败",
