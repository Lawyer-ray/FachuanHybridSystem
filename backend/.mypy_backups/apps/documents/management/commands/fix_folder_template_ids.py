...
class Command(BaseCommand):
    ...
    def _detect_cross_template_duplicates(self, *, repair_service: FolderTemplateStructureIdRepairService, templates: list[FolderTemplate]) -> None:
    ...
    def _fix_templates(self, *, repair_service: FolderTemplateStructureIdRepairService, templates: list[FolderTemplate], dry_run: bool) -> int:
        ...
        for template in templates:
            ...
            if not template.structure or not isinstance(template.structure, dict):
                ...
                continue
            fixed_structure, changes = repair_service.repair_structure_ids_global(structure=template.structure, global_used_ids=global_used_ids)
            if changes > 0:
