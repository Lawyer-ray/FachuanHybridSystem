...
from apps.documents.services.pdf_merge_utils import add_page_numbers as add_page_numbers_util
from apps.documents.services.pdf_merge_utils import convert_docx_to_pdf, convert_image_to_pdf
...
class PDFMergeValidator:
    ...
    def get_items(self, evidence_list: EvidenceList) -> Any:
        ...
        if not items.exists():
            raise ValidationException(message='证据清单没有任何文件', code='NO_FILES_TO_MERGE', errors={'evidence_list_id': cast(int, evidence_list.pk)})
        return items

    def assert_supported_format(self, ext: str, file_path: str) -> None:
        if ext not in self.SUPPORTED_FORMATS:
            raise BusinessException(message=f'不支持的文件格式: {ext}', code='UNSUPPORTED_FILE_FORMAT', errors={'file_path': file_path, 'extension': ext})
...
class PDFMergeWorkflow:
    ...
    def merge_evidence_files(self, evidence_list: EvidenceList, progress_callback: Callable[[int, int, str], None] | None=None) -> str:
        ...
        try:
            ...
            raise BusinessException(message=f'PDF 合并失败: {e!s}', code='PDF_MERGE_FAILED', errors={'original_error': str(e)}) from e

    def _merge_all_items(self, merged_pdf: Any, items: Any, temp_files: Any, total_files: Any, progress_callback: Any) -> None:
        import pikepdf
        for index, item in enumerate(items, start=1):
            try:
                ...
                raise BusinessException(message=f'处理文件 {item.file_name} 失败: {e!s}', code='FILE_CONVERSION_FAILED', errors={'item_id': item.id, 'file_name': item.file_name}) from e
