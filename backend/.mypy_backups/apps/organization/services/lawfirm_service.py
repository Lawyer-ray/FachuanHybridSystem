...
class LawFirmService:
    ...
    def get_lawfirm(self, lawfirm_id: int, user: Lawyer) -> LawFirm:
        ...
        if not lawfirm:
            raise NotFoundError(message=f"律所不存在", code="LAWFIRM_NOT_FOUND")
    ...
    def create_lawfirm(self, data: LawFirmIn, user: Lawyer) -> LawFirm:
        ...
        if not self._check_create_permission(user):
            logger.warning(
                f"用户 {user.id} 尝试创建律所但权限不足", extra={"user_id": user.id, "action": "create_lawfirm"}
            )
        ...
        # 4. 记录日志
        logger.info(f"律所创建成功", extra={"lawfirm_id": lawfirm.id, "user_id": user.id, "action": "create_lawfirm"})
    ...
    def update_lawfirm(self, lawfirm_id: int, data: LawFirmUpdateIn, user: Lawyer) -> LawFirm:
        ...
        # 5. 记录日志
        logger.info(f"律所更新成功", extra={"lawfirm_id": lawfirm.id, "user_id": user.id, "action": "update_lawfirm"})
    ...
    def delete_lawfirm(self, lawfirm_id: int, user: Lawyer) -> None:
        ...
        # 5. 记录日志
        logger.info(f"律所删除成功", extra={"lawfirm_id": lawfirm_id, "user_id": user.id, "action": "delete_lawfirm"})
