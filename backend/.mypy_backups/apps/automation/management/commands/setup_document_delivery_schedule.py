...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser: Any) -> None:
        parser.add_argument('--interval', type=int, default=5, help='执行间隔（分钟），默认5分钟')
        parser.add_argument('--remove', action='store_true', help='移除现有的调度任务')
        parser.add_argument('--name', type=str, default='document_delivery_periodic_check', help='调度任务名称')
    ...
    def handle(self, *args, **options) -> None:
        ...
        if options['remove']:
            ...
            return
        if is_dry_run:
            self.stdout.write(f'将创建调度任务: {schedule_name}\n  - 执行间隔: {interval_minutes} 分钟\n  - 执行命令: execute_document_delivery_schedules')
        else:
            try:
                task_id = schedule('django.core.management.call_command', 'execute_document_delivery_schedules', schedule_type='I', minutes=interval_minutes, name=schedule_name, repeats=-1)
                self.stdout.write(self.style.SUCCESS(f'成功创建调度任务: {schedule_name}\n  - 执行间隔: {interval_minutes} 分钟\n  - 任务ID: {task_id}\n  - 执行命令: execute_document_delivery_schedules'))
                logger.info(f'文书送达调度任务已创建: name={schedule_name}, interval={interval_minutes}分钟, task_id={task_id}')
    ...
    def _show_current_schedules(self) -> None:
        ...
        if schedules.exists():
            ...
            for schedule in schedules:
                ...
                self.stdout.write(f'  - [{schedule.id}] {schedule.name} - {status}\n    间隔: {schedule.minutes}分钟, 下次运行: {next_run}')
        else:
            self.stdout.write('\n当前没有文书送达相关的调度任务')