...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser: Any) -> None:
        parser.add_argument('--interval', type=int, default=5, help='Django Q 检查间隔（分钟），默认5分钟')
        parser.add_argument('--reset', action='store_true', help='重置所有文书送达相关的调度任务')
    ...
    def _setup_schedules(self, schedule_service: Any, interval_minutes: Any, is_dry_run: Any) -> None:
        """设置调度任务"""
        schedule_name = 'document_delivery_periodic_check'
        self.stdout.write(f'设置 Django Q 调度任务:')
        self.stdout.write(f'  - 任务名称: {schedule_name}')
        self.stdout.write(f'  - 检查间隔: {interval_minutes} 分钟')
        self.stdout.write(f'  - 执行命令: execute_document_delivery_schedules')
        if not is_dry_run:
            task_id = schedule_service.setup_django_q_schedule(interval_minutes=interval_minutes, schedule_name=schedule_name)
    ...
    def _reset_schedules(self, schedule_service: Any, is_dry_run: Any) -> None:
        """重置调度任务"""
        schedule_name = 'document_delivery_periodic_check'
        self.stdout.write(f'重置文书送达调度任务:')
