...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser: Any) -> None:
        parser.add_argument('--dry-run', action='store_true', help='只显示到期任务，不实际执行')
        parser.add_argument('--schedule-id', type=int, help='执行指定的定时任务ID')
        parser.add_argument('--force', action='store_true', help='强制执行指定任务（忽略是否到期）')
    ...
    def handle(self, *args, **options) -> None:
        ...
        for schedule in schedules:
            try:
                ...
                logger.info(f'定时任务执行完成: schedule_id={schedule.id}, processed={result.processed_count}, failed={result.failed_count}')
            except Exception as e:
                error_msg = f'执行定时任务失败: schedule_id={schedule.id}, 错误: {str(e)}'
                logger.error(error_msg)
                total_failed += 1
                if verbose:
                    self.stdout.write(self.style.ERROR(f'任务 [{schedule.id}] 执行失败: {str(e)}'))
        if verbose:
            self.stdout.write('=' * 60)
    ...
    def _get_specific_schedule(self, schedule_id: Any, force: Any=False) -> list:
        """获取指定的定时任务"""
        ...
        if not force and schedule.next_run_at and (schedule.next_run_at > timezone.now()):
            self.stdout.write(self.style.WARNING(f'任务 [{schedule_id}] 尚未到期 ({schedule.next_run_at})，使用 --force 强制执行'))
            return []
        return [schedule]

    def _show_schedule_info(self, schedules: Any) -> None:
        """显示定时任务信息"""
        self.stdout.write(f'\n找到 {len(schedules)} 个需要执行的定时任务:')
        for schedule in schedules:
            credential_info = self._get_credential_info(schedule.credential_id)
            self.stdout.write(f'  - [{schedule.id}] {credential_info} - 每天{schedule.runs_per_day}次，间隔{schedule.hour_interval}小时 - 截止{schedule.cutoff_hours}小时内')
    ...
    def _get_credential_info(self, credential_id: Any) -> Any:
        """获取凭证信息"""
        try:
    ...
    def _show_execution_result(self, schedule: Any, result: Any) -> None:
        """显示执行结果"""
        self.stdout.write(f'  结果: 发现 {result.total_found} 个文书，处理 {result.processed_count} 个，跳过 {result.skipped_count} 个，失败 {result.failed_count} 个')
