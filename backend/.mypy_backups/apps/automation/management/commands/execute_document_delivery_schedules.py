...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options) -> None:
        ...
        for schedule in schedules:
            try:
                ...
                logger.info(
                    f"定时任务执行完成: schedule_id={schedule.id}, processed={result.processed_count}, failed={result.failed_count}"
                )
    ...
    def _show_schedule_info(self, schedules: Any) -> None:
        ...
        for schedule in schedules:
            ...
            self.stdout.write(
                f"  - [{schedule.id}] {credential_info} - 每天{schedule.runs_per_day}次，间隔{schedule.hour_interval}小时 - 截止{schedule.cutoff_hours}小时内"
            )
    ...
    def _show_execution_result(self, schedule: Any, result: Any) -> None:
        """显示执行结果"""
        self.stdout.write(
            f"  结果: 发现 {result.total_found} 个文书，处理 {result.processed_count} 个，跳过 {result.skipped_count} 个，失败 {result.failed_count} 个"
        )
