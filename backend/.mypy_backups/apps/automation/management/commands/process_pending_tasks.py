...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser: Any) -> None:
        parser.add_argument('--reset', action='store_true', help='同时重置卡住的 running 任务')
        parser.add_argument('--dry-run', action='store_true', help='只显示待处理任务，不实际执行')
    ...
    def handle(self, *args, **options) -> None:
        ...
        if options['dry_run']:
            ...
            if pending_count > 0:
                ...
                pending_tasks = ScraperTask.objects.filter(status=ScraperTaskStatus.PENDING).order_by('priority', '-created_at')[:10]
