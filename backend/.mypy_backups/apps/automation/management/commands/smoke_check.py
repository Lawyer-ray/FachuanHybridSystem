...
class _DummyAutoNamerService:
    ...
    def process_document_for_naming(self, uploaded_file: Any, prompt: Any, model: Any, limit: Any=None, preview_page: Any=None) -> None:
        return {'text': 'ok', 'ollama_response': {'filename': uploaded_file.name}, 'error': None}
...
class _DummyDocumentProcessorService:
    ...
    def process_uploaded_file(self, uploaded_file: Any, limit: Any=None, preview_page: Any=None) -> None:
        return self._Result(getattr(uploaded_file, 'name', 'unknown'))
...
class Command(BaseCommand):
    ...
    def _ensure_smoke_superuser(self) -> None:
        ...
        if user:
            ...
            return user
        return User.objects.create_superuser(username=username, email='smoke_admin@example.com', password='smoke_admin_password')
    ...
    def _check_upload_endpoints(self, client: Client) -> None:
        ...
        with patch('apps.core.dependencies.build_document_processing_service', return_value=_DummyDocumentProcessorService()):
    ...
    def _check_websocket(self, client: Client, user: Any) -> None:
        ...
        async def _run() -> None:
            ...
            with patch('apps.litigation_ai.services.conversation_flow_service.ConversationFlowService.handle_init', autospec=True, return_value=None):
                communicator = WebsocketCommunicator(application, f'/ws/litigation/sessions/{session.session_id}/', headers=[])
    ...
    def _check_django_q(self) -> None:
        ...
        proc = SubprocessRunner(allowed_programs={str(sys.executable)}).popen(args=[], cwd=str(Path(__file__).resolve().parents[4] / 'apiSystem'), env=env, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
