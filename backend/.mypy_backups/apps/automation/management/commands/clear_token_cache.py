...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options) -> None:
        ...
        if do_all:
            ...
            return
        manager.invalidate_site_cache(site, accounts=accounts or None)
        if do_blacklist:
            manager.invalidate_blacklist_cache()
        masked: list[Any] = []
        self.stdout.write(self.style.SUCCESS(f'已执行:site={site!r} credentials=1 token={len(accounts)} account_stats={len(accounts)}' + (f' accounts={masked!r}' if masked else '') + (' blacklist=1' if do_blacklist else '')))
