...
from apps.automation.services.court_document_recognition.data_classes import BindingResult, DocumentType, RecognitionResponse, RecognitionResult
...
class RecognizeCourtDocumentUsecase:
    ...
    def execute(self, *, file_path: str, user: Any | None=None) -> RecognitionResponse:
        ...
        try:
            ...
            recognition = RecognitionResult(document_type=doc_type, case_number=case_number, key_time=key_time, raw_text=extraction_result.text, confidence=confidence, extraction_method=extraction_result.extraction_method)
            binding, renamed_file_path = self._bind_document(doc_type, case_number, key_time, extraction_result.text, file_path, user)
            logger.info('文书识别完成', extra={'action': 'recognize_document', 'document_type': doc_type.value, 'case_number': case_number})
    ...
    def _empty_response(self, file_path: Any, extraction_result: Any) -> None:
        """文本提取失败时返回空结果"""
        return RecognitionResponse(recognition=RecognitionResult(document_type=DocumentType.OTHER, case_number=None, key_time=None, raw_text='', confidence=0.0, extraction_method=extraction_result.extraction_method), binding=BindingResult.failure_result(message='无法从文书中提取文字', error_code='TEXT_EXTRACTION_FAILED'), file_path=file_path)
    ...
    def _bind_document(self, doc_type: Any, case_number: Any, key_time: Any, raw_text: Any, file_path: Any, user: Any) -> None:
        """绑定文书到案件,返回 (binding, renamed_file_path)"""
        _UNSUPPORTED = {DocumentType.OTHER: ('暂时只支持传票识别,其他文书类型敬请期待', 'UNSUPPORTED_DOCUMENT_TYPE'), DocumentType.EXECUTION_RULING: ('执行裁定书绑定功能开发中,敬请期待', 'FEATURE_NOT_IMPLEMENTED')}
        if doc_type != DocumentType.SUMMONS or not case_number:
            if not case_number and doc_type == DocumentType.SUMMONS:
                return (BindingResult.failure_result(message='未识别到案号,无法绑定案件', error_code='CASE_NUMBER_NOT_FOUND'), file_path)
        ...
        log_content = self.binding_service.format_log_content(document_type=doc_type, case_number=case_number, key_time=key_time, raw_text=raw_text)
        binding = self.binding_service.bind_document_to_case(case_number=case_number, document_type=doc_type, content=log_content, key_time=key_time, file_path=renamed_file_path, user=user)
        return (binding, renamed_file_path)

    def _rename_document(self, *, file_path: str, document_type: DocumentType, case_name: str) -> str:
        try:
            title_map = {DocumentType.SUMMONS: '传票', DocumentType.EXECUTION_RULING: '执行裁定书', DocumentType.OTHER: '司法文书'}
            title = title_map.get(document_type, '司法文书')
            new_filename = self.document_renamer.generate_filename(title=title, case_name=case_name, received_date=date.today())
            ...
            logger.info('文书重命名成功', extra={'action': 'rename_document', 'original_path': file_path, 'new_path': str(new_path), 'document_type': document_type.value, 'case_name': case_name})
            return str(new_path)
