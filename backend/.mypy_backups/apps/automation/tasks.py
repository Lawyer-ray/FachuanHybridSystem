...
def check_stuck_tasks() -> None:
    ...
    if stuck_tasks:
        monitor_service.send_alert('任务超时告警', f'发现 {len(stuck_tasks)} 个任务执行超时（>30分钟）', level='warning')
...
def execute_scraper_task(task_id: int, **kwargs) -> None:
    ...
    try:
        ...
        if task.can_retry():
            ...
            Schedule.objects.create(func='apps.automation.tasks.execute_scraper_task', args=str(task.id), schedule_type=Schedule.ONCE, next_run=next_run_time, name=f'retry_task_{task.id}_{task.retry_count}')
...
def _send_recognition_notification(task: Any, result: Any) -> None:
    ...
    try:
        ...
        notification_result = notification_service.send_notification(case_id=result.binding.case_id, document_type=task.document_type, case_number=task.case_number, key_time=task.key_time, file_path=file_path, case_name=result.binding.case_name)
        ...
        task.save(update_fields=['notification_sent', 'notification_sent_at', 'notification_file_sent', 'notification_error'])
    except Exception as e:
