...
class DocumentDeliveryProcessor:
    ...
    def __init__(self, case_matcher: Optional['CaseMatcher']=None, document_renamer: Optional['DocumentRenamer']=None, notification_service: Optional['SMSNotificationService']=None):
        """初始化文书处理服务，支持依赖注入"""
    ...
    def process_downloaded_document(self, file_path: str, record: DocumentDeliveryRecord, credential_id: int) -> DocumentProcessResult:
        ...
        try:
            ...
            process_result = self.process_sms_in_thread(record=record, file_path=file_path, extracted_files=extracted_files or [file_path], credential_id=credential_id)
            ...
            result.error_message = f'处理下载文书失败: {str(e)}'
            logger.error(result.error_message)
        return result

    def process_sms_in_thread(self, record: DocumentDeliveryRecord, file_path: str, extracted_files: List[str], credential_id: int) -> dict[str, Any]:
        """在独立线程中执行 SMS 处理流程"""
        ...
        def do_process():
            try:
                ...
                if matched_case:
                    ...
                    renamed_files, case_log_id = self.rename_and_attach_documents(sms=sms, case=matched_case, extracted_files=extracted_files)
                    if renamed_files:
    ...
    def match_case_by_document_parties(self, document_paths: List[str]) -> None:
        """
        从文书中提取当事人进行案件匹配
            ...
            try:
                if os.path.exists(file_path):
                    with open(file_path, 'rb') as f:
                        file_content = f.read()
                    uploaded_file = SimpleUploadedFile(name=os.path.basename(file_path), content=file_content, content_type='application/octet-stream')
                    case_log_service.upload_attachments(log_id=log_id, files=[uploaded_file], user=None, perm_open_access=True)
            except Exception as e:
    ...
    def extract_zip_if_needed(self, file_path: str) -> Optional[List[str]]:
