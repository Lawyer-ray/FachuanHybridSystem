...
from .data_classes import DocumentDeliveryRecord, DocumentQueryResult, DocumentProcessResult, DocumentRecord, DocumentListResponse
...
class DocumentDeliveryService:
    ...
    def __init__(self, case_matcher: Optional['CaseMatcher']=None, document_renamer: Optional['DocumentRenamer']=None, notification_service: Optional['SMSNotificationService']=None, auto_login_service: Optional['IAutoLoginService']=None, api_client: Optional['CourtDocumentApiClient']=None, token_service: Optional['DocumentDeliveryTokenService']=None):
        """
        初始化文书送达服务
    ...
    def _query_via_api(self, token: str, cutoff_time: datetime, credential_id: int) -> DocumentQueryResult:
        ...
        try:
            ...
            self._process_document_page(documents=first_response.documents, token=token, cutoff_time=cutoff_time, credential_id=credential_id, result=result)
            for page_num in range(2, total_pages + 1):
                logger.info(f'处理第 {page_num}/{total_pages} 页')
                try:
                    page_response = self.api_client.fetch_document_list(token=token, page_num=page_num, page_size=page_size)
                    self._process_document_page(documents=page_response.documents, token=token, cutoff_time=cutoff_time, credential_id=credential_id, result=result)
            ...
            result.errors.append(error_msg)
            raise
        logger.info(f'API 查询完成: 发现={result.total_found}, 处理={result.processed_count}, 跳过={result.skipped_count}, 失败={result.failed_count}')
        return result

    def _process_document_page(self, documents: List[DocumentRecord], token: str, cutoff_time: datetime, credential_id: int, result: DocumentQueryResult) -> None:
        """
        处理一页文书记录
    ...
    def _process_document_via_api(self, record: DocumentRecord, token: str, credential_id: int) -> DocumentProcessResult:
        """
        通过 API 处理单个文书
        ...
        try:
            ...
            delivery_record = DocumentDeliveryRecord(case_number=record.ah, send_time=send_time, element_index=0, document_name=record.wsmc, court_name=record.fymc)
            process_result = self._process_sms_in_thread(record=delivery_record, file_path=downloaded_files[0], extracted_files=downloaded_files, credential_id=credential_id)
    ...
    def _check_api_document_not_processed(self, credential_id: int, record: DocumentRecord) -> bool:
        ...
        def do_check():
            try:
                ...
                completed_sms = CourtSMS.objects.filter(case_numbers__contains=[record.ah], status=CourtSMSStatus.COMPLETED).first()
                if completed_sms:
                    ...
                    if send_time:
                        ...
                        send_time = timezone.make_aware(send_time)
                    if send_time:
                        existing_history = DocumentQueryHistory.objects.filter(credential_id=credential_id, case_number=record.ah, send_time=send_time).first()
                        if existing_history:
    ...
    def _sync_login_with_page(self, browser_service: Any, credential: Any, page: Any) -> str:
        """
    ...
    def query_and_download(self, credential_id: int, cutoff_time: datetime, tab: str='pending', debug_mode: bool=True) -> DocumentQueryResult:
        """
        查询并下载文书（三级降级策略）
        ...
        return self._query_via_playwright(credential_id=credential_id, cutoff_time=cutoff_time, tab=tab, debug_mode=debug_mode)

    def _try_api_approach(self, credential_id: int, cutoff_time: datetime) -> Optional[DocumentQueryResult]:
        """
        尝试使用 API 方式查询文书
        ...
        try:
            ...
            if not token:
                AutomationLogger.log_fallback_triggered(from_method='api', to_method='playwright', reason='Token 获取失败', credential_id=credential_id)
                return None
            ...
            AutomationLogger.log_document_query_statistics(total_found=result.total_found, processed_count=result.processed_count, skipped_count=result.skipped_count, failed_count=result.failed_count, query_method='api', credential_id=credential_id)
            return result
        except Exception as e:
            error_type = type(e).__name__
            error_msg = str(e)
            AutomationLogger.log_fallback_triggered(from_method='api', to_method='playwright', reason=error_msg, error_type=error_type, credential_id=credential_id)
            AutomationLogger.log_api_error_detail(api_name='document_query_api', error_type=error_type, error_message=error_msg, stack_trace=traceback.format_exc())
            return None
    ...
    def _acquire_token_via_service(self, site_name: str, credential_id: int) -> Optional[str]:
        """
    ...
    def _refresh_token_if_expired(self, credential_id: int, current_token: str) -> Optional[str]:
        """
    ...
    def _try_api_after_login(self, token: str, cutoff_time: datetime, credential_id: int) -> Optional[DocumentQueryResult]:
        """
        ...
        try:
            result = self._query_via_api(token=token, cutoff_time=cutoff_time, credential_id=credential_id)
            AutomationLogger.log_document_query_statistics(total_found=result.total_found, processed_count=result.processed_count, skipped_count=result.skipped_count, failed_count=result.failed_count, query_method='api_after_login', credential_id=credential_id)
            return result
        except Exception as e:
            error_type = type(e).__name__
            error_msg = str(e)
            AutomationLogger.log_fallback_triggered(from_method='api_after_login', to_method='playwright_page', reason=error_msg, error_type=error_type, credential_id=credential_id)
            AutomationLogger.log_api_error_detail(api_name='api_after_login', error_type=error_type, error_message=error_msg, stack_trace=traceback.format_exc())
            return None

    def _query_via_playwright(self, credential_id: int, cutoff_time: datetime, tab: str='pending', debug_mode: bool=True) -> DocumentQueryResult:
        """
        ...
        try:
            ...
            try:
                try:
                    ...
                    return result
                if token:
                    api_result = self._try_api_after_login(token=token, cutoff_time=cutoff_time, credential_id=credential_id)
                    if api_result is not None:
        ...
        def do_process():
            try:
                ...
                if matched_case:
                    ...
                    renamed_files, case_log_id = self._rename_and_attach_documents(sms=sms, case=case, extracted_files=extracted_files)
                    if renamed_files:
