...
class CourtApiError(ExternalServiceError):
    ...
    def __init__(self, message: str = "法院 API 调用错误", code: Optional[str] = None, errors: Optional[dict] = None):
...
class TokenExpiredError(TokenError):
    ...
    def __init__(self, message: str = "Token 已过期", code: Optional[str] = None, errors: Optional[dict] = None):
...
class DocumentRecord:
    ...
    def parse_fssj(self) -> Optional[datetime]:
        """解析 fssj 为 datetime"""
        if not self.fssj:
            return None
        try:
    ...
    def _make_request(
        ...
        for attempt in range(retry_count + 1):
            try:
                ...
                last_error = e
                logger.warning(f"请求超时: {url}, attempt={attempt + 1}, error={str(e)}")
                if attempt < retry_count:
                    continue
                raise NetworkError(message=f"请求超时: {str(e)}", errors={"url": url, "timeout": self._timeout})

            except httpx.RequestError as e:
                last_error = e
                logger.warning(f"网络错误: {url}, attempt={attempt + 1}, error={str(e)}")
                if attempt < retry_count:
                    continue
                raise NetworkError(message=f"网络错误: {str(e)}", errors={"url": url})
                ...
                last_error = e
                logger.error(f"未知错误: {url}, error={str(e)}")
                raise CourtApiError(message=f"API 调用失败: {str(e)}", errors={"url": url})

        # 不应该到达这里
        raise NetworkError(message=f"请求失败: {str(last_error)}", errors={"url": url})
        ...
        try:
            ...
            for doc_data in documents_data:
                try:
                    ...
                    logger.warning(f"解析文书记录失败: {str(e)}, data={doc_data}")
                    continue
        ...
        try:
            ...
            for doc_data in documents_data:
                try:
                    ...
                    logger.warning(f"解析文书详情失败: {str(e)}, data={doc_data}")
                    continue
