...
class ExtractorValidatorsMixin:
    ...
    def _validate_case_number_in_text(self, case_number: str, text: str) -> bool:
        ...
        seq_match = re.search(r"(\d{3,})号?$", case_number)
        ...
        # 检查年份是否在原文中出现(允许不同括号格式)
        year_patterns: list[Any] = [
            f"({year})",
            f"({year})",
            f"〔{year}〕",
            f"[{year}]",
    ...
    def _handle_ollama_only_result(
        self, ollama_datetime, ollama_valid, ollama_validity_score, ollama_validity_reason, validated_regex_results
    ) -> tuple[datetime | None, str]:
        """处理只有 Ollama 结果的情况"""
    ...
    def _cross_validate_results(
        self, best_regex, valid_regex_results, ollama_datetime, ollama_validity_score
    ) -> tuple[datetime | None, str]:
        """交叉校验正则和 Ollama 结果"""
        best_regex_dt = best_regex["datetime"]
        best_regex_combined = best_regex["combined_score"]
