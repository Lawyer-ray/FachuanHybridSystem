...
"""
    ...
    def _select_best_datetime(
        self, regex_results: List[Tuple[datetime, str, int]], ollama_datetime: Optional[datetime]
        ...
        date_diff = abs((best_regex_dt.date() - ollama_datetime.date()).days)  # type: ignore[attr-defined]
        ...
        if date_diff == 0:
            # 日期一致但时间不同
            logger.warning(f"时间冲突: 正则={best_regex_dt}, Ollama={ollama_datetime}, " f"时间差={time_diff}秒")
        ...
        # 日期不一致
        logger.warning(
            f"日期冲突: 正则={best_regex_dt.date()}, Ollama={ollama_datetime.date()}, "  # type: ignore[attr-defined]
            f"差异={date_diff}天"
        )
    ...
    def extract_summons_info(self, text: str) -> Dict[str, Any]:
        ...
        if not text or not text.strip():
            logger.warning(
                "传票内容为空，无法提取信息", extra={"action": "extract_summons_info", "result": "empty_text"}
            )
    ...
    def extract_execution_info(self, text: str) -> Dict[str, Any]:
        ...
        if not text or not text.strip():
            logger.warning(
                "执行裁定书内容为空，无法提取信息", extra={"action": "extract_execution_info", "result": "empty_text"}
            )
