...
class FeishuChatProvider(ChatProvider):
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            raise ConfigurationException(
                message=f"无法加载飞书配置: {str(e)}", platform="feishu", errors={"original_error": str(e)}
            ) from e
    ...
    def _get_tenant_access_token(self) -> str:
        ...
        try:
            ...
            raise ChatProviderException(
                message=f"网络请求失败: {str(e)}", platform="feishu", errors={"original_error": str(e)}
            ) from e
        except (KeyError, ValueError) as e:
            logger.error(f"解析飞书API响应失败: {str(e)}")
            raise ChatProviderException(
                message=f"API响应格式错误: {str(e)}", platform="feishu", errors={"original_error": str(e)}
            ) from e
