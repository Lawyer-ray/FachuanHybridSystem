...
class PreservationQuoteServiceAdapter(IPreservationQuoteService):
    ...
    def __init__(self, token_service: Optional[TokenService]=None, insurance_client: Optional[CourtInsuranceClient]=None, auto_token_service: Optional[IAutoTokenAcquisitionService]=None):
        """
        初始化服务适配器
    ...
    def service(self) -> PreservationQuoteService:
        ...
        if self._service is None:
            self._service = EnhancedPreservationQuoteService(token_service=self.token_service, insurance_client=self.insurance_client, auto_token_service=self.auto_token_service)
        return self._service

    def create_quote(self, case_name: str, target_amount: Any, applicant_name: str, respondent_name: str, court_name: str, case_type: str='财产保全', **kwargs) -> Any:
        """
        创建询价任务
    ...
    def list_quotes(self, status: Optional[str]=None, limit: int=20, offset: int=0, page: Optional[int]=None, page_size: Optional[int]=None) -> Dict[str, Any]:
        """
        获取询价记录列表
    ...
    def create_quote_internal(self, case_name: str, target_amount: Any, applicant_name: str, respondent_name: str, court_name: str, case_type: str='财产保全', **kwargs) -> Any:
        """创建询价任务（内部接口，无权限检查）"""
    ...
    def list_quotes_internal(self, status: Optional[str]=None, limit: int=20, offset: int=0, page: Optional[int]=None, page_size: Optional[int]=None) -> Dict[str, Any]:
        """获取询价记录列表（内部接口，无权限检查）"""
...
class EnhancedPreservationQuoteService(PreservationQuoteService):
    ...
    def __init__(self, token_service: TokenService, insurance_client: CourtInsuranceClient, auto_token_service: IAutoTokenAcquisitionService):
        """
        初始化增强版服务
        ...
        """
        super().__init__(token_service=token_service, auto_token_service=auto_token_service, insurance_client=insurance_client)
    ...
    async def _get_valid_token(self, credential_id: int=None) -> str:
        ...
        logger.info('开始获取Token（增强版）', extra={'action': 'get_valid_token_enhanced_start', 'credential_id': credential_id, 'method': 'auto_acquisition_integrated'})
        ...
        try:
            ...
            logger.info('✅ Token获取成功（增强版）', extra={'action': 'get_valid_token_enhanced_success', 'site_name': site_name, 'credential_id': credential_id, 'token_length': len(token) if token else 0})
            return token
        except Exception as e:
            logger.error(f'❌ Token获取失败（增强版）: {e}', extra={'action': 'get_valid_token_enhanced_failed', 'site_name': site_name, 'credential_id': credential_id, 'error_type': type(e).__name__, 'error_message': str(e)}, exc_info=True)
