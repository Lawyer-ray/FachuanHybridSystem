...
class AutoTokenAcquisitionService:
    ...
    async def acquire_token_if_needed(self, site_name: str, credential_id: Optional[int] = None) -> str:
        """
        如果需要则自动获取token
        ...
        # 记录性能监控开始
        performance_monitor.record_acquisition_start(acquisition_id, site_name, credential_id or "auto")

        try:
            # 并发控制和资源获取
            await concurrency_optimizer.acquire_resource(acquisition_id, site_name, credential_id or "auto")
            ...
            try:
                ...
                if credential_id:
                    ...
                    if credential:
                        ...
                        # 没有找到可用账号
                        logger.error(
                            f"没有找到可用账号", extra={"acquisition_id": acquisition_id, "site_name": site_name}
                        )
                ...
                # 释放并发资源
                await concurrency_optimizer.release_resource(acquisition_id, site_name, credential_id or "auto")
    ...
    async def _check_any_valid_token(self, site_name: str) -> Optional[str]:
        ...
        try:
            ...
            if token:
                logger.info(f"找到有效Token", extra={"site_name": site_name, "account": available_accounts.account})
                return token
        ...
        try:
            ...
            try:
                ...
                await self.token_service.save_token_internal(
                    site_name=site_name, account=credential.account, token=token, expires_in=3600  # 默认1小时过期
                )
                ...
                logger.info(
                    f"AutoLoginService超时，检查Token是否已保存",
                    extra={"acquisition_id": acquisition_id, "site_name": site_name, "account": credential.account},
                )
                ...
                if saved_token:
                    logger.info(
                        f"✅ AutoLoginService超时但Token已保存成功",
                        extra={"acquisition_id": acquisition_id, "site_name": site_name, "account": credential.account},
                    )
            ...
            if isinstance(
                ...
                return TokenAcquisitionResult(
                    ...
                    total_duration=total_duration,
                    login_attempts=login_attempts,
                    error_details={"message": f"未预期错误: {str(e)}", "error_type": "UnexpectedError"},
                )
