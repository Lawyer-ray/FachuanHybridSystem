...
class TokenCacheManager:
    ...
    def get_cached_token(self, site_name: str, account: str) -> Optional[str]:
        ...
        try:
            ...
            if cached_data:
                logger.debug(
                    f"Token缓存命中", extra={"site_name": site_name, "account": account, "cache_key": cache_key}
                )
                return cached_data.get("token")

            logger.debug(f"Token缓存未命中", extra={"site_name": site_name, "account": account, "cache_key": cache_key})
            return None
    ...
    def cache_token(self, site_name: str, account: str, token: str, expires_at: Optional[datetime] = None) -> None:
        """
        缓存Token
    ...
    def invalidate_token_cache(self, site_name: str, account: str) -> None:
        ...
        try:
            ...
            logger.info(f"Token缓存已失效", extra={"site_name": site_name, "account": account, "cache_key": cache_key})
        except Exception as e:
    ...
    def get_cached_credentials(self, site_name: str) -> Optional[List[AccountCredentialDTO]]:
        ...
        try:
            ...
            logger.debug(f"账号凭证缓存未命中", extra={"site_name": site_name, "cache_key": cache_key})
            return None
    ...
    def cache_credentials(self, site_name: str, credentials: List[AccountCredentialDTO]) -> None:
        ...
        try:
            ...
            logger.info(
                f"账号凭证已缓存", extra={"site_name": site_name, "cache_key": cache_key, "count": len(credentials)}
            )
        except Exception as e:
    ...
    def invalidate_credentials_cache(self, site_name: str) -> None:
        ...
        try:
            cache.delete(cache_key)
            logger.info(f"账号凭证缓存已失效", extra={"site_name": site_name, "cache_key": cache_key})
        except Exception as e:
    ...
    def get_cached_account_stats(self, account: str, site_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if cached_data:
                logger.debug(
                    f"账号统计缓存命中", extra={"account": account, "site_name": site_name, "cache_key": cache_key}
                )
                return cached_data
    ...
    def cache_account_stats(self, account: str, site_name: str, stats: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.debug(f"账号统计已缓存", extra={"account": account, "site_name": site_name, "cache_key": cache_key})
        except Exception as e:
    ...
    def invalidate_account_stats_cache(self, account: str, site_name: str) -> None:
        ...
        try:
            cache.delete(cache_key)
            logger.debug(
                f"账号统计缓存已失效", extra={"account": account, "site_name": site_name, "cache_key": cache_key}
            )
        except Exception as e:
    ...
    def get_cached_blacklist(self) -> Optional[List[str]]:
        ...
        try:
            ...
            if cached_data:
                logger.debug(f"黑名单缓存命中", extra={"cache_key": cache_key, "count": len(cached_data)})
                return cached_data
    ...
    def cache_blacklist(self, blacklist: List[str]) -> None:
        ...
        try:
            ...
            logger.debug(f"黑名单已缓存", extra={"cache_key": cache_key, "count": len(blacklist)})
        except Exception as e:
