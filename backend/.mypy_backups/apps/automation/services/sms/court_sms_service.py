...
class CourtSMSService:
    ...
    def __init__(self, parser: SMSParserService=None, matcher: CaseMatcher=None, case_number_extractor: Optional['CaseNumberExtractorService']=None, document_attachment: Optional['DocumentAttachmentService']=None, notification: Optional['SMSNotificationService']=None, case_service: Optional['ICaseService']=None, client_service: Optional['IClientService']=None, lawyer_service: Optional['ILawyerService']=None, case_chat_service: Optional['ICaseChatService']=None):
        """
        初始化服务，支持依赖注入
    ...
    def submit_sms(self, content: str, received_at: datetime=None) -> CourtSMS:
        ...
        if not content or not content.strip():
            raise ValidationException(message='短信内容不能为空', code='EMPTY_SMS_CONTENT', errors={'content': '短信内容不能为空'})
        if received_at is None:
            received_at = timezone.now()
        try:
            sms = CourtSMS.objects.create(content=content.strip(), received_at=received_at, status=CourtSMSStatus.PENDING)
            logger.info(f'创建短信记录成功: ID={sms.id}, 长度={len(content)}')
            task_id = async_task('apps.automation.services.sms.court_sms_service.process_sms_async', sms.id, task_name=f'court_sms_processing_{sms.id}')
            logger.info(f'提交异步处理任务: SMS ID={sms.id}, Task ID={task_id}')
            return sms
        except Exception as e:
            logger.error(f'提交短信处理失败: {str(e)}')
            raise ValidationException(message=f'提交短信处理失败: {str(e)}', code='SMS_SUBMIT_FAILED', errors={'error': str(e)})
    ...
    def assign_case(self, sms_id: int, case_id: int) -> CourtSMS:
        ...
        try:
            ...
            if success:
                ...
                return sms
            task_id = async_task('apps.automation.services.sms.court_sms_service.process_sms_from_renaming', sms.id, task_name=f'court_sms_continue_{sms.id}')
            logger.info(f'触发后续处理任务: SMS ID={sms.id}, Task ID={task_id}')
            return sms
        except Exception as e:
            logger.error(f'手动指定案件失败: SMS ID={sms_id}, Case ID={case_id}, 错误: {str(e)}')
            raise ValidationException(message=f'手动指定案件失败: {str(e)}', code='CASE_ASSIGNMENT_FAILED', errors={'error': str(e)})
    ...
    def retry_processing(self, sms_id: int) -> CourtSMS:
        ...
        try:
            ...
            task_id = async_task('apps.automation.services.sms.court_sms_service.process_sms_async', sms.id, task_name=f'court_sms_retry_{sms.id}_{sms.retry_count}')
            logger.info(f'重新提交处理任务: SMS ID={sms.id}, Task ID={task_id}')
            return sms
        except Exception as e:
            logger.error(f'重新处理短信失败: SMS ID={sms_id}, 错误: {str(e)}')
            raise ValidationException(message=f'重新处理短信失败: {str(e)}', code='SMS_RETRY_FAILED', errors={'error': str(e)})
    ...
    def process_sms(self, sms_id: int) -> CourtSMS:
        ...
        try:
            ...
            sms.save()
            raise ValidationException(message=f'处理短信失败: {str(e)}', code='SMS_PROCESSING_FAILED', errors={'sms_id': sms_id, 'error': str(e)})
    ...
    def _extract_and_update_sms_from_documents(self, sms: CourtSMS) -> None:
        ...
        if has_updates:
            ...
            sms.save()
            logger.info(f'已更新短信记录的案号和当事人: SMS ID={sms.id}, 案号={unique_case_numbers}, 当事人={unique_party_names}')
        ...
        try:
            ...
            if sms.case and renamed_paths:
                ...
                if case_numbers_to_sync:
                    success_count = self.case_number_extractor.sync_to_case(case_id=sms.case.id, case_numbers=case_numbers_to_sync, sms_id=sms.id)
    ...
    def _create_download_task(self, sms: CourtSMS) -> Optional[ScraperTask]:
        ...
        try:
            ...
            task = ScraperTask.objects.create(task_type=ScraperTaskType.COURT_DOCUMENT, url=download_url, case=sms.case, config={'court_sms_id': sms.id, 'auto_download': True, 'source': 'court_sms'})
            logger.info(f'创建下载任务成功: Task ID={task.id}, URL={download_url}')
            queue_task_id = async_task('apps.automation.tasks.execute_scraper_task', task.id, task_name=f'court_document_download_{task.id}')
    ...
    def _should_wait_for_document_download(self, sms: CourtSMS) -> bool:
        ...
        try:
            ...
            logger.info(f'短信 {sms.id} 文书状态统计: 总数={all_documents.count()}, 成功={successful_documents.count()}, 失败={failed_documents.count()}, 待下载={pending_documents.count()}, 下载中={downloading_documents.count()}')
            ...
            if sms.scraper_task.status in [ScraperTaskStatus.SUCCESS, ScraperTaskStatus.FAILED]:
                logger.info(f'短信 {sms.id} 的下载任务已完成（状态: {sms.scraper_task.status}），不再等待')
                return False
            if sms.scraper_task.status == ScraperTaskStatus.RUNNING and all_documents.count() > 0 and (successful_documents.count() == 0) and (pending_documents.count() == 0) and (downloading_documents.count() == 0):
                logger.info(f'短信 {sms.id} 的下载任务运行中但所有文书都已失败，不再等待')
                return False
            if pending_documents.exists() or downloading_documents.exists() or sms.scraper_task.status in [ScraperTaskStatus.PENDING, ScraperTaskStatus.RUNNING]:
    ...
    def _create_case_binding(self, sms: CourtSMS) -> bool:
        ...
        try:
            ...
            case_log = case_log_service.create_log(case_id=sms.case.id, content=f'收到法院短信：{sms.content}', user=system_user)
    ...
    def _add_case_numbers_to_case(self, sms: CourtSMS) -> None:
        ...
        try:
            ...
            for case_number in valid_case_numbers:
                success = self.case_service.add_case_number_internal(case_id=sms.case.id, case_number=case_number, user_id=user_id)
                if success:
                    added_count += 1
            if added_count > 0:
