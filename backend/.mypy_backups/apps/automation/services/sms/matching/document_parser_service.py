...
class DocumentParserService:
    ...
    def extract_parties_from_document(self, document_path: str) -> List[str]:
        ...
        try:
            ...
            result = doc_service.extract_document_content_by_path_internal(document_path, limit=3000)
            ...
            logger.info(f"从文书中提取到 {len(result['text'])} 字符的内容，" f"删除换行符后为 {len(content)} 字符")

            # 在现有客户数据库中匹配当事人（不使用 Ollama）
            return self.match_parties_from_content(content)

        except Exception as e:
            logger.error(f"从文书提取当事人失败: {str(e)}")
            return []
    ...
    def match_parties_from_content(self, content: str) -> List[str]:
        ...
        try:
            ...
            if matched_parties:
                logger.info(f"从文书内容中匹配到 {len(matched_parties)} 个当事人: " f"{matched_parties}")
            else:
