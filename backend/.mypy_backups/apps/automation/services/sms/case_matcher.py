...
class CaseMatcher:
        ...
        try:
            ...
            if party_names:
                ...
                if len(matched_cases) == 1:
                    ...
                    logger.info(f"当事人匹配到多个案件（共{len(matched_cases)}个），" f"选择最新的: {case.name}")
                    return case
            ...
            raise ValidationException(
                message=f"案件匹配失败: {str(e)}", code="CASE_MATCH_FAILED", errors={"error": str(e)}
            )
    ...
    def _narrow_down_by_case_number_features(self, cases: List, case_numbers: List[str]) -> Optional:
        """
        第三优先级：通过案号特征缩小案件范围
