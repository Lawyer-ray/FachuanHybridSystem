...
def recognize_captcha(request, payload: CaptchaRecognizeIn):
    ...
    **失败响应示例**:
    ...
    # 记录请求结果（不记录图片数据）
    if result.success:
        logger.info(f"验证码识别成功: text={result.text}, " f"processing_time={result.processing_time:.3f}s")
    else:
        logger.warning(f"验证码识别失败: error={result.error}, " f"processing_time={result.processing_time:.3f}s")
