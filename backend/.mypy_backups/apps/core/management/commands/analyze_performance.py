...
class Command(BaseCommand):
    ...
    def _parse_logs(self, log_file: str, hours: int) -> dict:
        ...
        Returns:
            API 统计数据
        """
        api_stats = defaultdict(lambda: {'count': 0, 'total_time': 0, 'total_queries': 0, 'max_time': 0, 'max_queries': 0, 'errors': 0})
    ...
    def _analyze_performance(self, api_stats: dict, threshold: int, top_n: int) -> None:
        """
        分析性能数据
        ...
        for i, (endpoint, stats) in enumerate(sorted_by_time[:top_n], 1):
            self.stdout.write(f'{i}. {endpoint}\n   调用次数: {stats['count']}\n   平均响应时间: {stats['avg_time']:.2f}ms\n   最大响应时间: {stats['max_time']:.2f}ms\n   平均查询次数: {stats['avg_queries']:.2f}\n   错误次数: {stats['errors']}\n')
        self.stdout.write(self.style.SUCCESS(f'\n=== 查询次数最多的 {top_n} 个 API ===\n'))
        for i, (endpoint, stats) in enumerate(sorted_by_queries[:top_n], 1):
            if stats['avg_queries'] > 0:
                self.stdout.write(f'{i}. {endpoint}\n   调用次数: {stats['count']}\n   平均查询次数: {stats['avg_queries']:.2f}\n   最大查询次数: {stats['max_queries']}\n   平均响应时间: {stats['avg_time']:.2f}ms\n')
        ...
        if slow_apis:
            ...
            for endpoint, stats in slow_apis:
                self.stdout.write(f'⚠️  {endpoint}\n   平均响应时间: {stats['avg_time']:.2f}ms\n   调用次数: {stats['count']}\n')
