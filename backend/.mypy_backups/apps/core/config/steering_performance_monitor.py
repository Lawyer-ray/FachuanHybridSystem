...
class PerformanceDataCollector:
    ...
    def record_loading_end(self, load_id: str, spec_path: str, success: bool, cache_hit: bool=False, error_message: Optional[str]=None, file_size_bytes: int=0) -> LoadingPerformanceData:
        """记录加载结束"""
        with self._lock:
            ...
            perf_data = LoadingPerformanceData(spec_path=spec_path, start_time=start_time, end_time=end_time, duration_ms=duration_ms, success=success, error_message=error_message, cache_hit=cache_hit, file_size_bytes=file_size_bytes, memory_usage_mb=memory_usage_mb)
    ...
    def record_alert(self, alert: PerformanceAlert) -> None:
        ...
        with self._lock:
            ...
            log_level = {AlertLevel.INFO: logging.INFO, AlertLevel.WARNING: logging.WARNING, AlertLevel.ERROR: logging.ERROR, AlertLevel.CRITICAL: logging.CRITICAL}.get(alert.level, logging.INFO)
    ...
    def get_loading_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {'current_concurrent_loads': self._current_loads, 'total_loads': self._total_loads, 'successful_loads': self._successful_loads, 'failed_loads': self._failed_loads, 'success_rate': success_rate, 'cache_hits': self._cache_hits, 'cache_misses': self._cache_misses, 'cache_hit_rate': cache_hit_rate, 'load_time_stats': {'avg_ms': avg_load_time, 'min_ms': min_load_time, 'max_ms': max_load_time, 'p50_ms': p50_load_time, 'p95_ms': p95_load_time, 'p99_ms': p99_load_time}, 'memory_usage_mb': self._get_current_memory_usage()}
...
class PerformanceAnalyzer:
    ...
    def __init__(self, data_collector: PerformanceDataCollector, thresholds: PerformanceThresholds):
    ...
    def analyze_loading_performance(self, spec_path: Optional[str]=None) -> Dict[str, Any]:
        ...
        with self._cache_lock:
            ...
            analysis = {'total_loads': len(history), 'success_rate': sum((1 for h in history if h.success)) / len(history), 'cache_hit_rate': sum((1 for h in history if h.cache_hit)) / len(history), 'performance_trend': self._analyze_performance_trend(history), 'slow_specifications': self._identify_slow_specifications(history), 'error_patterns': self._analyze_error_patterns(history), 'recommendations': self._generate_recommendations(history)}
    ...
    def _analyze_performance_trend(self, history: List[LoadingPerformanceData]) -> Dict[str, Any]:
        ...
        if trend_change > 20:
            ...
            trend = 'stable'
        return {'trend': trend, 'trend_change_percent': trend_change, 'first_half_avg_ms': first_half_avg, 'second_half_avg_ms': second_half_avg}
    ...
    def _identify_slow_specifications(self, history: List[LoadingPerformanceData]) -> List[Dict[str, Any]]:
        ...
        for spec_path, durations in spec_stats.items():
            ...
            if avg_duration > self.thresholds.load_time_warning_ms:
                slow_specs.append({'spec_path': spec_path, 'avg_duration_ms': avg_duration, 'max_duration_ms': max_duration, 'load_count': len(durations), 'severity': self._get_severity_level(avg_duration)})
        slow_specs.sort(key=lambda x: x['avg_duration_ms'], reverse=True)
        return slow_specs[:10]
    ...
    def _analyze_error_patterns(self, history: List[LoadingPerformanceData]) -> Dict[str, Any]:
        ...
        for error_msg, occurrences in error_groups.items():
            error_frequency[error_msg] = {'count': len(occurrences), 'percentage': len(occurrences) / len(failed_loads) * 100, 'affected_specs': list(set((h.spec_path for h in occurrences)))}
        return {'total_errors': len(failed_loads), 'error_rate': len(failed_loads) / len(history) * 100, 'error_frequency': error_frequency}
    ...
    def _generate_recommendations(self, history: List[LoadingPerformanceData]) -> List[str]:
        ...
        large_files = [h for h in history if h.file_size_bytes > 100 * 1024]
        if large_files:
...
class SteeringPerformanceMonitor:
    ...
    def monitor_cached_loading(self, spec_path: str, loading_func: Callable, cache_hit: bool) -> Any:
        """监控缓存加载"""
        if not self.enabled:
            return loading_func()
        load_id = self.data_collector.record_loading_start(spec_path)
        try:
            ...
            try:
                ...
                pass
            perf_data = self.data_collector.record_loading_end(load_id, spec_path, success=True, cache_hit=cache_hit, file_size_bytes=file_size)
            self._check_performance_thresholds(perf_data)
            return result
        except Exception as e:
            self.data_collector.record_loading_end(load_id, spec_path, success=False, cache_hit=cache_hit, error_message=str(e))
            raise
    ...
    def _check_performance_thresholds(self, perf_data: LoadingPerformanceData) -> None:
        ...
        if duration_ms > self.thresholds.load_time_critical_ms:
            alert = PerformanceAlert(level=AlertLevel.CRITICAL, message=f'规范加载时间严重超标: {perf_data.spec_path} ({duration_ms:.1f}ms)', metric_name='load_time_ms', threshold=self.thresholds.load_time_critical_ms, actual_value=duration_ms, timestamp=time.time(), metadata={'spec_path': perf_data.spec_path})
            self._trigger_alert(alert)
        elif duration_ms > self.thresholds.load_time_error_ms:
            alert = PerformanceAlert(level=AlertLevel.ERROR, message=f'规范加载时间超标: {perf_data.spec_path} ({duration_ms:.1f}ms)', metric_name='load_time_ms', threshold=self.thresholds.load_time_error_ms, actual_value=duration_ms, timestamp=time.time(), metadata={'spec_path': perf_data.spec_path})
            self._trigger_alert(alert)
        elif duration_ms > self.thresholds.load_time_warning_ms:
            alert = PerformanceAlert(level=AlertLevel.WARNING, message=f'规范加载时间较长: {perf_data.spec_path} ({duration_ms:.1f}ms)', metric_name='load_time_ms', threshold=self.thresholds.load_time_warning_ms, actual_value=duration_ms, timestamp=time.time(), metadata={'spec_path': perf_data.spec_path})
            self._trigger_alert(alert)
        memory_mb = perf_data.memory_usage_mb
        if memory_mb > self.thresholds.memory_usage_critical_mb:
            alert = PerformanceAlert(level=AlertLevel.CRITICAL, message=f'内存使用严重超标: {memory_mb:.1f}MB', metric_name='memory_usage_mb', threshold=self.thresholds.memory_usage_critical_mb, actual_value=memory_mb, timestamp=time.time())
            self._trigger_alert(alert)
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        return {'enabled': True, 'statistics': stats, 'analysis': analysis, 'recent_alerts': [{'level': alert.level.value, 'message': alert.message, 'timestamp': alert.timestamp} for alert in recent_alerts], 'thresholds': {'load_time_warning_ms': self.thresholds.load_time_warning_ms, 'load_time_error_ms': self.thresholds.load_time_error_ms, 'memory_usage_warning_mb': self.thresholds.memory_usage_warning_mb, 'cache_hit_rate_warning': self.thresholds.cache_hit_rate_warning}}
            ...
            while True:
                try:
                    ...
                    if cache_hit_rate < self.thresholds.cache_hit_rate_error:
                        alert = PerformanceAlert(level=AlertLevel.ERROR, message=f'缓存命中率过低: {cache_hit_rate:.1%}', metric_name='cache_hit_rate', threshold=self.thresholds.cache_hit_rate_error, actual_value=cache_hit_rate, timestamp=time.time())
                        self._trigger_alert(alert)
                    elif cache_hit_rate < self.thresholds.cache_hit_rate_warning:
                        alert = PerformanceAlert(level=AlertLevel.WARNING, message=f'缓存命中率较低: {cache_hit_rate:.1%}', metric_name='cache_hit_rate', threshold=self.thresholds.cache_hit_rate_warning, actual_value=cache_hit_rate, timestamp=time.time())
                        self._trigger_alert(alert)
                    concurrent_loads = stats.get('current_concurrent_loads', 0)
                    if concurrent_loads > self.thresholds.concurrent_loads_error:
                        alert = PerformanceAlert(level=AlertLevel.ERROR, message=f'并发加载数过高: {concurrent_loads}', metric_name='concurrent_loads', threshold=self.thresholds.concurrent_loads_error, actual_value=concurrent_loads, timestamp=time.time())
                        self._trigger_alert(alert)
                except Exception as e:
