...
class CompatibleSettings:
    ...
    def _build_django_mapping(self) -> Dict[str, str]:
        ...
        Returns:
            Dict[str, str]: 映射字典
        """
        return {'SECRET_KEY': 'django.secret_key', 'DEBUG': 'django.debug', 'ALLOWED_HOSTS': 'django.allowed_hosts', 'INSTALLED_APPS': 'django.installed_apps', 'MIDDLEWARE': 'django.middleware', 'ROOT_URLCONF': 'django.root_urlconf', 'TEMPLATES': 'django.templates', 'WSGI_APPLICATION': 'django.wsgi_application', 'DATABASES': 'database', 'LANGUAGE_CODE': 'django.language_code', 'TIME_ZONE': 'django.time_zone', 'USE_I18N': 'django.use_i18n', 'USE_TZ': 'django.use_tz', 'STATIC_URL': 'django.static_url', 'STATIC_ROOT': 'django.static_root', 'MEDIA_URL': 'django.media_url', 'MEDIA_ROOT': 'django.media_root', 'AUTH_USER_MODEL': 'django.auth_user_model', 'AUTH_PASSWORD_VALIDATORS': 'django.auth_password_validators', 'CORS_ALLOW_ALL_ORIGINS': 'cors.allow_all_origins', 'CORS_ALLOWED_ORIGINS': 'cors.allowed_origins', 'CORS_ALLOW_CREDENTIALS': 'cors.allow_credentials', 'CORS_ALLOW_HEADERS': 'cors.allow_headers', 'CSRF_TRUSTED_ORIGINS': 'cors.csrf_trusted_origins', 'CACHES': 'performance.cache', 'LOGGING': 'logging', 'MOONSHOT_BASE_URL': 'services.moonshot.base_url', 'MOONSHOT_API_KEY': 'services.moonshot.api_key', 'OLLAMA': 'services.ollama', 'FEISHU': 'chat_platforms.feishu', 'DINGTALK': 'chat_platforms.dingtalk', 'WECHAT_WORK': 'chat_platforms.wechat_work', 'TELEGRAM': 'chat_platforms.telegram', 'SLACK': 'chat_platforms.slack', 'CASE_CHAT': 'features.case_chat', 'COURT_SMS_PROCESSING': 'features.court_sms', 'DOCUMENT_PROCESSING': 'features.document_processing', 'Q_CLUSTER': 'performance.q_cluster', 'RATE_LIMIT': 'performance.rate_limit', 'SCRAPER_ENCRYPTION_KEY': 'security.scraper_encryption_key', 'PERM_OPEN_ACCESS': 'security.perm_open_access', 'API_VERSION': 'api.version'}
    ...
    def __setattr__(self, name: str, value: Any) -> None:
        ...
        Args:
            ...
            value: 配置值
        """
        if name.startswith('_') or name in ['_config_manager', '_django_to_config_mapping', '_config_to_django_mapping', '_fallback_to_django', '_django_settings_cache']:
            super().__setattr__(name, value)
            return
        if name in self._django_to_config_mapping:
            config_key = self._django_to_config_mapping[name]
            self._config_manager.set(config_key, value)
        elif self._fallback_to_django:
    ...
    def get_all_settings(self) -> Dict[str, Any]:
        ...
        if self._fallback_to_django:
            for attr_name in dir(django_settings):
                if not attr_name.startswith('_') and attr_name.isupper():
                    if attr_name not in settings_dict:
                        try:
