...
class SteeringConfigProvider:
    ...
    def _get_default_loading_rules(self) -> List[SteeringLoadingRule]:
        ...
        return [
            SteeringLoadingRule(
                pattern="core/*.md", condition="always", priority=100, cache_ttl=7200, performance_threshold_ms=50.0
            ),
            SteeringLoadingRule(
...
class SteeringPerformanceMonitor:
    ...
    def _check_performance_thresholds(
        self, spec_path: str, load_time_ms: float, perf_config: SteeringPerformanceConfig
    ) -> None:
        """检查性能阈值"""
        if load_time_ms > perf_config.error_threshold_ms:
            logger.error(
                f"规范加载性能严重超标 {spec_path}: {load_time_ms:.2f}ms (阈值: {perf_config.error_threshold_ms}ms)"
            )
        elif load_time_ms > perf_config.warn_threshold_ms:
            logger.warning(
                f"规范加载性能超标 {spec_path}: {load_time_ms:.2f}ms (阈值: {perf_config.warn_threshold_ms}ms)"
            )
