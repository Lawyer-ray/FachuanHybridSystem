...
class SteeringConfigProvider:
    ...
    def get_loading_rules(self) -> List[SteeringLoadingRule]:
        ...
        with self._lock:
            ...
            for rule_config in rules_config:
                rule = SteeringLoadingRule(pattern=rule_config.get('pattern', ''), condition=rule_config.get('condition', 'manual'), priority=rule_config.get('priority', 0), cache_ttl=rule_config.get('cache_ttl', 3600), dependencies=rule_config.get('dependencies', []), performance_threshold_ms=rule_config.get('performance_threshold_ms', 100.0))
                rules.append(rule)
            if not rules:
    ...
    def get_cache_config(self) -> SteeringCacheConfig:
        ...
        with self._lock:
            ...
            if cache_key in self._cache:
                return self._cache[cache_key]
            config = SteeringCacheConfig(enabled=self.config_manager.get('steering.cache.enabled', True), ttl_seconds=self.config_manager.get('steering.cache.ttl_seconds', 3600), max_entries=self.config_manager.get('steering.cache.max_entries', 1000), memory_limit_mb=self.config_manager.get('steering.cache.memory_limit_mb', 100), auto_cleanup=self.config_manager.get('steering.cache.auto_cleanup', True), cleanup_interval=self.config_manager.get('steering.cache.cleanup_interval', 300))
    ...
    def get_performance_config(self) -> SteeringPerformanceConfig:
        ...
        with self._lock:
            ...
            if cache_key in self._cache:
                return self._cache[cache_key]
            config = SteeringPerformanceConfig(load_threshold_ms=self.config_manager.get('steering.performance.load_threshold_ms', 100.0), warn_threshold_ms=self.config_manager.get('steering.performance.warn_threshold_ms', 500.0), error_threshold_ms=self.config_manager.get('steering.performance.error_threshold_ms', 2000.0), enable_monitoring=self.config_manager.get('steering.performance.enable_monitoring', True), enable_profiling=self.config_manager.get('steering.performance.enable_profiling', False), max_concurrent_loads=self.config_manager.get('steering.performance.max_concurrent_loads', 4))
            self._cache[cache_key] = config
            return config
    ...
    def get_dependency_config(self) -> SteeringDependencyConfig:
        ...
        with self._lock:
            ...
            if cache_key in self._cache:
                return self._cache[cache_key]
            config = SteeringDependencyConfig(auto_resolve=self.config_manager.get('steering.dependencies.auto_resolve', True), max_depth=self.config_manager.get('steering.dependencies.max_depth', 10), circular_detection=self.config_manager.get('steering.dependencies.circular_detection', True), load_order_strategy=self.config_manager.get('steering.dependencies.load_order_strategy', 'priority'))
            self._cache[cache_key] = config
            return config

    def _get_default_loading_rules(self) -> List[SteeringLoadingRule]:
        """获取默认加载规则"""
        return [SteeringLoadingRule(pattern='core/*.md', condition='always', priority=100, cache_ttl=7200, performance_threshold_ms=50.0), SteeringLoadingRule(pattern='layers/api-layer.md', condition='fileMatch', priority=80, cache_ttl=3600, performance_threshold_ms=100.0), SteeringLoadingRule(pattern='layers/service-layer.md', condition='fileMatch', priority=80, cache_ttl=3600, performance_threshold_ms=100.0), SteeringLoadingRule(pattern='layers/admin-layer.md', condition='fileMatch', priority=80, cache_ttl=3600, performance_threshold_ms=100.0), SteeringLoadingRule(pattern='layers/model-layer.md', condition='fileMatch', priority=80, cache_ttl=3600, performance_threshold_ms=100.0), SteeringLoadingRule(pattern='modules/*.md', condition='fileMatch', priority=60, cache_ttl=3600, performance_threshold_ms=150.0)]
...
class SteeringCacheManager:
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        """获取缓存统计信息"""
        with self._lock:
            return {'cache_size': len(self._cache), 'memory_usage_estimate': self._estimate_memory_usage(), 'oldest_entry': min(self._access_times.values()) if self._access_times else None, 'newest_entry': max(self._access_times.values()) if self._access_times else None}
...
class SteeringPerformanceMonitor:
    ...
    def _record_metric(self, spec_path: str, load_time_ms: float, success: bool) -> None:
        ...
        with self._lock:
            if spec_path not in self._metrics:
                self._metrics[spec_path] = {'total_loads': 0, 'successful_loads': 0, 'failed_loads': 0, 'total_time_ms': 0.0, 'min_time_ms': float('inf'), 'max_time_ms': 0.0, 'avg_time_ms': 0.0}
    ...
    def _check_performance_thresholds(self, spec_path: str, load_time_ms: float, perf_config: SteeringPerformanceConfig) -> None:
        """检查性能阈值"""
        if load_time_ms > perf_config.error_threshold_ms:
            logger.error(f'规范加载性能严重超标 {spec_path}: {load_time_ms:.2f}ms (阈值: {perf_config.error_threshold_ms}ms)')
        elif load_time_ms > perf_config.warn_threshold_ms:
            logger.warning(f'规范加载性能超标 {spec_path}: {load_time_ms:.2f}ms (阈值: {perf_config.warn_threshold_ms}ms)')
        elif load_time_ms > perf_config.load_threshold_ms:
            logger.info(f'规范加载较慢 {spec_path}: {load_time_ms:.2f}ms (阈值: {perf_config.load_threshold_ms}ms)')

    def get_performance_stats(self) -> Dict[str, Any]:
        """获取性能统计信息"""
        with self._lock:
            return {'total_specifications': len(self._metrics), 'metrics': dict(self._metrics)}
...
class SteeringIntegrationManager:
        ...
        for spec_path in load_order_result.ordered_specs:
            try:
                ...
                if cached_spec is not None:
                    spec = self.performance_monitor.monitor_cached_loading(spec_path, lambda: cached_spec, cache_hit=True)
                else:
                    spec = self.performance_monitor.monitor_cached_loading(spec_path, lambda: self._load_specification_file(spec_path), cache_hit=False)
    ...
    def get_integration_stats(self) -> Dict[str, Any]:
        """获取集成统计信息"""
        return {'cache_stats': self.cache_manager.get_cache_stats(), 'cache_strategy_stats': self.cache_strategy_manager.get_stats(), 'performance_report': self.performance_monitor.get_performance_report(), 'dependency_stats': self.dependency_manager.get_statistics(), 'config_provider_cache_size': len(self.config_provider._cache)}

    def export_integration_report(self, output_path: str) -> None:
        """导出集成报告"""
        try:
            report = {'timestamp': time.time(), 'integration_stats': self.get_integration_stats(), 'configuration': {'cache_strategy': self.cache_strategy_manager.strategy_type.value, 'performance_monitoring': self.performance_monitor.enabled, 'dependency_auto_resolve': self.dependency_manager.auto_resolve}}
            import json
