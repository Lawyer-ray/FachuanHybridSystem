...
def create_config_registry() -> Dict[str, ConfigField]:
    ...
    registry["django.debug"] = ConfigField(
        name="django.debug", type=bool, default=False, env_var="DEBUG", description="调试模式开关，生产环境必须为 False"
    )

    registry["django.allowed_hosts"] = ConfigField(
        name="django.allowed_hosts", type=list, default=["localhost", "127.0.0.1"], description="允许的主机列表"
    )
    ...
    registry["database.host"] = ConfigField(
        name="database.host", type=str, default="localhost", env_var="DB_HOST", description="数据库主机地址"
    )
    ...
    registry["cors.allowed_origins"] = ConfigField(
        name="cors.allowed_origins", type=list, default=[], description="CORS 允许的来源列表"
    )

    registry["cors.trusted_origins"] = ConfigField(
        name="cors.trusted_origins", type=list, default=[], description="CSRF 信任的来源列表"
    )
    ...
    registry["chat_platforms.feishu.webhook_url"] = ConfigField(
        name="chat_platforms.feishu.webhook_url", type=str, env_var="FEISHU_WEBHOOK_URL", description="飞书 Webhook URL"
    )
    ...
    registry["chat_platforms.dingtalk.app_key"] = ConfigField(
        name="chat_platforms.dingtalk.app_key", type=str, env_var="DINGTALK_APP_KEY", description="钉钉应用 Key"
    )
    ...
    registry["chat_platforms.dingtalk.agent_id"] = ConfigField(
        name="chat_platforms.dingtalk.agent_id", type=str, env_var="DINGTALK_AGENT_ID", description="钉钉应用 Agent ID"
    )
    ...
    registry["features.case_chat.auto_create_on_push"] = ConfigField(
        name="features.case_chat.auto_create_on_push", type=bool, default=True, description="推送时自动创建聊天群"
    )
    ...
    registry["features.court_sms.auto_recovery"] = ConfigField(
        name="features.court_sms.auto_recovery", type=bool, default=True, description="是否启用自动恢复"
    )
    ...
    registry["pagination.max_page_size"] = ConfigField(
        name="pagination.max_page_size", type=int, default=100, min_value=1, max_value=10000, description="最大分页大小"
    )
    ...
    registry["validation.decimal_places"] = ConfigField(
        name="validation.decimal_places", type=int, default=2, min_value=0, max_value=10, description="金额字段小数位数"
    )
    ...
    registry["steering.conditional_loading.enabled"] = ConfigField(
        name="steering.conditional_loading.enabled", type=bool, default=True, description="是否启用条件加载"
    )
    ...
    for key, field in CONFIG_REGISTRY.items():
        ...
        if field.env_var:
            if field.env_var in env_vars:
                errors.append(
                    f"环境变量 '{field.env_var}' 被多个配置项使用: " f"'{env_vars[field.env_var]}' 和 '{key}'"
                )
            else:
