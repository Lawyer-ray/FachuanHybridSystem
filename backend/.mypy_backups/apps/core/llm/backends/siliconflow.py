...
class SiliconFlowBackend:
    ...
    async def achat(
        ...
        **kwargs: Any,
        ...
        """
        api_key = (
            self._config.api_key if (self._config and self._config.api_key) else await LLMConfig.get_api_key_async()
        )
        base_url = (
        ...
        **kwargs: Any,
        ...
        try:
            ...
            raise LLMAuthenticationError(
                message="SiliconFlow API Key 无效或缺失",
                errors={"detail": str(e)},
            ) from e
        except httpx.TimeoutException as e:
            logger.warning("SiliconFlow 请求超时", extra={"timeout": self.timeout, "error": str(e)})
...
if TYPE_CHECKING:
    _backend: ILLMBackend = SiliconFlowBackend()
