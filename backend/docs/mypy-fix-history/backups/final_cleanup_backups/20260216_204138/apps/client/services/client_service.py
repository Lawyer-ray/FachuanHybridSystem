...
User = get_user_model()  # type: ignore[valid-type]
...
class ClientService:
    ...
    def __init__(self, identity_doc_service: Optional["ClientIdentityDocService"] = None) -> None:
        """
        初始化服务
    ...
    def list_clients(
        ...
        user: Optional[User] = None,  # type: ignore[valid-type]
    ) -> QuerySet:
        """
        获取客户列表
    ...
    def get_client(self, client_id: int, user: Optional[User] = None) -> Client:  # type: ignore[valid-type]
        """
        获取客户
        ...
        if not client:
            raise NotFoundError(message=f"客户不存在", code="CLIENT_NOT_FOUND")
    ...
    @transaction.atomic
    def create_client(self, data: Dict[str, Any], user: Optional[User] = None) -> Client:  # type: ignore[valid-type]
        """
        创建客户
        ...
        logger.info(
            f"客户创建成功",
            extra={"client_id": client.id, "user_id": user.id if user else None, "action": "create_client"},
        )

        return client

    @transaction.atomic
    def update_client(self, client_id: int, data: Dict[str, Any], user: Optional[User] = None) -> Client:
        """
        更新客户
        ...
        logger.info(
            f"客户更新成功",
            extra={"client_id": client.id, "user_id": user.id if user else None, "action": "update_client"},
        )

        return client

    @transaction.atomic
    def delete_client(self, client_id: int, user: Optional[User] = None) -> None:  # type: ignore[valid-type]
        """
        删除客户
        ...
        logger.info(
            f"客户删除成功",
            extra={"client_id": client_id, "user_id": user.id if user else None, "action": "delete_client"},
        )
    ...
    def _check_create_permission(self, user: User) -> bool:  # type: ignore[valid-type]
        ...
        return user.is_authenticated and (  # type: ignore[no-any-return]
            user.has_perm("client.add_client") or user.is_admin or user.is_superuser
        )

    def _check_update_permission(self, user: User, client: Client) -> bool:  # type: ignore[valid-type]
        """检查更新权限（私有方法）"""
        return user.is_authenticated and (  # type: ignore[no-any-return]
            user.has_perm("client.change_client") or user.is_admin or user.is_superuser
        )

    def _check_delete_permission(self, user: User, client: Client) -> bool:  # type: ignore[valid-type]
        """检查删除权限（私有方法）"""
        return user.is_authenticated and (  # type: ignore[no-any-return]
            user.has_perm("client.delete_client") or user.is_admin or user.is_superuser
        )
...
class ClientServiceAdapter(IClientService):
    ...
    def __init__(self, client_service: Optional[ClientService] = None) -> None:
        """
        初始化适配器
