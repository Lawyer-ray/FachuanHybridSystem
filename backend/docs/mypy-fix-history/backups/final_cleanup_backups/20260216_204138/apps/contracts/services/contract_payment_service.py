...
from ..models import Contract, ContractFinanceLog, ContractPayment, InvoiceStatus, LogLevel
...
class ContractPaymentService:
    ...
    @transaction.atomic
    def update_payment(
        self, payment_id: int, data: Dict[str, Any], user: Any | None = None, confirm: bool = False
    ) -> ContractPayment:
        """
        更新收款记录
        ...
        if "invoiced_amount" in data or "invoice_status" in data:
            ...
            inv_status = self._calculate_invoice_status(
                invoiced_amount, float(obj.amount), data.get("invoice_status", obj.invoice_status)
            )
        ...
        obj.save()
        self._log_finance(
            obj.contract_id, self._get_user_id(user), "update_payment", "INFO", {"payment_id": obj.id, "old": old}
        )
        return obj
    ...
    def _calculate_invoice_status(
        self, invoiced_amount: float, amount: float, current_status: Optional[str] = None
    ) -> str:
        """
        计算发票状态
    ...
    def _log_finance(
        ...
        try:
            ContractFinanceLog.objects.create(
                contract_id=contract_id, action=action, level=level, actor_id=actor_id, payload=payload or {}
            )
        except Exception:
            pass
