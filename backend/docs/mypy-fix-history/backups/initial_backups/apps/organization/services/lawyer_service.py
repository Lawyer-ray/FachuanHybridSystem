...
class LawyerService:
    ...
    def get_lawyer(self, lawyer_id: int, user: Lawyer) -> Lawyer:
        ...
        if not lawyer:
            raise NotFoundError(message=f"律师不存在", code="LAWYER_NOT_FOUND")
    ...
    def list_lawyers(
        self, page: int = 1, page_size: int = 20, filters: dict = None, user: Lawyer = None
    ) -> QuerySet[Lawyer]:
        """
        列表查询
    ...
    def create_lawyer(self, data: LawyerCreateIn, user: Lawyer, license_pdf=None) -> Lawyer:
        ...
        # 6. 记录日志
        logger.info(f"律师创建成功", extra={"lawyer_id": lawyer.id, "user_id": user.id, "action": "create_lawyer"})
    ...
    def update_lawyer(self, lawyer_id: int, data: LawyerUpdateIn, user: Lawyer, license_pdf=None) -> Lawyer:
        ...
        # 6. 记录日志
        logger.info(f"律师更新成功", extra={"lawyer_id": lawyer.id, "user_id": user.id, "action": "update_lawyer"})
    ...
    def delete_lawyer(self, lawyer_id: int, user: Lawyer) -> None:
        ...
        # 5. 记录日志
        logger.info(f"律师删除成功", extra={"lawyer_id": lawyer_id, "user_id": user.id, "action": "delete_lawyer"})
    ...
    def _set_lawyer_teams(self, lawyer: Lawyer, team_ids: List[int], law_firm: Optional[LawFirm]) -> None:
        """设置律师团队（私有方法）"""
    ...
    def _set_biz_teams(self, lawyer: Lawyer, team_ids: List[int], law_firm: Optional[LawFirm]) -> None:
        """设置业务团队（私有方法）"""
