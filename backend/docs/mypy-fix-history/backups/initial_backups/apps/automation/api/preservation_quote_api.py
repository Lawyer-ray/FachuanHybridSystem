...
def create_preservation_quote(request: HttpRequest, data: PreservationQuoteCreateSchema):
    """
    创建询价任务
...
def list_preservation_quotes(
    request: HttpRequest, page: int = 1, page_size: Optional[int] = None, status: Optional[str] = None
):
    """
    列表查询询价任务
...
def get_preservation_quote(request: HttpRequest, quote_id: int):
    """
    获取询价任务详情
...
async def execute_preservation_quote(request: HttpRequest, quote_id: int):
    """
    执行询价任务
...
async def retry_preservation_quote(request: HttpRequest, quote_id: int):
    """
    重试询价任务
