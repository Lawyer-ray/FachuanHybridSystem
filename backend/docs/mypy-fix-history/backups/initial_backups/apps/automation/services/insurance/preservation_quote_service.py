...
class PreservationQuoteService:
    ...
    async def execute_quote(self, quote_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            companies = await self._fetch_insurance_companies(
                token=token, category_id=quote.category_id, corp_id=quote.corp_id
            )
            ...
            premium_results = await self._fetch_all_premiums(
                token=token, preserve_amount=quote.preserve_amount, corp_id=quote.corp_id, companies=companies
            )
            ...
            await sync_to_async(quote.save)(
                update_fields=["success_count", "failed_count", "status", "finished_at", "error_message"]
            )
    ...
    async def retry_quote(self, quote_id: int) -> Dict[str, Any]:
        ...
        if quote.status not in [QuoteStatus.FAILED, QuoteStatus.PARTIAL_SUCCESS]:
            logger.warning(
                "任务状态不允许重试",
                extra={"action": "retry_quote_invalid_status", "quote_id": quote.id, "current_status": quote.status},
            )
    ...
    def list_quotes(
        self, page: int = 1, page_size: Optional[int] = None, status: Optional[str] = None
        ...
        logger.info(
            "查询询价任务列表", extra={"action": "list_quotes", "page": page, "page_size": page_size, "status": status}
        )
    ...
    async def _fetch_insurance_companies(self, token: str, category_id: str, corp_id: str) -> List[InsuranceCompany]:
        ...
        logger.info(
            "开始获取保险公司列表",
            extra={"action": "fetch_insurance_companies_wrapper_start", "category_id": category_id, "corp_id": corp_id},
        )
        try:
            ...
            if not companies:
                logger.error(
                    "未获取到保险公司列表",
                    extra={"action": "fetch_insurance_companies_empty", "category_id": category_id, "corp_id": corp_id},
                )
                raise CompanyListEmptyError(message="未获取到保险公司列表，请检查分类 ID 和法院 ID 是否正确")
            logger.info(
                f"✅ 获取到 {len(companies)} 家保险公司",
                extra={"action": "fetch_insurance_companies_wrapper_success", "companies_count": len(companies)},
            )
            return companies
    ...
    async def _fetch_all_premiums(
        self, token: str, preserve_amount: Decimal, corp_id: str, companies: List[InsuranceCompany]
    ) -> List[PremiumResult]:
        """
        并发查询所有保险公司报价
        ...
        results = await self.insurance_client.fetch_all_premiums(
            bearer_token=token, preserve_amount=preserve_amount, corp_id=corp_id, companies=companies
        )
        return results
