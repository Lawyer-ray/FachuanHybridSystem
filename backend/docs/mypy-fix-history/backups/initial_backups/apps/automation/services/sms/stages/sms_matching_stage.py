    ...
    def _create_case_binding(self, sms: CourtSMS) -> bool:
        ...
        try:
            ...
            case_log = case_log_service.create_log(
                case_id=sms.case.id, content=f"收到法院短信：{sms.content}", user=user
            )
    ...
    def _add_case_numbers_to_case(self, sms: CourtSMS) -> None:
        ...
        try:
            ...
            for num in valid_nums:
                self.case_service.add_case_number_internal(case_id=sms.case.id, case_number=num, user_id=user_id)
        except Exception as e:
