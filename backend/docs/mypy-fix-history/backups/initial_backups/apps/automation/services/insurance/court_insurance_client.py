...
class CourtInsuranceClient:
    ...
    def __init__(self, token_service: Optional[ITokenService] = None):
        ...
        try:
            self._client = httpx.AsyncClient(
                limits=limits, timeout=self.default_timeout, http2=True, follow_redirects=True
            )
            http2_enabled = True
        except ImportError:
            logger.warning(
            ...
            self._client = httpx.AsyncClient(
                limits=limits, timeout=self.default_timeout, http2=False, follow_redirects=True
            )
            http2_enabled = False
        logger.info(
            "✅ httpx 客户端已初始化",
    ...
    async def fetch_insurance_companies(
        self, bearer_token: str, c_pid: str, fy_id: str, timeout: float = None, max_retries: int = 3
        ...
        for attempt in range(1, max_retries + 1):
            try:
                return await self._fetch_insurance_companies_once(
                    bearer_token=bearer_token, c_pid=c_pid, fy_id=fy_id, timeout=timeout, attempt=attempt
                )
    ...
    async def fetch_premium(
        self, bearer_token: str, preserve_amount: Decimal, institution: str, corp_id: str, timeout: float = None
    ) -> PremiumResult:
        """
        查询单个保险公司报价
        ...
        try:
            ...
            response = await self._client.post(
                self.premium_query_url, headers=headers, params=params, json=request_body, timeout=timeout
            )
        ...
        for i, result in enumerate(results):
            ...
            if isinstance(result, Exception):
                ...
                premium_results.append(
                    PremiumResult(
                        company=company, premium=None, status="failed", error_message=error_msg, response_data=None
                    )
                )
