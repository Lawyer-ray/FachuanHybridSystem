...
class DocumentDeliveryService:
    ...
    def _query_via_api(self, token: str, cutoff_time: datetime, credential_id: int) -> DocumentQueryResult:
        ...
        try:
            ...
            raise
        logger.info(
            f"API 查询完成: 发现={result.total_found}, 处理={result.processed_count}, 跳过={result.skipped_count}, 失败={result.failed_count}"
        )
        return result
                        ...
                        try:
                            if os.path.exists(file_path):
                                ...
                                case_log_service.upload_attachments(
                                    log_id=case_log.id, files=[uploaded_file], user=None, perm_open_access=True
                                )
