...
class CourtDocumentRecognitionService:
    ...
    def recognize_document(self, file_path: str, user: Optional[Any] = None) -> RecognitionResponse:
        """
        识别文书并绑定案件
        ...
        try:
            ...
            return RecognitionResponse(
                recognition=recognition, binding=binding, file_path=renamed_file_path  # 返回重命名后的路径
            )
    ...
    def recognize_document_from_text(self, text: str) -> RecognitionResult:
        ...
        logger.info(f"开始从文本识别文书", extra={"action": "recognize_document_from_text", "text_length": len(text)})
