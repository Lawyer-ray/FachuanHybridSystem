...
class RetryManager:
    ...
    def execute_with_retry(
        self, operation: Callable[[], Any], operation_name: str = "operation", context: Optional[Dict[str, Any]] = None
    ) -> Any:
        """执行带重试的操作
        ...
        while True:
            try:
                ...
                logger.warning(
                    f"操作失败: {operation_name}, 尝试 {attempt_number + 1}, 错误类型: {error_type.value}, 错误: {str(e)}"
                )
