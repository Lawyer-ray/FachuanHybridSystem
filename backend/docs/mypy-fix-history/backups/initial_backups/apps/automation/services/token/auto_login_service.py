...
from apps.core.interfaces import AccountCredentialDTO, IAutoLoginService, IBrowserService, LoginAttemptResult
...
class AutoLoginService:
    ...
    def __init__(self, retry_config: Optional[RetryConfig] = None, browser_service: Optional[IBrowserService] = None):
        """
        初始化自动登录服务
        ...
        try:
            ...
            if isinstance(e, (LoginFailedError, NetworkError, AutoTokenAcquisitionError)):
                ...
                raise LoginFailedError(
                    message=f"登录过程中发生未预期错误: {str(e)}", attempts=self._login_attempts.copy()
                )
    ...
    async def _login_with_retries(self, credential: AccountCredentialDTO) -> str:
        ...
        if isinstance(last_exception, NetworkError):
            ...
            raise LoginFailedError(
                message=f"网络重试失败，最后错误: {str(last_exception)}", attempts=self._login_attempts.copy()
            )
