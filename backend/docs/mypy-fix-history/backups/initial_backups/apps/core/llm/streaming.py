...
def _build_stream_kwargs(messages, model, temperature, max_tokens) -> dict[str, Any]:
...
async def astream_with_fallback(
    *,
    ...
    if backend and not fallback:
        async for chunk in get_backend(backend).astream(**kwargs):
            yield chunk
        return
    ...
    for name, backend_instance in backends_to_try:
        ...
        try:
            ait = backend_instance.astream(**kwargs).__aiter__()
