...
class PerformanceAnalyzer:
    ...
    def _analyze_performance_trend(self, history: List[LoadingPerformanceData]) -> Dict[str, Any]:
        ...
        second_half_avg = sum(moving_averages[len(moving_averages) // 2 :]) / (
            len(moving_averages) - len(moving_averages) // 2
        )
...
class SteeringPerformanceMonitor:
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "analysis": analysis,
            "recent_alerts": [
                {"level": alert.level.value, "message": alert.message, "timestamp": alert.timestamp}
