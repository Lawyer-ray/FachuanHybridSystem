...
class MigrationStateTracker:
    ...
    def __init__(self, db_path: Optional[str] = None, log_file: Optional[str] = None):
        """
        初始化状态跟踪器
    ...
    def fail_migration(self, migration_id: str, error_message: str, error_code: Optional[str] = None) -> None:
        """
        迁移失败
    ...
    def complete_step(self, migration_id: str, step_name: str, details: Optional[Dict[str, Any]] = None) -> None:
        """
        完成步骤
    ...
    def fail_step(
        self, migration_id: str, step_name: str, error_message: str, error_code: Optional[str] = None
    ) -> None:
        """
        步骤失败
    ...
    def record_warning(
        self, migration_id: str, warning_message: str, step_name: Optional[str] = None, config_key: Optional[str] = None
    ) -> None:
        """
        记录警告
    ...
    def get_migration_progress(self, migration_id: str) -> Optional[MigrationProgress]:
        ...
        """
        with self._get_db_connection() as conn:
            row = conn.execute("SELECT * FROM migration_progress WHERE migration_id = ?", (migration_id,)).fetchone()
    ...
    def get_migration_events(
        self, migration_id: str, event_types: Optional[List[MigrationEventType]] = None, limit: Optional[int] = None
    ) -> List[MigrationEvent]:
        """
        获取迁移事件
    ...
    def get_migration_statistics(self) -> MigrationStatistics:
        ...
        with self._get_db_connection() as conn:
            ...
            return MigrationStatistics(
                ...
                last_migration_time=(
                    datetime.fromisoformat(stats_row["last_migration"]) if stats_row["last_migration"] else None
                ),
            )
