...
- **移除 i18n 国际化**：移除所有 `_()` 翻译函数调用和 `django.utils.translation` 导入，简化代码结构
- **代码格式统一**：全量执行 `ruff format`，统一代码风格
- **mypy 类型检查修复**：修正 `type: ignore` 错误代码、移除多余的类型忽略注释
- **plugins 子模块化**：`backend/plugins` 迁移为独立 Git 子模块（FachuanPlugins）
