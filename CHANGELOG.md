...
- **归档占位符-案件案号**：支持多案号逗号分隔；案件审理结果从所有案号中取有内容的值。
- **裁判文书解析**：支持裁定类文书（撤诉裁定等）。
- **案件进展页面**：修复 progress.html 模板中文引号导致 TemplateSyntaxError。
- **案件当事人下拉框**：只显示关联合同/补充协议的当事人。
...
- **归档清单增强**：
  - 子项展示、拖拽排序、跨项移动、预览。
  - 收起/展开全部按钮，子项默认排序规则。
  - 子项默认全部展开，预览按钮改为 SVG 图标。
  - 委托合同/收费凭证等清单项无子项详情时正确展示。
- **案件日志删除按钮**：仅管理员可见。
- **案件进展-从邮件往来同步**：案件详情页"案件进展"增加"从邮件往来同步"按钮。
- **文件夹浏览器增强**：优先定位到关联合同文件夹；扫描弹窗增加"打开文件夹"按钮；案件编辑页案号区域增加"打开文件夹"按钮。
- **合同和案件列表默认筛选**：默认筛选 status=active。
