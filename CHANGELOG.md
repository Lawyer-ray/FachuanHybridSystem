...
### 新增
- 企业信用报告自动化：法人当事人编辑页新增"获取企业信用报告"按钮，自动完成登录→搜索→详情→申请报告全流程，用户只需手动完成极验验证码（共3次）
- `GsxtReportTask` 模型：记录报告申请任务状态（等待验证码/等待邮件/成功/失败）
- 当事人编辑页新增"企业信用报告任务" inline，显示任务状态、进度提示及163邮箱收件箱快捷链接
- 上传营业执照后自动将对应报告任务标记为成功
