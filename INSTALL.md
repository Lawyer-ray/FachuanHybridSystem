...
## 5. AI 平台配置（多 Key 并发 / 多平台）

系统通过后台「AI 平台」管理页（`/admin/core/llmprovider/`）支持 **多个 AI 平台、每个平台多个 API Key**，用于提升并发和接入不同厂商的大模型。AI 能力已统一在「AI 平台」页管理，不再使用「系统配置」下的 AI 服务项。

### 5.1 初始化 AI 服务

首次使用或空库时，点击列表页右上角「**初始化 AI 服务**」按钮，会幂等地写入一条基础平台（律所 kimi 默认地址 `http://116.196.92.175:8001/v1`、默认模型 `kimi26`）。已存在任何平台时按钮不会覆盖已有数据。

也可以在「AI 平台」列表页手动新增一条记录：

| 字段 | 示例 | 说明 |
| --- | --- | --- |
| 平台名称 | 律所 kimi | 任意可辨识名称，唯一 |
| API 地址 | `http://116.196.92.175:8001/v1` | OpenAI-compatible 接口地址 |
| API Keys | 每行一个 Key | **多 Key 自动轮询 + 失败切换**，用于提升并发 |
| 默认模型 | `kimi26` | 未指定模型时使用 |
| 模型列表 | `kimi26,kimi26-128k` | 该平台提供的模型，逗号分隔；按模型名自动路由到本平台 |
| 向量模型 | `text-embedding-v3`（可留空） | 该平台独立的向量模型；**留空表示不启用向量能力** |
| 每 Key 并发上限 | `3`（默认）或数字 | 单个 Key 同时进行的请求数上限；`0` 表示不限制 |
| 优先级 | `1` | 数字越小越优先；模型未匹配任何平台时用最高优先级平台 |
| 启用 | 勾选 | 停用即不再参与路由 |

> 本地 vLLM 无需鉴权时，API Keys 留空即可。
...
| --- | --- |
| 平台名称 | 小米 |
| API 地址 | 小米 OpenAI-compatible 服务地址 |
| API Keys | 小米平台分配的 Key |
| 默认模型 | `mimo-v1` |
| 模型列表 | `mimo-v1,mimo-v2` |
| 优先级 | `2` |

调用时按**模型名匹配平台**：请求 `kimi26` 走律所平台，请求 `mimo-v1` 走小米平台；匹配不到的模型使用优先级最高的平台。
