...
系统通过后台「AI 平台」管理页（`/admin/core/llmprovider/`）支持 **多个 AI 平台、每个平台多个 API Key**，用于提升并发和接入不同厂商的大模型。

### 5.1 配置一个平台（如律所自建 kimi 服务器）
...
「系统配置」里的 `OPENAI_COMPATIBLE_API_KEY / BASE_URL / DEFAULT_MODEL` 仍保留（兼容旧环境单 Key 用法）。配置了「AI 平台」后，平台配置优先生效；未配置任何平台时自动回退旧配置。
